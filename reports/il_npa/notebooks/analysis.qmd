...
# Set aggregation level for analysis: "poly" or "block"
ANALYSIS_LEVEL = "block"  # Options: "poly", "block"

# Set display labels based on aggregation level
unit_label = "blocks" if ANALYSIS_LEVEL == "block" else "construction polygons"
unit_label_singular = "block" if ANALYSIS_LEVEL == "block" else "construction polygon"
```

```{python}
# Load summary data based on aggregation level
if ANALYSIS_LEVEL == "block":
    geojson_files = sorted(outputs_dir.glob('peoplesgas_with_buildings_streets_block_*.geojson'))
    print(f"Loading block-level data...")
else:
    geojson_files = sorted(outputs_dir.glob('peoplesgas_with_buildings_streets_poly_*.geojson'))
    print(f"Loading polygon-level data...")

if not geojson_files:
    raise FileNotFoundError(f"No summary GeoJSON files found for ANALYSIS_LEVEL='{ANALYSIS_LEVEL}'. Run geo_data_cleaning.qmd first.")
...
if ANALYSIS_LEVEL == "block":
    ...
    print(f"  {len(pg_summary):,} construction polygons")
```
...
pg_summary["elec_cost_total_w_decomm"] = pg_summary["elec_cost_total"] + pg_summary["street_miles"] * cost_decomm_mile
...
pg_summary["lpp_over_npa_w_decomm"] = pg_summary["lpp_cost"] / \
    pg_summary["elec_cost_total_w_decomm"]


# bin the lpp_over_npa values
pg_summary["comp_cost_discrete"] = pg_summary["lpp_over_npa"].apply(
    lambda x: ("PRP cheaper" if x < 1 else "equal" if 1 <= x <=
               1.015 else "1-2 x" if 1.015 < x < 2 else "> 2")
)

pg_summary["comp_cost_discrete_res_only"] = pg_summary["lpp_over_npa"].apply(
    lambda x: ("PRP cheaper" if x < 1 else "equal" if 1 <= x <=
               1.015 else "1-2 x" if 1.015 < x < 2 else "> 2")
)
...
# bin the lpp_over_npa_w_decomm values
pg_summary["comp_cost_discrete_w_decomm"] = pg_summary["lpp_over_npa_w_decomm"].apply(
    lambda x: ("PRP cheaper" if x < 1 else "equal" if 1 <= x <=
               1.015 else "1-2 x" if 1.015 < x < 2 else "> 2")
)

# Overwrite comp_cost_discrete_w_decomm where ind_flag is True
pg_summary.loc[pg_summary["ind_flag"], "comp_cost_discrete_w_decomm"] = "not eligible"
...
```{python}
output_geojson = outputs_dir / f"final_peoplesgas_with_buildings_streets_{ANALYSIS_LEVEL}_{datetime.now().strftime('%Y%m%d')}.geojson"
pg_summary.to_file(output_geojson, driver="GeoJSON")
print(f"✅ Exported pg_summary to {output_geojson}")
```

# Summary stats no decommissioning costs
```{python}
# Calculate the percent where npa_over_lpp > 1
num_gt_1 = (pg_summary['lpp_over_npa'] > 1).sum()
total_units = len(pg_summary)
percent_gt_1 = (num_gt_1 / total_units) * 100

print(f"{percent_gt_1:.1f}% of {unit_label} have npa_over_lpp > 1")
...
```{python}
# Calculate the percent where lpp_over_npa > 1 and com_ind_flag == False
mask = (pg_summary["lpp_over_npa"] > 1) * (pg_summary["comm_ind_flag"] == False)
percent_gt_1_and_comm_ind_false = (mask.sum() / len(pg_summary)) * 100
print(f"{percent_gt_1_and_comm_ind_false:.1f}% of {unit_label} have lpp_over_npa > 1 and no commercial or industrial parcels")
```

## Industrial only

```{python}
# Calculate the percent where lpp_over_npa > 1 and ind_flag == False
mask_ind = (pg_summary['lpp_over_npa'] > 1) & (pg_summary['ind_flag'] == False)
percent_gt_1_and_ind_false = (mask_ind.sum() / len(pg_summary)) * 100
print(f"{percent_gt_1_and_ind_false:.1f}% of {unit_label} have lpp_over_npa > 1 and ind_flag == False")
```

# Summary stats with decommissioning costs
```{python}
# Calculate the percent where npa_over_lpp > 1 (with _dc suffix)
num_gt_1_dc = (pg_summary['lpp_over_npa_w_decomm'] > 1).sum()
total_units_dc = len(pg_summary)
percent_gt_1_dc = (num_gt_1_dc / total_units_dc) * 100

print(f"{percent_gt_1_dc:.1f}% of {unit_label} have npa_over_lpp > 1")
...
```{python}
# Calculate the percent where lpp_over_npa > 1 and com_ind_flag == False (with _dc suffix)
mask_dc = (pg_summary["lpp_over_npa_w_decomm"] > 1) * (pg_summary["comm_ind_flag"] == False)
percent_gt_1_and_comm_ind_false_dc = (mask_dc.sum() / len(pg_summary)) * 100
print(f"{percent_gt_1_and_comm_ind_false_dc:.1f}% of {unit_label} have lpp_over_npa > 1 and no commercial or industrial parcels")
```
...
```{python}
# Calculate the percent where lpp_over_npa > 1 and ind_flag == False (with _dc suffix)
mask_ind_dc = (pg_summary['lpp_over_npa_w_decomm'] > 1) & (pg_summary['ind_flag'] == False)
num_gt_1_and_ind_false_dc = mask_ind_dc.sum()
percent_gt_1_and_ind_false_dc = (num_gt_1_and_ind_false_dc / len(pg_summary)) * 100
print(f"{percent_gt_1_and_ind_false_dc:.1f}% of {unit_label} have lpp_over_npa > 1 and ind_flag == False")
```
...
filtered_data = pg_summary[np.isfinite(pg_summary['lpp_over_npa'])]
...
# Residential and Commerical (exclude industrial)
...
filtered_data = pg_summary[(pg_summary['lpp_over_npa'] > 10) & (pg_summary['ind_flag'] == False)]
map_title = f"{unit_label.capitalize()} with lpp_over_npa > 10 and no industrial parcels"
m = plot_pg_polygons_map(
    filtered_data,
    'pct_residential',
    discrete=False,
...
filtered_data = pg_summary[np.isfinite(pg_summary['lpp_over_npa_w_decomm'])]
m = plot_pg_polygons_map(
    filtered_data,
...
# Polygon-Level Rollup

Optional aggregation of block-level data to polygon level for comparison with original analysis.

```{python}
if ANALYSIS_LEVEL == "block":
    print("\n📊 Aggregating block-level data to polygon level for comparison...")

    # Aggregate metrics to polygon level
    poly_summary = pg_summary.groupby('OBJECTID').agg({
        'street_miles': 'sum',
        'sf_parcels': 'sum',
        'mf_parcels': 'sum',
        'mf_units': 'sum',
        'commercial_parcels': 'sum',
        'industrial_parcels': 'sum',
        'non_vacant_parcels': 'sum',
        'total_residential_units': 'sum',
        'lpp_cost': 'sum',
        'elec_cost_total': 'sum',
        'elec_cost_total_w_decomm': 'sum'
    }).reset_index()

    # Recalculate cost ratios at polygon level
    poly_summary["lpp_over_npa"] = poly_summary["lpp_cost"] / poly_summary["elec_cost_total"]
    poly_summary["lpp_over_npa_w_decomm"] = poly_summary["lpp_cost"] / poly_summary["elec_cost_total_w_decomm"]

    # Recalculate flags
    poly_summary["comm_ind_total_count"] = poly_summary["commercial_parcels"] + poly_summary["industrial_parcels"]
    poly_summary["comm_ind_flag"] = poly_summary["comm_ind_total_count"] > 0
    poly_summary["ind_flag"] = poly_summary["industrial_parcels"] > 0

    print(f"  Rolled up {len(pg_summary):,} blocks to {len(poly_summary):,} construction polygons")

    # Show comparison
    print(f"\n  Polygon-level totals (from block aggregation):")
    print(f"    Total street miles: {poly_summary['street_miles'].sum():.2f}")
    print(f"    Total parcels: {(poly_summary['sf_parcels'] + poly_summary['mf_parcels'] + poly_summary['commercial_parcels']).sum():,.0f}")
    print(f"    Total residential units: {poly_summary['total_residential_units'].sum():,.0f}")
else:
    print(f"\nSkipping rollup (already at polygon level)")
    poly_summary = None
```
