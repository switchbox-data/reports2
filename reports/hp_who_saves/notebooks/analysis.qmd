...
print(counts_baseline_heating_type)
```
...
plot_winter_vs_summer_peak_loads <- ggplot(
  housing_units_peak_loads |> filter(upgrade==0), aes(x = winter_peak_load, y = summer_peak_load)) +
geom_point() +
labs(x = "Winter Peak Load (kW)", y = "Summer Peak Load (kW)")
...
plot_annual_peak_growth_by_upgrade <- ggplot(
  housing_units_peak_loads |> filter(upgrade %in% c(1, 2, 3)),
  aes(x = annual_peak_load_pct)
) +
  geom_histogram(binwidth = 0.05) +
  facet_wrap(~ upgrade, ncol = 1) +
  labs(
    x = "Annual Peak Load Pct",
    y = "Number of Homes",
...
hist_winter_peak_load_pct_hist_upgrade0 <- ggplot(
  housing_units_peak_loads |> filter(upgrade==0), aes(x = annual_peak_load_pct)
) +
  geom_histogram(binwidth = 0.05, fill = "steelblue", color = "white") +
  labs(
    x = "Winter Peak Load Pct",
    y = "Number of Homes",
    title = "Upgrade 0"
  ) +
...
hist_winter_peak_load_pct_hist_upgrade1 <- ggplot(
  housing_units_peak_loads |> filter(upgrade==1), aes(x = annual_peak_load_pct)
) +
  geom_histogram(binwidth = 0.05, fill = "steelblue", color = "white") +
  labs(x = "Winter Peak Load Pct", y = "Number of Homes", title = "Upgrade 1") +
  scale_x_continuous(limits = c(0, 5), breaks = seq(0, 5, by = 0.5), labels = scales::percent_format(accuracy = 1))

# Upgrade 2
hist_winter_peak_load_pct_hist_upgrade2 <- ggplot(
  housing_units_peak_loads |> filter(upgrade==2), aes(x = annual_peak_load_pct)
) +
  geom_histogram(binwidth = 0.05, fill = "steelblue", color = "white") +
  labs(
    x = "Winter Peak Load Pct",
...
hist_winter_peak_load_pct_hist_upgrade3 <- ggplot(
  housing_units_peak_loads |> filter(upgrade==3), aes(x = annual_peak_load_pct)
) +
  geom_histogram(binwidth = 0.05, fill = "steelblue", color = "white") +
  labs(x = "Winter Peak Load Pct", y = "Number of Homes", title = "Upgrade 3") +
  scale_x_continuous(limits = c(0, 5), breaks = seq(0, 5, by = 0.5), labels = scales::percent_format(accuracy = 1))
