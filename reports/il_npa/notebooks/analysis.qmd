...
pg_summary_projected <- pg_summary |>
  st_transform(32616)

pg_summary <- pg_summary |>
  mutate(
    block_perimeter_miles = st_length(pg_summary_projected$geometry) / 1609.34,
    block_area_sq_km = st_area(pg_summary_projected$geometry) / 1e6,
    perimeter_to_street_miles_ratio = street_miles / as.numeric(block_perimeter_miles),
    parcels_per_sq_km = non_vacant_parcels / as.numeric(block_area_sq_km)
  )
...
series <- pg_summary |>
  filter(is.finite(npa_over_prp_w_decomm)) |>
  pull(npa_over_prp_w_decomm)

ggplot(data.frame(x = series), aes(x = x)) +
  geom_histogram(bins = 50, fill = 'coral', color = 'black', alpha = 0.7) +
  geom_vline(xintercept = median(series),
             color = 'red', linetype = 'dashed', linewidth = 1) +
  labs(x = 'npa_over_prp_w_decomm', y = 'Count',
       title = 'Histogram of npa_over_prp_w_decomm') +
  theme_minimal() +
  theme(panel.grid = element_line(alpha = 0.3))

cat("Summary statistics for npa_over_prp_w_decomm:\n")
cat("  Count:", length(series), "\n")
cat("  Mean:", round(mean(series), 2), "\n")
cat("  Median:", round(median(series), 2), "\n")
cat("  Std Dev:", round(sd(series), 2), "\n")
cat("  Min:", round(min(series), 2), "\n")
cat("  Max:", round(max(series), 2), "\n")
```
