...
for run_label, run_path in run_dirs.items():
    ...
    run_ht_summary["run_label"] = run_label
...
# Marginal economic burden is benchmark-independent: deduplicate to one row per (run, group)
marginal_data = (
    cost_mix_long[cost_mix_long["cost_source"] == "Marginal (economic burden)"]
    .drop_duplicates(subset=["run_label", "group"])
)

(
    ggplot(marginal_data, aes(x="group", y="musd_total_per_year", fill="group"))
    + geom_col(width=0.6)
    + facet_wrap("~run_label", ncol=2)
    + labs(
        title="Marginal Economic Burden by Run and Customer Group ($M/year)",
        x="Customer group",
        y="$ million per year",
        fill="Group",
    )
    + theme_minimal()
    + theme(figure_size=(8, 3), legend_position="none")
)
```

```{python}
# Residual allocation varies by benchmark — show all three side-by-side per group
residual_data = cost_mix_long[cost_mix_long["cost_source"] == "Residual allocation"]

(
    ggplot(residual_data, aes(x="group", y="musd_total_per_year", fill="benchmark"))
    + geom_col(position=position_dodge(width=0.8), width=0.7)
    + facet_wrap("~run_label", ncol=2)
    + labs(
        title="Residual Allocation by Benchmark, Run, and Customer Group ($M/year)",
        x="Customer group",
        y="$ million per year",
        fill="Residual benchmark",
    )
    + theme_minimal()
...
```{python}
(
    ggplot(hourly_by_heating_type, aes(x="time", y="load_kwh", fill="heating_type"))
...
```{python}
(
    ggplot(hourly_by_heating_type, aes(x="time", y="load_kwh", color="heating_type"))
...
```{python}
ht_bat_long = ht_summary_all.melt(
    id_vars=["group", "run_label"],
    value_vars=["BAT_vol_weighted_avg", "BAT_peak_weighted_avg", "BAT_percustomer_weighted_avg"],
    var_name="bat_key",
    value_name="bat_usd_per_customer_year",
).assign(
    bat_label=lambda d: d["bat_key"].map(
        {
