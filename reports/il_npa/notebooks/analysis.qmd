...
if street_files:
    ...
    if len(streets) > 0:
        ...
        if 'street_nam' in streets.columns:
            ...
            print(f"  ⚠️ Missing street properties - may need to re-download")
...
if parcels_classified is not None and buildings is not None and len(buildings) > 0:
    ...
    # Apply fallback logic for missing unit data
    # 1. Single-family: set to 1 unit
    ...
    mf_mask = (parcels_with_units['sf_mf'] == 'multi-family') & \
              (parcels_with_units['building_units'].isna()) & \
...
if pg_polygons is not None:
    # Create summary dataframe indexed by OBJECTID
    summary = pd.DataFrame(index=pg_polygons['OBJECTID'])
    summary['OBJECTID'] = pg_polygons['OBJECTID'].values

    # Add metrics (these are already indexed by OBJECTID from groupby operations)
...
if summary is not None and pg_polygons is not None:
    ...
    # Write to GeoJSON
    print(f"\n💾 Exporting summary dataset to GeoJSON...")
