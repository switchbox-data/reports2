...
#-----------------------------------------------------------------
# Update baseline heating type and set as factor
...
annual_changes_y  <- annual_changes_total |>
  ...
  left_join(housing_units |> filter(upgrade==0)|> filter(baseline_heating_type == "Natural Gas")|> select(bldg_id, afue_value), by = c("bldg_id"))
...
annual_changes_x <- annual_changes_y |>
  summarize(
    counts = n(),
    median_annual_change = median(annual_changes_y),
    .by = c(afue_value)
  )
