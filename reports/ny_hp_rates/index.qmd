...
$$w_h = \underbrace{\phi_s}_{\text{season share}} \cdot \underbrace{\frac{\max(L_h - T_s,\ 0)}{\sum_{h' \in \text{season } s} \max(L_{h'} - T_{s'},\ 0)}}_{\text{within-season share}}$$
...
CenHud's workbook dilutes by peak-share (project MW ÷ local peak) and reports flat nominal dollars across years. To
compare across utilities we re-normalize on a capacity-based denominator (system peak MW) and apply a 2.1%/yr cost
escalator so the nominal-flat series becomes a real-2026 time series consistent with the other six utilities
...
are identical to NiMo/ConEd/O&R/CenHud:

$$\text{Annualized \$/kW} = \frac{\text{Total Capital}}{\text{Capacity}} \times r$$

where:

- $\text{Total Capital}$ — total capital cost of the project
- $\text{Capacity}$ — nameplate capacity of the project (kW)
- $r$ — composite annualization rate (Economic Carrying Charge Rate)

scoped by in-service year. Loss factors from W4 are applied when computing the "Total at Primary" column. No projects enter
...
$$MC(Y) = \frac{\sum_j F_j \times C_j}{P_{\text{system}}}$$

where:

- $MC(Y)$ — diluted marginal cost for year $Y$ ($/kW)
- $F_j$ — base-year (FY2026) annualized cost per MW for project $j$
- $C_j$ — capacity of project $j$ (MW)
- $P_{\text{system}}$ — system peak demand (MW)
