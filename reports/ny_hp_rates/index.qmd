...
Having determined the correct delivery {{< glossary "Revenue requirement" display="revenue requirement" >}} for heat pump customers at each utility, the next question is how to collect it.

We design and model the impact of a simple {{< glossary "Seasonal rate" display="seasonal heat pump rate" >}} for every utility in New York state. By design, these rates would lower heat pump customer delivery bills in every winter month while keeping summer bills the same — so no heat pump customer would pay more on the heat pump rate than on their current {{< glossary "Default rates" display="default rate" >}} in any month. This rate would only be available to heat pump customers, and would only apply to the delivery side of the bill.

This rate is calibrated to each utility's heat pump revenue requirement.
In summer, the {{< glossary "Volumetric charges" display="volumetric" >}} delivery rate equals the utility's current
...
The exact winter rate would be unique to each electric utility, reflecting the {{< glossary "Cost of service" display="cost of service" >}} of heat pump customers in that territory.
...
After installing heat pumps under {{< glossary "Default rates" display="default rates" >}}, `{python} pct(v.pct_burdened_hp_default_lmi40)` of these households
...
After installing heat pumps under the modeled seasonal heat pump rate,
`{python} pct(v.pct_burdened_hp_seasonal_lmi40)` of these households would still be highly burdened. This assumes that LMI households that install heat pumps also adopt the modeled seasonal rate.

We also model a scenario in which every eligible LMI household is enrolled in existing electric and gas discounts at the time of heat pump installation — not just those already enrolled today.

The final bar in @fig-burden-bar-lmi-current shows what post-heat pump installation burdens would look like under this scenario — seasonal heat pump rate adoption plus full LMI-discount enrollment. The number of highly energy burdened households would drop by
...
If all heat pump customers opted in to {{< glossary "Seasonal rate" display="seasonal heat pump rates" >}}, this {{< glossary "Cross-subsidy" display="cross-subsidy" >}} would be removed. The {{< glossary "Volumetric charges" display="volumetric" >}} delivery rate
...
per year**, and it gets spread across `{python} f"{v.nonhp_households_millions:,.1f}"` million non-heat-pump households.
...
The {{< glossary "Seasonal rate" display="seasonal heat pump rate" >}} corrects one {{< glossary "Cross-subsidy" display="cross-subsidy" >}} — the one heat pump customers pay into. But heat pump customers are not the only New Yorkers overpaying for delivery. {{< glossary "Electric resistance heating" display="Electric resistance" >}} customers overpay by **`{python} dollar(abs(v.elec_resistance_mean_delivery_bat))`** per year on average — slightly more per household than heat pump customers — and there are far more of them than there are electric customers with heat pumps. Together, these electric heating customers generate all of the cross-subsidy flowing to households that heat with fossil fuels lke natural gas, oil, and propane.

What if we fixed both overpayments at once? We can calculate delivery {{< glossary "Cost of service" display="cost of service" >}} for heat pump _and_ electric resistance customers together, set a combined delivery {{< glossary "Revenue requirement" display="revenue requirement" >}} for this "electric heating" group, and design a single seasonal rate calibrated to collect that revenue, and no more. Every electric heating customer in the state would be eligible — heat pump customers and electric resistance customers alike. If all such customers enrolled, the rate would eliminate the cross-subsidy from all of them to fossil-heated households.
...
The seasonal design mirrors the one we used for the heat pump-only rate: the summer {{< glossary "Volumetric charges" display="volumetric" >}} rate matches the current {{< glossary "Default rates" display="default rate" >}} — so no electric heating customer pays more in summer than they do today — and the winter rate is lowered enough to bring electric heating customers' annual delivery revenue in line with their combined cost of service.

If every heat pump and electric resistance customer opted in, what would happen to the overpayments and underpayments we saw earlier?

::::: {.column-screen-inset-right}
{{< embed notebooks/cost_of_service_by_subclass.qmd#tbl-eh-seasonal-sw-avg-pc >}}
:::::

By design, the electric heating rate collects annual delivery revenue from heat pump and electric resistance customers equal to their combined cost of service, thereby essentially eliminating overpayments. And because a larger cross-subsidy is being unwound, the underpayments by natural gas, oil, and propane customers shrink further than they did under the heat pump-only rate.

In other words, this rate would fix the {{< glossary "Fairness (rate design)" display="fairness" >}} problem that affects all electric heating customers in New York State. What would that mean for their bills?
...
Under default rates, only **`{python} pct(v.pct_natgas_save_default_lmi40)`** of gas-heated households would save by switching to heat pumps. Under the electric heating rate, **`{python} pct(v.pct_natgas_save_ehrate_lmi40)`** would — essentially matching the heat pump-only rate's **`{python} pct(v.pct_natgas_save_hprate_lmi40)`**. And the share losing more than $1,000 per year drops from `{python} pct(v.pct_natgas_lose_1k_default_lmi40)` to just `{python} pct(v.pct_natgas_lose_1k_ehrate_lmi40)`.

In other words, extending eligibility to electric resistance customers does not weaken the case for switching away from gas: the summer rate is unchanged, the winter rate is only slightly higher, and gas-to-heat-pump bill improvements are essentially the same.

### What happens to electric resistance customers under the electric heating rate?

Electric resistance customers that keep their existing equipment but adopt the electric heating rate would see their bills fall — because the rate finally collects delivery revenue from them in line with their cost of service.
...
Electric resistance customers have been overpaying for delivery for decades — not because they cause disproportionately high capacity costs, but because {{< glossary "Flat volumetric rates" display="flat volumetric delivery rates" >}} overcharge any customer who uses more electricity than average. Under the modeled electric heating rate, that overcharge goes away.

### Does lowering electric resistance bills weaken the case for switching to heat pumps?
...
The electric heating rate narrows the operating-cost gap between heat pumps and electric resistance, but the gap remains substantial — approximately a thousand dollars a year, on average. And for the electric resistance population, operating costs aren't the main barrier to heat pump adoption in the first place: the upfront cost of new equipment is.

### Bill impact on non-electric-heating customers

The heat pump-only seasonal rate removes a cross-subsidy of **`{python} f"${v.hp_total_cross_subsidy_delivery_millions:,.0f}"` million** per year. The electric heating rate removes the overpayment from electric resistance customers as well — another **`{python} f"${v.elec_resistance_total_cross_subsidy_delivery_millions:,.0f}"` million** per year — bringing the total cross-subsidy removed to **`{python} f"${v.eh_total_cross_subsidy_delivery_millions:,.0f}"` million** per year. That larger correction is absorbed by the customers left on the default rate: households that heat with natural gas, oil, and propane.
...
On average, non-electric-heating households would pay **`{python} dollar(v.mean_monthly_fossil_delta_ehrate, 2)`** more per month — roughly five times the **`{python} dollar(v.rebal_nonhp_mean_monthly_delta, 2)`** per month month impact of the heat pump-only rate, reflecting the larger cross-subsidy being unwound. Even so, **`{python} pct(v.pct_fossil_monthly_under_5_ehrate)`** would see a monthly change of less than $5, and **`{python} pct(v.pct_fossil_monthly_under_15_ehrate)`** would see their bills go up by $15 per month or less.

As with the heat pump-only rate, in a policy context this would not be a rate hike; it would be the removal of a cross-subsidy that gas, oil, and propane customers have been receiving for years from their electric-heating neighbors — a cross-subsidy that runs directly against New York's own policy of electrifying buildings. Extending the seasonal rate to all electric heating customers would end that cross-subsidy in full, rather than removing just the portion attributable to heat pump customers.
...
For each utility in New York, we've therefore designed TOU rates that cover both supply and delivery, specifically for heat pump customers. These rates are both {{< glossary "Cost-based rate" display="cost-based" >}} _and_ {{< glossary "Cost-reflective rate" display="cost-reflective" >}}: the on-peak and off-peak prices are aligned with the underlying {{< glossary "Marginal costs" display="marginal" >}} energy and {{< glossary "Capacity costs" display="capacity costs" >}}, while the total delivery revenue collected from heat pump customers remains the same as under the seasonal rate — preserving the {{< glossary "Cost allocation" display="cost allocation" >}} that eliminates the {{< glossary "Cross-subsidy" display="cross-subsidy" >}}. Supply revenue is also collected at the same total, but redistributed across peak and off-peak hours to better reflect when generation costs actually occur.
...
Adding {{< glossary "Cost-reflective rate" display="cost-reflectiveness" >}} to the {{< glossary "Seasonal rate" display="seasonal rate" >}} improves gas-to-heat-pump bill outcomes modestly (@fig-quadrant-bar-natgas-tou-state).
...
To be clear: the TOU rate would eliminate the {{< glossary "Cross-subsidy" display="cross-subsidy" >}} as well as the {{< glossary "Seasonal rate" display="seasonal rate" >}}, and would save heat pump customers slightly more money. From that perspective, it is superior to the seasonal rate we modeled earlier.

Nearly all of improvement in bill impact, compared to the {{< glossary "Default rates" display="default rate" >}}, comes from the fact that the rate is **{{< glossary "Cost-based rate" display="cost-based" >}}**, not from the fact that it is **{{< glossary "Cost-reflective rate" display="cost-reflective" >}}**: in other words, nearly all of the improvement does _not_ come from the inclusion of two price levels (that reflect the underlying {{< glossary "Marginal costs" display="marginal costs" >}} of energy and infrastructure) per se, but from the fact that the "average" price level is lower than under the default rate.
...
If implemented, the heat pump rates modeled above would be a medium-term solution: they would need to be gradually adjusted over time, and eventually replaced if the grid become fully winter-peaking.
...
In the short-term, heat pump adoption will only trigger **generation {{< glossary "Capacity costs" display="capacity costs" >}}** to the extent that it drives up the **summer peak**, the maxium annual demand that determines how much generation capacity the system needs. [^which-upgrades]
...
The heat pump rate modeled in this report is a short- to medium-term fix. It corrects today's significant {{< glossary "Cross-subsidy" display="cross-subsidy" >}}, where heat pump customers overpay for delivery because the system has spare winter capacity.
...
1. On-peak and off-peak prices are set ahead of time rather than responding to real-time conditions. On any given day, the on-peak price will likely overshoot or undershoot the underlying marginal energy and T&D capacitycosts.
