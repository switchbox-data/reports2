...
hist_for_single_rate_version <- function(
  ...
  category_percentages <- plot_data %>%
    ...
    bind_rows(
      ...
      tibble(
        ...
        count = sum(.$count[.$bill_category %in% c("small_savings", "large_savings")]),
        percentage = sum(.$percentage[.$bill_category %in% c("small_savings", "large_savings")])
      )
    ) %>%
    bind_rows(
      .,
      tibble(
        ...
        count = sum(.$count[.$bill_category %in% c("small_increase", "large_increase")]),
  ...
  px_hist_by_rate_version <- ggplot(binned_data, aes(x = bin_mid, y = count, fill = bill_category)) +
    ...
    labs(
      title = if (title == "auto") {
        title = case_when(
          bill_type == "annual_change_total" ~ glue::glue("Change in Total {season} Energy Bills after Switching to Heat Pump from {baseline_heating_type_option}"),
          bill_type == "annual_change_elec" ~ glue::glue("Switching from {baseline_heating_type_option} to Heat Pump: Change in {season} Electric Bill"),
          bill_type == "annual_change_gas" ~ glue::glue("Switching from {baseline_heating_type_option} to Heat Pump: Change in {season} Gas Bill"),
          bill_type == "annual_change_fuel_oil" ~ glue::glue("Switching from {baseline_heating_type_option} to Heat Pump: Change in {season} Fuel Oil Bill"),
          bill_type == "annual_change_propane" ~ glue::glue("Switching from {baseline_heating_type_option} to Heat Pump: Change in {season} Propane Bill"),
...
plot_bill_change_histograms <- function(
  ...
  combined_plot <- p1 / p2 / p3 +
    ...
    plot_annotation(
      title = case_when(
        bill_type == "annual_change_total" ~ glue::glue("Change in Total Annual Energy Bills after Switching to Heat Pump from {baseline_heating_type_option}"),
        bill_type == "annual_change_elec" ~ glue::glue("Switching from {baseline_heating_type_option} to Heat Pump: Change in Annual Electric Bill"),
        bill_type == "annual_change_gas" ~ glue::glue("Switching from {baseline_heating_type_option} to Heat Pump: Change in Annual Gas Bill"),
        bill_type == "annual_change_fuel_oil" ~ glue::glue("Switching from {baseline_heating_type_option} to Heat Pump: Change in Annual Fuel Oil Bill"),
        bill_type == "annual_change_propane" ~ glue::glue("Switching from {baseline_heating_type_option} to Heat Pump: Change in Annual Propane Bill"),
        TRUE ~ "Heat Pump Rate Bill Changes" # Default case
      ),
...
plot_energy_burden_histogram_standalone <- function(data, name, x_limits = c(0, 1.0), y_limits = c(0, 50000/242.13), binwidth = 0.01, show_x_axis = TRUE, show_y_axis = TRUE) {
