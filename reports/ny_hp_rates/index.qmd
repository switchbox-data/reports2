...
central air conditioning. It pays `{python} dollar(v.median_energy_total)` for energy every year — close to the median
bill for gas-heated homes in the state.[^buffalo_home]
...
In the case of the Buffalo home, its annual electricity use and its annual electric bill both grow by
...
To illustrate, let's return to our Buffalo home, which is served by NYSEG, and has an annual delivery
...
After switching to the heat pump, this particular homes's delivery cost-of-service is unchanged, because its new electricity use is highly concentrated in the winter, during hours when marginal capacity costs are zero.[^notmuch]
...
year. In other words: if NYSEG's delivery rates were fair, the home's *total* electric bill would shrink by
...
The heat pump rate proposed above has planned obsolescence built in: it is intended to be medium-term solution, to be gradually adjusted over time, and eventually replaced once the grid become fully winter-peaking.
...
While the grid has spare capacity in winter at present, this will not always be the case. Each customer installing a heat pump nudges up the winter peak of their feeder, substation, local transmission lines, bulk transmission line, and statewide generators by a tiny amount. Over time, these increases will add up, and eventually drive one or more of these components to become constrained in winter.

As this begins to happen across the state, heat pump adoption will become the main driver of grid capacity investment, the cost-of-service of heat pump customers will gradually increase, and the rate will need to be adjusted upwards to reflect that.

So how quickly will New York State's grid become winter-peaking?

It will not happen all at once, or at the same time for all utilities, and it will depend on the pace of heat pump adoption.[^perf-caveat]
...
#| echo: false
#| output: asis
print(Path("cache/bat_by_utility_table.html").read_text())
```
:::
...
#| echo: false
#| output: asis
print(Path("cache/tbl_seasonal_elasticities.html").read_text())
```
:::
