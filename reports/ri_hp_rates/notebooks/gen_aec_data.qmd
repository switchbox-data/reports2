...
```{python}
    def write_to_gsheet(df, sheet_name, worksheet_name):
        ...
        if worksheet_name not in [worksheet.title for worksheet in worksheets]:
            sh.add_worksheet(worksheet_name, df.height + 1, df.width)
        else:
...
```{python}
    for col in df_bau_loads.columns[1:]:
        merged_df = df_bau_loads[["timestamp", col]].rename({col: f"bau_{col}"}).join(df_hp_loads[["timestamp",    col]].rename({col: f"hp_{col}"}), on="timestamp").with_columns([(pl.col(f"hp_{col}") - pl.col(f"bau_{col}")).alias(f"{col}_delta"),  pl.col("timestamp").dt.strftime("%Y-%m-%d %H:%M:%S").alias("timestamp")])
        print(merged_df.head())
        write_to_gsheet(merged_df, f"test_data_sheet", f"1_{col}")
```
