...
library(ggrepel)
library(here)
here::i_am("reports/il_npa/notebooks/analysis.qmd")

outputs_dir <- here("reports", "il_npa", "data", "outputs")
if (!dir.exists(outputs_dir)) {
  dir.create(outputs_dir, recursive = TRUE)
}

USE_PROJECT_COSTS <- TRUE

source(here("lib", "ggplot", "switchbox_theme.R"))
# Match showtext's text-rendering DPI to Quarto's actual chart device
# DPI (~96).  The shared theme leaves it at 300 (designed for retina
# raster output), which makes 12pt labels render as ~37pt on Quarto's
# 96-DPI device — every label overlaps its bars and clips the panel.
...
if (USE_PROJECT_COSTS) {
  geojson_files <- list.files(outputs_dir,
    pattern = "peoplesgas_with_buildings_streets_block_.*_with_est_prp\\.geojson$",
    full.names = TRUE) |> sort()
} else {
  geojson_files <- list.files(outputs_dir,
    pattern = "peoplesgas_with_buildings_streets_block_[^_]*\\.geojson$",
    full.names = TRUE) |> sort()
}

if (length(geojson_files) == 0) {
  stop("No summary GeoJSON files found. Run geo_data_cleaning.qmd (and allocate_prp_costs.py if USE_PROJECT_COSTS=TRUE) first.")
}
...
# PRP program scope and total spending — loaded from input sheet so the
# system-wide denominator and total program cost stay in sync with PGL's
# filings. `lpp_program_3` is the 1,020-mile remaining CI/DI scope ordered
# by the 2024 SMP Investigation Order; `lpp_spending_total` is the implied
# total PRP capital through 2034 from the year-by-year trajectory in
# NSG-PGL Ex. 1.0 (Eidukas) at 12-13:240-247.
total_prp_miles <- prp_df |>
  filter(variable == "lpp_program_3") |>
  pull(value)
lpp_spending_total <- prp_df |>
  filter(variable == "lpp_spending_total") |>
  pull(value)
...
### Electrification cost derivation

The per-home electrification costs ($C_{\text{SF}}$ and $C_{\text{MF}}$) are derived from ComEd's CY2024 Cost-Effectiveness Analysis, which reports average costs for individual electrification measures deployed through ComEd's Whole Home Electric program. The source data is in the `comed_itemized cots` tab of our [cost assumptions spreadsheet](https://docs.google.com/spreadsheets/d/1xxa47dClvp0rosZhUP1R7790CNXMLSD_0ExrPccR3p0/edit?gid=681639934#gid=681639934).

For each end-use category, the ComEd data reports multiple program variants (e.g. DAC and non-DAC, or different equipment types like ducted vs. ductless heat pumps). We compute a **quantity-weighted average** across variants within each category: each variant's average cost is weighted by the number of units deployed in CY2024, so variants with more installations pull the average toward their cost. We then sum the weighted averages across end-use categories to get the total per-home or per-unit cost.

```{r}
#| echo: false

itemized_url <- "https://docs.google.com/spreadsheets/d/1xxa47dClvp0rosZhUP1R7790CNXMLSD_0ExrPccR3p0/export?format=csv&gid=681639934"
itemized_df <- read_csv(itemized_url) |>
  mutate(
    `Average Cost` = as.numeric(gsub("[\\$,]", "", `Average Cost`)),
...
# SF electrification cost breakdown (rows 58-67 in the sheet = "Whole Home Electric, SF")
sf_dryer <- itemized_df |>
  filter(`Savings Category` %in% c(
    "Clothes Dryer - Electrification - DAC",
    "Clothes Dryer - Electrification"
  ), Component == "SF")
sf_dryer_cost <- weighted.mean(sf_dryer$`Average Cost`, sf_dryer$Quantity)

sf_cooktop <- itemized_df |>
  filter(`Savings Category` %in% c(
    "Cooktop - Electrification - DAC",
    "Cooktop - Electrification"
  ), Component == "SF")
sf_cooktop_cost <- weighted.mean(sf_cooktop$`Average Cost`, sf_cooktop$Quantity)

sf_hpwh <- itemized_df |>
  filter(`Savings Category` %in% c(
    "Heat Pump Water Heater - Electrification - DAC",
    "Heat Pump Water Heater - Electrification"
  ), Component == "SF")
sf_hpwh_cost <- weighted.mean(sf_hpwh$`Average Cost`, sf_hpwh$Quantity)

sf_hvac <- itemized_df |>
  filter(`Savings Category` %in% c(
    "Ductless Heat Pump - Electrification - DAC",
    "Ductless Heat Pump - Electrification",
    "Air Source Heat Pump - Electrification - DAC",
    "Air Source Heat Pump - Electrification"
  ), Component == "SF")
sf_hvac_cost <- weighted.mean(sf_hvac$`Average Cost`, sf_hvac$Quantity)

sf_total <- sf_dryer_cost + sf_cooktop_cost + sf_hpwh_cost + sf_hvac_cost

# MF electrification cost breakdown (rows 49-57 in the sheet = "Whole Home Electric, MF")
mf_dryer <- itemized_df |>
  filter(`Savings Category` %in% c(
    "Clothes Dryer - Electrification - DAC",
    "Clothes Dryer - Electrification"
  ), Component == "MF")
mf_dryer_cost <- weighted.mean(mf_dryer$`Average Cost`, mf_dryer$Quantity)

mf_cooktop <- itemized_df |>
  filter(`Savings Category` == "Cooktop - Electrification - DAC", Component == "MF")
mf_cooktop_cost <- mf_cooktop$`Average Cost`

mf_hw <- itemized_df |>
  filter(`Savings Category` %in% c(
    "Heat Pump Water Heater - Electrification - DAC",
    "Heat Pump Water Heater - Electrification",
    "Electric Water Heater - Electrification - DAC"
  ), Component == "MF")
mf_hw_cost <- weighted.mean(mf_hw$`Average Cost`, mf_hw$Quantity)
...
mf_hvac_cost <- weighted.mean(mf_hvac$`Average Cost`, mf_hvac$Quantity)

mf_total <- mf_dryer_cost + mf_cooktop_cost + mf_hw_cost + mf_hvac_cost
```

The table below shows the itemized costs for each end-use category. Each row is a quantity-weighted average of the program variants within that category. Where costs differ across variants, a range (min–max) is shown.
...
Let's verify these match the values in our cost assumptions sheet:

```{r}
#| echo: false

stopifnot(abs(sf_total - cost_elec_sf) < 1)
stopifnot(abs(mf_total - cost_elec_mf) < 1)
```
...
pg_summary <- pg_summary |>
  mutate(
    prp_cost = if_else(
      USE_PROJECT_COSTS & !is.na(est_prp),
      est_prp,
      street_miles * cost_lpp_mile
    ),
    ...
    # Split elec_cost_total into the two underlying rate-base ledgers.
    # Grid upgrades are recovered through ComEd electric rates (per ComEd's
    # ICC-approved delivery charges and Multi-Year Integrated Grid Plan),
    # not through PGL gas rates and not directly out of household pockets.
    grid_upgrade_cost_block = grid_upgrade_cost_hh *
      (sf_parcels + mf_units),
    elec_pgl_cost_total = elec_cost_total - grid_upgrade_cost_block,
...
#
# Scattershot cost has two ledger components:
#   - Household: heat-pump equipment, water heater, cooktop, dryer (the
#     out-of-pocket cost households pay, after federal incentives, when they
#     electrify their own home).
#   - ComEd electric ratepayers: distribution-grid upgrades, recovered through
#     ComEd's delivery charges from all electric customers, not directly from
#     the electrifying household.
# We split the NPV both ways so downstream report variables can attribute
# scattershot cost to the correct payer.

npv_factor <- sum(1 / (1 + DISCOUNT_RATE)^(0:(TIME_PERIOD - 1)))

pg_summary <- pg_summary |>
  mutate(
    # Annual cost of scattershot electrification (evenly distributed)
    # Excludes decommissioning: gas infrastructure remains in operation under scattershot
    scattershot_annual_cost = (PCT_ELEC * (elec_sf_cost + elec_mf_cost)) / TIME_PERIOD,
    scattershot_annual_cost_household = (PCT_ELEC *
      ((sf_parcels * cost_elec_sf) + (mf_units * cost_elec_mf))) / TIME_PERIOD,
    scattershot_annual_cost_grid = (PCT_ELEC * grid_upgrade_cost_block) /
      TIME_PERIOD,

    # NPV of scattershot costs (t=0 to TIME_PERIOD-1)
    scattershot_npv = scattershot_annual_cost * npv_factor,
    scattershot_npv_household = scattershot_annual_cost_household *
      npv_factor,
    scattershot_npv_grid = scattershot_annual_cost_grid * npv_factor,
...
# blocks with cost ratio between 1.0 and 2.0 (PRP-equivalent up to 2x PRP),
# expressed both as a share of all res blocks and as a share of the "remaining"
# blocks (those not already cheaper than PRP under Perspective 1)
n_res_blocks_1_to_2 <- sum(res_only$npa_over_prp > 1 & res_only$npa_over_prp < 2, na.rm = TRUE)
pct_res_blocks_1_to_2 <- (n_res_blocks_1_to_2 / n_res_blocks)
pct_remaining_blocks_1_to_2 <- (n_res_blocks_1_to_2 / (n_res_blocks - n_res_blocks_cheaper))
```
...
#
# Each NPA cost stream is split across the rate-base ledger that actually
# funds it, so downstream consumers (the testimony narrative in particular)
# can attribute spending to the correct payer.
#
#   - PGL gas ratepayers fund electrification equipment and pipe
#     decommissioning under coordinated TE; under PRP they fund pipe
#     replacement.
#   - ComEd electric ratepayers fund local distribution-grid upgrades to
#     accommodate new winter electric load, recovered through ComEd delivery
#     charges.
#   - Households fund their own equipment out of pocket (and through federal
#     incentives) under scattershot; they pay nothing additional under
#     coordinated TE because the utility does the work.

# PRP cost (PGL gas ratepayer ledger).
total_prp_cost_portfolio <- sum(res_only$prp_cost, na.rm = TRUE)

# Coordinated TE cost, split by ledger.
total_npa_cost_portfolio <- sum(res_only$elec_cost_total, na.rm = TRUE)
total_npa_pgl_cost_portfolio <- sum(res_only$elec_pgl_cost_total, na.rm = TRUE)
total_grid_upgrade_cost_portfolio <- sum(res_only$grid_upgrade_cost_block, na.rm = TRUE)

# Scattershot scenario, split by ledger. The household number is the figure
# that should appear in any "households fund X" claim; the grid component is
# socialized across all ComEd electric ratepayers.
total_ss_npv_portfolio <- sum(res_only$scattershot_npv, na.rm = TRUE)
total_ss_npv_household_portfolio <- sum(res_only$scattershot_npv_household, na.rm = TRUE)
total_ss_npv_grid_portfolio <- sum(res_only$scattershot_npv_grid, na.rm = TRUE)

# Combined PRP + scattershot cost (full societal envelope).
total_prp_ss_portfolio <- total_prp_cost_portfolio + total_ss_npv_portfolio

# Net societal savings from coordinated TE.
...
# Net savings on the PGL-gas-ratepayer + household ledger only — the two
# ledgers paired in the §VII Q4 narrative. ComEd electric ratepayers fund
# grid upgrades under both scenarios (more under coordinated TE, since 100%
# of homes electrify versus 30% under scattershot); their portion is tracked
# separately above and discussed alongside the two-ledger comparison.
portfolio_savings_pgl_household <- (total_prp_cost_portfolio +
  total_ss_npv_household_portfolio) - total_npa_pgl_cost_portfolio
```
...
scenario_totals_ss <- bar_data_ss |>
  group_by(scenario) |>
...
bar_data_ss |>
  ...
  geom_text(
    data = scenario_totals_ss,
    aes(
      x = scenario,
      y = total,
      label = scales::dollar(total, scale = 1e-6, suffix = "M", accuracy = 1)
    ),
    vjust = -0.6,
    fontface = "bold",
    color = "#333333"
  ...
  scale_y_continuous(
    limits = c(0, 750e6),
    breaks = seq(0, 700e6, by = 100e6),
    labels = scales::dollar_format(scale = 1e-6, suffix = "M", accuracy = 1),
    expand = c(0, 0)
  ) +
  scale_x_discrete(limits = c("PRP + scattershot", "Coordinated TE")) +
  scale_fill_manual(
    values = c(
    ...
    labels = c(
      ...
      "total_elec_cost" = "Coordinated targeted electrification",
      "total_ss_npv" = "Scattershot electrification"
    ),
    breaks = c("total_prp_cost", "total_ss_npv", "total_elec_cost")
  ) +
  labs(
    x = "",
    y = "Total upfront capital cost",
    fill = ""
  ) +
  theme(
...
```{r}
#| label: fig-single-block-eg
#| fig-height: 9
#| fig-width: 8

sample_id <- "170314407001003" #"170316913001026"
# Total street centerline running along this block's borders, measured
# from streets_in_construction.geojson (E 87th Pl 0.125 mi + portions of
# S Langley Ave 0.021 mi and S St Lawrence Ave 0.040 mi). Used as the
# proxy for miles of CI/DI gas main beneath the streets — see
# geo_data_cleaning.qmd ("We use street miles as a proxy for pipeline
# miles to be replaced").
total_pipe_touching <- 0.1949
...
plot_data <- res_only |>
  ...
  mutate(
    ...
    grid_cost = total_residential_units * grid_upgrade_cost_hh,
    decomm_cost = street_miles * cost_decomm_mile,
    scattershot_annual = (PCT_ELEC * (elec_sf_cost + elec_mf_cost + grid_cost)) / TIME_PERIOD,
...
#| fig-cap: "Estimated total planned PRP spending by project start year. We include both fully residential blocks and mixed-zoning blocks (those with one or more commercial or industrial parcels)."
pg_summary |>
  mutate(
    full_res = case_when(
...
# total_prp_miles is loaded from the input sheet (lpp_program_3) at the top
# of the notebook; do not redeclare it here.
...
```{r}
save.image(file = here("reports", "il_npa", "report_variables.RData"))
```
