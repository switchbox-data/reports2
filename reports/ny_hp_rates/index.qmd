...
Something about the cost shift.
...
Utility customers pay {{< glossary "Utility bills" display="bills" >}} in order to cover the utility's {{< glossary "Utility costs" display="costs" >}}, and these bills are calculated using {{< glossary "Utility rates" display="rates" >}}.
...
[^demand-charges]: Rate design enthusiasts will be quick to point out the omission of demand charges, where customers pay for the higest kW of demand they use in a month—power instead of energy. Demand charges are out of scope for this report.
...
Companies build power plants, transmission lines, and distribution lines to produce and deliver electricity to customers. They incur {{< glossary "Capital costs" display="capital costs" >}} while doing so, and ultimately need to recoup these costs from customers—plus a profit margin. Examples of these capital costs are shown in the table below.

In addition to these costs they incur when building this infrastructure, these companies also incur {{< glossary "Operating costs" display="operating costs" >}} to operate and maintain their infrastructure, and run their business. Examples of these capital and operating costs are shown in the table below.

::: {.column-page-right}

|                    | Capital costs                                                        | Operating costs                                                          |
| ------------------ | :------------------------------------------------------------------ | :----------------------------------------------------------------------- |
| **Generation**     | **Capacity costs**: Power plants, turbines, solar arrays, battery storage                | **Fixed**: Staff, inspections, insurance<br>**Variable (with output)**: Fuel, water, consumables              |
| **Transmission**   | **Capacity costs**: High-voltage lines, bulk substations, switching stations             | **Fixed**: Infrastructure maintenance, vegetation management, property taxes, admin     |
| **Distribution**   | **Capacity costs**: Feeders, transformers, secondary lines<br>**Connection costs**: Meters, service drops        | **Fixed**: Infrastructure maintenance, vegetation management, storm restoration, property taxes, admin<br>**Variable (with customers)**: Billing, customer service, meter reading |

:::

The operating costs of traditional fossil-fueled power plants go by a special name: **energy costs**, or how much it costs each power plant to generate an additional kWh of electricity at any given moment. This is largely the cost of the fuel they burn to produce it, and a few other consummables.[^renewables]
...
Formerly, all three kinds of infrastructure were owned and operated by electrical utilities. Today, they only own the distribution lines. Transmissions lines are owned by separate companies called transmission owners, and generation is owned by still other companies called power producers. Regardless of ownership, all of these entities ultimately recoup their customers by charging customers through their electric bills.
:::
...
1. Let's say a family moves into a newly built home. When they first sign up for electricity service, the utility installs a meter, and connect their home to the local power line a service line. In other words, they cause some distribution capital costs (connection costs).
2. The utility starts reading their meter, and sending them bills. The family has therefore caused new distribution operating costs (ones that vary with the number of customers the utility serves).
3. As the family uses electricity, hour by hour, they cause generator operating costs (energy costs) — the generators have to burn a little more fuel.
4. During the summer, they run their air conditioner during a heat wave, at the same time as many other customers. Most generators are running at full tilt—including many that sit idle for much of the year. By consuming electricity during these peak hours, the family is "causing" generation capital costs (capacity costs), because they are contributing to the need to pay many of these "peaker" plants just to stay in business, so they'll be around to serve this peak demand.
5. But generation is not the only part of the infrastructure that's running at "full tilt" during the heat wave—the transmission line that brings electricity to their city, the distribution line that brings it to their home, and the substation that connects the lines may also be approaching their {{< glossary "Capacity (infrastructure)" display="capacity" >}} — the maximum current they can carry without damaging the equipment. The family turning on their air conditioner at this time is therefore causing transmission and distribution capital costs (capacity costs), because they are contributing to the need for the utility to upgrade these components so they can handle even higher demand in the future.
6. During most of the year, however, the family are simply benefiting from the transmission and distribution infrastructure that is _already built_, not causing the need for upgrades. How are these "historical" capital costs to be paid for?

It's a vital question, because the vast majority of transmission and distribution (T&D) capital costs that utilities need to recoup from customers are {{< glossary "Sunk costs" display="sunk" >}} — the infrastructure is already built, and it has to be paid for, regardless of what customers do going forward.[^fixed_operating_costs] These are called {{< glossary "Embedded costs" display="embedded costs" >}}.

Only a small fraction of the T&D capital costs are {{< glossary "Marginal costs" display="marginal" >}} — costs for capacity upgrades that haven't been set in stone yet, soon-to-be-incurred costs that could be lowered if fewer customers use electricity when the grid is strained.

[^fixed_operating_costs]: Similarly, most T&D _operating_ costs are fixed: the utility's utility will need to trim trees and restore service after storms regardless of how customers consume electricity during the year.

::: {.callout-note}
## Why are most T&D capital costs sunk?
Generation capital and operating costs are short-lived: customers pay for them shortly after causing them. Households pay for the energy costs they caused that month on their bill at the end of the month. Generators receive the capacity payments they need to stay in business for another year more or less during that same year.

T&D operating costs are also short-lived: customers pay for the utility's annual tree trimming and customer service budgets over the course of the year.
...
The latest wave of T&D capacity upgrades that will be triggered by the forthcoming summer peak—the {{< glossary "Marginal costs" display="marginal" >}} {{< glossary "Capacity costs" display="capacity costs" >}}—will therefore make up a very small fraction of all the historical (and {{< glossary "Sunk costs" display="sunk" >}}) capital costs that the utility still needs to recoup from customers.
:::

Marginal costs are those that are directly driven by customer behavior. These include:

- Generation operating (energy) costs
- Generation capacity costs
- Distribution capital and variable operating costs that vary with number of customers: connection costs, customer service
- New transmission and distribution capacity costs

Embedded costs are those that are already incurred, and are not directly driven by customer behavior. These include:

- Transmission and distribution capacity costs that still have to be paid off
- Fixed transmission and distribution operating costs

Allocating _marginal_ costs to customers using cost-causation is straightforward, at least in principle: you can measure each customer's marginal costs based on when they use electricity.

Allocating _embedded_ costs using cost-causation is difficult to do in practice—which customers caused this transformer to be upgraded 30 years ago, are they even still around?

And it is contested in theory, challenged by at least two alternative principles:

- **Beneficiary-pays**: shouldn't customers that _currently_ benefit from the transformer help pay for it, rather than only those that _originally_ caused the transformer to be upgraded?
- **Ability to pay**: many economists argue that given that these embedded costs are sunk—that current customer behavior won't cause the costs to change, unlike marginal costs—they shouldn't be allocated based on cost-causation at all, but based on ability to pay.

So in order to figure out a customer's cost-of-service, you don't just need to measure their marginal costs, you need to choose a principle for allocating the embedded costs.

In this report, we choose cost-causation, because it's what the utility's use. We assume that a customer's current infrastructure cost-causation (their marginal costs) is a good proxy for how they cause previous waves of infrastructure upgrades.

::: {.callout-note}
What about power plants? Don't customer just benefit from this pre-existing infrastructure, just like transmission and distribution—and need to help pay for it?

Actually, no: Generation works differently. Power plants compete in a market — if revenues don't cover a plant's costs, it can shut down. Customers pay the market clearing price for energy and capacity, not any individual plant's embedded costs. So unlike with poles and wires, customers aren't on the hook for keeping specific generators in business. Technically, they "cause" all generation capacity costs as they consume electricity, unlike transmission and distribution capacity costs, which are (mostly) sunk—they have to be paid for regardless of what they do.
:::
KEEP FOR MAYBE COPY
Embedded or residual costs are the costs that have to be paid no matter what customers do: the costs of the infrastructure that is already built, which is the the vast majority of transmission and distribution capacity and operating costs. Also, most transmission and distribution operating costs: the infrastructure has to be repaired, trees have to be trimmed, etc.

Marginal costs are those that are still "up for grabs", that could vary at the end of the year depending on how much electricity is consumed, and when. They include all of the energy costs—if you customers consume less, less fuel is burned. It also includes all generation capacity costs, for reason explained above. And it includes the "next wave" of transmission and distribution capacity costs—the upgrades that are triggered by rising demand this year, to ensure that the lights don't go out when demand rises again next year.

## Bills

Bills have supply and delivery

Supply pays for generation capital and operating costs. So you pay for the energy you consume, and the capacity you need to keep the lights on.

Delivery pays for transmission and distribution operating and capital costs—that marginal costs, but mostly the embedded costs.

## Rates

Bills rarely collect a customer's cost-of-service, and that's because they are calculated using rates, which are imperfect proxies for costs.

Fixed charges: pay the same every month. Usually they pay for customer-related costs: metering, billing, customer service.

Only on delivery side.

Volumetric charges: pay based on how much you use. Usually they pay for everything else: energy, generation capacity, transmission, distribution.

These are both on the supply and delivery side.

On the supply side, volumetric rates do a decent job of collecting a user's marginal supply costs: they pay for the energy they consume, and the capacity they need to keep the lights on.

On the delivery side, volumetric rates do a poor job of collecting a user's delivery cost-of-service: flat volumetric rates charge the same amount for each kWh of consumption, which assumes that each hour incurs the same T&D capital costs. But when know that the marginal costs of T&D capacity only really happen at certain times of the year, when the grid is at capacity.

And the embedded costs—well they already happened. Depending on how you allocate them—in proportion to their marginal costs, based on their income—there's no guarantee that how much electricity a user consumes will line up well with their "fair share" of the costs they cause, however you define it.

Then there are cost-reflective volumetric rates, which are designed to change with underlying marginal energy and capacity costs. This is good, because then the price can actually INFLUENCE THOSE COSTS, WHICH ARE UP FOR GRABS, because it incentivizes custoemrs to consume more when the costs are high—when electrons are expensive, when the grid is at risk of needing an upgrade.

However, it might still turn out, and it does for HP, that customers who consume a lot of electricity but not necesssarily when they are incurring marginal capacity costs, will end up paying way more than their cost of service.

THE POINT: volumetric rates do okay for marginal energy costs, though it would be better if they were time varying. because things get averaged out.

They do less well for marginal energy costs, because these are only active for some times of year, and under flat volumetric rates, that price doesn't reflect that at all, and therefore customers won't get charged much more when they consume more during those times and less if they don't. This can get better with time-varying as well.

But volumetric rates do very poorly for embedded costs, because, as we'll see, they tend to way overcharge customers who consume a lot of electricity compared to their fair share of these costs.

This is the case of heat pump customers—they consume a lot of electricity, so they get charged a lot, but most of this isn't when they would be causing marginal capacity costs, so getting charged that much isn't in line with their cost of service.

---



These costs are called "costs of service".
...
## How customers cause costs


For T&D operating costs, its just the budget for that year. For T&D capacity costs, it's a chunk of repayment for that year. When you trigger a new cost, we only count is as you causing the first year. all the other years become residual in future year.

Diagram of all the costs in a year. Show which ones are embedded, which ones are marginal.
Clarify that the embedded ones (and the marginal td capacity ones) are just-one-year-at-a-time.

Technically, so are generation capacity costs. Stay alive one more year vs. pay back that year's chunk.

## Cost-of-service

Your chunk of the costs! In theory, your bill should equal it. Because of rates, it doesn't.
...
Imagine a single-family home in Buffalo: a three-bedroom house built in the 1990s, heated by a natural gas furnace with
central air conditioning. It pays `{python} dollar(v.median_energy_total)` for electricity and natural gas every year — close to the median total bill for gas-heated homes in the state.[^buffalo_home]
...
[^buffalo_home]:
    Specifically: a 1,698 sq ft, owner-occupied, two-story home with R-19 insulated concrete masonry walls,
    an 80% AFUE gas furnace, and SEER 13 central AC. The home is served by NYSEG (electricity) and National
    Fuel (gas).
...
[^why_save]: TODO add a note on why some gas-heated homes actually save by switching to heat pumps.
...
In the case of the Buffalo home, which had central air conditioning before the switch, its annual electricity use and its annual electric bill both grow by
...
To illustrate, let's return to our Buffalo home, which is served electricity by NYSEG, and has an annual delivery
...
After switching to the heat pump, this particular homes's electric delivery cost-of-service is unchanged, because its new electricity use is highly concentrated in the winter, during hours when marginal capacity costs are zero.[^notmuch]
...
[^notmuch]:
    ...
    fact that both the NYISO grid as a whole and NYSEG's local distribution system have ample winter headroom
    (see @sec-headroom).

But as we saw earlier—as a direct consequence of volumetric rates—the Buffalo home's annual electric
...
The power line on this block was upgraded in recent years, and the cost of this upgrade is paid back by customers over decades. Every year, the annual cost of this upgrade is paid for by the homes on this block.[^simplification]
...
## Option 1: seasonal delivery rate for heat pump customers
...
Under the seasonal heat pump rate, the Buffalo home's post-heat pump annual electric bill
...
If all heat pump customers opted in to seasonal heat pump rates rates, this cross-subsidy would be removed. The volumetric delivery rate
...
The seasonal heat pump rate proposed above has planned obsolescence built in: it is intended to be medium-term solution, to be gradually adjusted over time, and eventually replaced if the grid become fully winter-peaking.

The rates we have designed for each utility reflects today's reality: in the summer, some customers installing heat pumps are triggering modest grid capacity upgrades, in cases where they gain more cooling than before.

In the winter, these customers are significantly increasing their electricity use—if they previously heated with fossil fuels. Since the grid has spare capacity during these months, these new loads are not triggering grid capacity upgrades—though heat pumps customers are being charged as if they were. Our propose heat pump rate would correct that, by lowering the volumetric rate for these customers.
...
The winter peak is currently only **`{python} pct(gb.winter_summer_ratio_2025)`** of the summer peak in NYISO's 2025 baseline forecast, but which is expected to grow faster and eventually overtake it (@fig-combined-growth-slices).
...
In transmission-contrained zones like New York City and Long Island, where many multifamily buildings lack full cooling, heat pump adoption is likely already contributing to bulk transmission capacity costs.[^transmission-constrained] Upstate, heat pump adoption may not drive the need for bulk transmission upgrades until well after the system as a whole becomes winter-peaking.

[^transmission-constrained]: Albeit modestly, given the slow pace of heat pump adoption in NYC.

Finally, heat pump adoption will start to drive significant **distribution capacity** costs only as feeders and substations start becoming constrained in winter, which will happen happen gradually, and at a different times throughout the state. On the whole, this will likely occurs after the system as a whole becomes winter-peaking, due to existing headroom on the distribution grid.

:::{.column-page-inset-right}
{{< embed notebooks/gold_book_peaks.qmd#fig-season-natgrid >}}
:::

For instance, in National Grid's upstate territory, the utility-wide winter peak overtakes the summer peak around **`{python} '—' if gb.natgrid_crossover_baseline is None else str(int(gb.natgrid_crossover_baseline))`** (in the baseline scenario). But because National Grid's feeders summer peaks well below their normal capacity, the utility has significant headroom on the distribution grid, and the winter peak can keep growing until approximately **`{python} '—' if gb.natgrid_headroom_baseline is None else str(int(gb.natgrid_headroom_baseline))`** before this headroom is exhausted, and heat pump adoption starts driving significant distribution capacity costs.
...
[^distribution-costs]: See each utility's forecast, see @sec-peak-growth-by-utility.
...
The seasonal heat pump rate outlined above is fundamentally about solving the **fairness** problem: making sure we don't overcharge heat pump customers.
...
There are two distinct flavors of economic efficiency that well-designed time varying rates can promote:
  ...
  1. **Consumption efficiency**: making energy prices more reflective of the underlying cost of generating electricity, so that consumers use more electricity when it's cheaper to generate (like when the sun is shining), and less when it's more expensive (like when peakers are running).
  2. **Investment efficiency**: making network prices more reflective of the underlying cost of expanding grid capacity, so that consumers reduce electricity use during peak demand periods, thereby reducing the need for investments in grid capacity (whether in generation, transmission, or distribution).

**Fairness** is achieved through **cost-based rates**, defined as a rate that bills each customer in line with their cost-of-service (for delivery, in this case).

A rate can be more or less cost-based: in theory, a perfectly cost-based delivery rate would bill _every_ customer exactly their cost-of-service for delivery. Our proposed seasonal delivery rate is cost-based _on average_ for heat pump customers, because it lowers their volumetric rate in winter to a level that causes their average delivery bill to equal their average cost-of-service.[^cost-based-rate]

[^cost-based-rate]: Individual HP customers may still over or underpay relative to their cost-of-service, though less than under
...
A rate can also be fairly cost-based without being at all cost-reflective: imagine if heat pump and fossil heating customers paid all of their delivery costs through fixed charges that equaled the average cost-of-service of each group: `{python} dollar(v.hp_mean_monthly_delivery_cost)` a month for heat pump customers, and `{python} dollar(v.ff_mean_monthly_delivery_cost)` a month for fossil heating customers, at present. This rate would be cost-based _for each group_, meaning the average overpayment or underpayment would be zero for both. But it would not be cost-reflective, because it would not expose customers to different volumetric prices at different hours (indeed, there would be no volumetric prices at all).
...
For each utility in NYS, we've designed time-of-use rates specifically for heat pump customers that are **cost-reflective**, aligned with underlying marginal energy and capacity costs. Unlike most time-of-use rates currently available, in New York State, we've designed these rates to be **cost-based**, so that they also remove the cross-subsidy from HP customers to non-HP customers that exists today.

In other words, these rates would collect the same total revenue from heat pump customers as the seasonal rate we proposed earlier—both implement the same cost allocation—but would do so by charging different prices at different times of the day, not just at different times of the year.
...
It depends on the extent to which these households shift the electricity use to off-peak periods, which would produce lower bills.

Depending on the utility, we assumed that heat pump customers would be shift between 13% and 17% of their on-peak consumption to off-peak periods in summer, and between 6% and 8% in winter.[^load-shifts]
...
Nearly all of improvement in bill impact, compared to the default rate, comes from the fact that the rate is **cost-based**, not from the fact that it is **cost-reflective**: in other words, nearly all of the improvement comes _not_ come from the inclusion of two price levels (that reflect the underlying marginal costs of energy and infrastructure) per se, but from the fact that the "average" price level is lower than under the default rate.


<!--
## Flexibliity
- Discussion of time-of-use rates with no flexibility: compared to the seasonal rate, adding a time-of-use
...
### Customer counts, kWh, and revenue requirement assembly

Our simulation uses NREL's ResStock 2024.2 building stock model to generate hourly electricity loads for a representative sample of New York residential buildings. Each ResStock building carries a sampling weight; we rescale these weights so that the weighted customer count for each utility matches the EIA-861 reported residential customer count as of 2024. This ensures the simulation represents the right number of customers per utility.

However, the total electricity consumption (kWh) implied by these reweighted ResStock loads does not match EIA-861 reported residential sales. After customer reweighting, ResStock kWh is systematically higher than EIA kWh for most utilities — by as much as 18% for Con Edison, and 8% statewide.[^kwh-mismatch] We do not rescale the kWh to match EIA; the loads that enter the simulation are the raw ResStock hourly profiles, reweighted only for customer count.
...
The total revenue requirement we simulate against is assembled from two sources with different underlying assumptions. The **delivery revenue requirement** comes directly from each utility's most recent rate case (the table above) — a fixed dollar amount derived from the utility's own cost-of-service study, which uses its own projections of residential customer counts and kWh. The **top-up charges** (surcharges, supply commodity, and other volumetric charges not included in the rate-case delivery amount) are estimated separately: we take the actual filed $/kWh rates for each charge from 2025 tariff schedules and multiply them by the EIA customer counts and the ResStock kWh to produce annual dollar budgets.

This means the assumptions behind the rate-case delivery revenue requirement — the utility's own customer count and kWh projections from its cost-of-service study — are not necessarily the same as the customer counts and kWh we use for the top-up budgets. The rate-case delivery amount was designed to be recovered from a specific volume of kWh; we recover it from a different (typically larger) volume, which means the simulation's calibrated delivery rates are somewhat lower than the rates the utility actually filed. This does not affect relative comparisons between rate designs (the same kWh enters all scenarios), but it means the absolute $/kWh rates and dollar bill amounts in the simulation are approximate rather than exact reproductions of filed rates.
...
TOU rates create a financial incentive to shift electricity use away from peak hours — running the heat pump
harder before the peak period, for instance, or letting the house cool slightly during peak hours and
recovering afterward. This is demand flexibility: customers responding to price signals by changing _when_
they consume electricity, without reducing how much they consume overall.

The demand flexibility analysis models how heat pump customers shift electricity consumption from peak to
off-peak hours in response to TOU price signals. The key parameter is the elasticity of substitution
($\varepsilon$), which controls how much load shifts for a given peak-to-off-peak price ratio.[^elast_def] We calibrate
this parameter separately for each utility and season, anchored to empirical evidence from real TOU pricing
pilots.

[^elast_def]: **Elasticity**: How much customers change their behavior when prices change. A more negative
elasticity means more load shifts away from peak hours for a given peak-to-off-peak price ratio. The bigger the difference between the peak and off-peak rates, the more customers will shift their demand.



### How much do loads shift

We ground our elasticity in the Arcturus 2.0 meta-analysis [@faruqui_Arcturus20Metaanalysis_2017], which
synthesized results from 62 residential time-varying pricing pilots across North America, Europe, and
Australia. The study's central finding is that peak demand reduction is strongly predicted by the
peak-to-off-peak price ratio — the higher the ratio, the more customers shift. The relationship is
log-linear:

$$\text{peak reduction} = -0.011 + (-0.065) \times \ln(\text{price ratio})$$

This is the conservative "no enabling technology" baseline — customers responding to price signals alone,
without smart thermostats or automation. At a 2:1 price ratio, it predicts ~5.6% peak reduction; at 4:1,
~10.1%. Returns diminish as the ratio increases. Arcturus also estimates a higher-response model for
customers with enabling technology, which we use for our results given that most modern heat pumps are
programmable.[^arcturus-tech]

[^arcturus-tech]: Arcturus also estimates a "with enabling technology" model — customers with smart
thermostats or in-home displays. At the same price ratios, it predicts ~8.8% peak reduction at 2:1 and
~16.5% at 4:1, roughly 1.5–1.6x the no-tech response. We use the no-tech model as our conservative
baseline.

$$\text{peak reduction} = -0.011 + (-0.111) \times \ln(\text{price ratio})$$

At the same price ratios, it predicts ~8.8% peak reduction at 2:1 and ~16.5% at 4:1. Since most modern
heat pumps are programmable and can respond to price signals automatically, we use the with-tech model for
our presented results. That said, the resulting bill savings from demand shifting are modest either way —
on the order of $15–$35 per HP customer per year — compared to the much larger savings from simply being
on a TOU rate.

### Why calibration is needed

Our simulation uses a constant-elasticity model
($Q_{\text{shifted}} = Q \times (P_{\text{period}} / P_{\text{flat}})^\varepsilon$), which differs from
Arcturus's log-linear regression. We cannot directly equate the two functional forms. Instead, we calibrate
by:
...
This yields an empirically grounded elasticity — one that matches the aggregate demand response observed in
real pricing pilots at a comparable price ratio.

### Per-utility, per-season calibration

Each NY utility has different TOU price ratios derived from its marginal cost profile. Summer ratios range
from 2.87 (RG&E) to 4.33 (Con Edison); winter ratios are lower, ranging from 1.57 (Central Hudson, O&R) to
1.98 (Con Edison). Because Arcturus predicts different peak reductions at different ratios, a single
statewide elasticity would overshoot some utilities and undershoot others.

We calibrate each season independently. Summer's higher price ratios call for a mean there is a greateer incentive to shift emand (more negative elasticity), while winter's lower ratios mean slightly less load shifting. The calibration runs on actual ResStock HP building loads — the same hourly
consumption data used in the bill analysis — ensuring the elasticity reflects the load profile of the
buildings we are modeling.
...
Wider peak windows shift more kWh per unit of elasticity, so less elasticity is needed to reach the same
Arcturus target.
...
The calibrated elasticities reproduce the *average* response observed across real TOU pilots. They do not
capture the distribution of individual customer responses — in reality, some customers shift aggressively
while others do not shift at all. Our model applies the same $\varepsilon$ uniformly to every HP building,
producing identical percentage peak reductions across all buildings. This is a known simplification.

The bill savings from demand flexibility come almost entirely from **rate arbitrage**: consuming less
electricity at the peak rate and more at the off-peak rate. The system-level savings channel (lower marginal
costs reducing the revenue requirement) is negligible because delivery marginal costs are nonzero in only
40–113 hours per year, leaving very little cost to shift away from.

### Validation

We validated the shifting mechanics against CAIRO's own outputs for all seven NY utilities. The validation
confirms energy conservation (total kWh is preserved within machine precision), correct direction (peak kWh
decreases, off-peak increases), and consistency with CAIRO's internal demand response tracker (mean
$|\Delta\varepsilon|$ < 0.003 across all utilities).
