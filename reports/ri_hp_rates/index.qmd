...
[^cite-resstock]: @nrel_EndUseLoadProfiles_2024.

[^resstock-reweight]:
    ResStock's default national sample weights represent ≈ 252 real-world homes each. We
    rescale them so that the weighted sum equals the number of residential customers in
    RIE's rate-case test year, as specified in the
    [RDP revenue-requirement config](https://github.com/switchbox-data/rate-design-platform/blob/main/rate_design/hp_rates/ri/config/rev_requirement/rie_rate_case_test_year.yaml).
    This ensures bill totals aggregate correctly when computing utility-wide revenue requirements.

#### Energy prices and tariffs

- **Electricity** — RIE A-16 residential rate (2025). Delivery charges use the flat volumetric rates from RIE's rate-case test year filing; supply uses seasonal Last Resort Service rates. Sourced from Genability; verified against RIE Tariff No. 2095 (delivery) and Last Resort Service filing (supply).
- **Natural gas** — Rate 12 Residential Heating (2025) and Rate 10 Residential Non-Heating (2025): fixed charge + per-therm volumetric; source: RateAcuity
...
[^cite-eia877]: @eia_HeatingOilPropanePrices_2025.
...
- Homes in non-gas areas or with zero gas consumption → no gas tariff, zero gas bills
...
[^lidr-proposed]: @rie_Docket2545GEApplication_2025b, pg. 95 (DeSousa et al. direct testimony); tier discounts from Table 7 of Blazunas testimony in the same docket.
...
- Bill = tariff applied to hourly electricity profile + annual fossil fuel consumption × fuel price
- Bill change = (Upgrade 2 total energy bill) minus (Upgrade 0 total energy bill), both on default rates
...
- Framework: **Bill Alignment Test** (Simeone et al. 2023) — compares what each customer pays vs. their cost of service
...
| Sub-TX + distribution | Next MW of local delivery capacity | RIE MCOS study capital |

[^cite-lmp]: @iso-ne_ISONEWebServices_2025.

[^cite-fcm]: Forward capacity auction clearing price $P_m$ from @isone_ForwardCapacityAuction_2024; system hourly loads used to identify peak hours from @isone_CeltReport_2025.

[^cite-aesc]: @synapse_AvoidedEnergySupply_2024.
...
- For RIE: delivery residual is `{python} pct(v.delivery_residual_pct)` of total delivery revenue — marginal costs recover only `{python} pct(1 - v.delivery_residual_pct)` of delivery revenue
...
- Standard method used in CA; referenced in BAT paper (Simeone et al., Appendix B)
...
- Calibrated so HP subclass BAT = 0: their delivery revenue equals their allocated cost of service. Equivalently: $r_{\text{HP}} = (RR_{\text{HP}} - FC_{\text{HP}}) / Q_{\text{HP}}$, where $RR_{\text{HP}}$ is the HP subclass revenue requirement (= $K \times EB_{\text{HP}}$), $FC_{\text{HP}}$ is fixed-charge revenue already collected from HP customers, and $Q_{\text{HP}}$ is total HP kWh. By construction, this rate collects exactly the allocated cost of service of heat pump customers as a group.
...
- **Energy burden** — LMI gas-heated households tracked across 3 rate scenarios × 2 enrollment assumptions (32% and 100%)
...
- **Feeder/transformer workbook** — RIE-provided; 2021–2035 summer peak loads (amps for feeders, MVA for transformers), nameplate ratings, N-1 contingency assessments for 2026 and 2035
- **RIE load forecast (RI Peak Report 2024)** — RIE's official 15-year load forecast: system-wide summer and winter net peak MW under extreme weather (95/5 scenario) through 2039; EH contribution to winter peak; cumulative HP adoption counts (Base and High scenarios).[^cite-rie-forecast]

[^cite-rie-forecast]: @rie_2024ElectricPeak_2024b.

#### Deriving winter utilization

- Feeder data is summer-only — RIE does not currently measure feeder-level winter peaks and does not forecast how those winter peaks will evolve. TODO: quote RIE correspondence on why, and Ryan on the timeline to compile this data.
- Winter demand derived from summer demand using an evolving system-wide winter/summer ratio from the load forecast, anchored at $W_0 = 0.70$ in 2024
- Winter rated capacity assumed equal to summer nameplate rating — a conservative assumption, since thermal limits do not degrade in winter (cooler ambient temperatures, if anything, increase cable ampacity)
...
- Three criteria: (1) peak > 100% of nameplate rating, (2) N-1 contingency = "yes", (3) borderline in 2026 → blank in 2035 (engineers already planning upgrade)
- 2026 snapshot: all three criteria applied directly
- 2039 projection: feeders constrained in 2026 excluded (assumed upgraded); remaining assessed on criteria 1–2, with "borderline" also counting
- Utilization for 2036–2039 extrapolated from 2035 using Peak Report growth rates

#### Headroom metric

- Headroom = $1 - \text{utilization}$ = remaining capacity as a share of nameplate rating
- Computed over the full population of feeders/transformers present in each year's dataset
