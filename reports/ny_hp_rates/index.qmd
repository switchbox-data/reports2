...
To follow along with this report, the reader needs to understand the electric utility {{< glossary "Utility costs" display=cCosts" >}}, {{< glossary "Utility bills" display="bills" >}}, and {{< glossary "Utility rates" display="rates" >}}. These concepts are introduced and explained in this section.
...
| ------------------ | -------------------------------------------------------------------: | -----------------------------------------------------------------------: |
| **Generation**     | Capacity costs: Power plants, turbines, solar arrays, battery storage                | Variable (with output): Fuel, water, consumables<br>Fixed: Staff, inspections, insurance                |
...
| **Distribution**   | Capacity costs: Feeders, transformers, secondary lines<br>Connection costs: Meters, service drops        | Variable (with customers): Billing, customer service, meter reading<br>Fixed: Infrastructure maintenance, vegetation management, storm restoration, property taxes, admin |
...
Ultimately, power sector companies incur these {{< glossary "Capital costs" display="capital" >}} and {{< glossary "Operating costs" display="operating" >}} in order to serve customers, be they tiny studio apartments or massive data centers. So they turn around and recoup these {{< glossary "Utility costs" display="costs" >}} from customers through their {{< glossary "Utility bills" display="electric bills" >}}. 
...
1. Let's say a family moves into a newly built home. When they first sign up for electricity service, they utility installs a meter, and connect their home to the local power line a service line. In other words, they cause some distribution capital costs (connection costs).
...
::: {.callout-note}
What about power pants? Don't customer just benefit from this pre-existing infrastructure, just like transmission and distribution—and need to help pay for it?
...
electric customers that heat with natural gas is `{python} dollar(v.ff_mean_delivery_cost)` per year, on average,
...
**`{python} f"{(v.hp_mean_delivery_bill / v.ff_mean_delivery_bill - 1) * 100:.0f}"`%** higher than those of electric customers that heat with fossil fuels, on average.
...
`{python} f"{(v.hp_mean_delivery_cost / v.ff_mean_delivery_cost - 1) * 100:.0f}"`% higher than that of electric customers that heat with fossil fuels.

In this report, we propose two options for a new rate for heat pump customers that would reduce their overpayments for delivery, eliminate their cross-subsidy to fossil fuel customers, and reintroduce fairness to delivery rates.

## Option 1: fair seasonal rate for heat pump customers
...
Each electric utility would have a unique winter delivery rate, reflecting the cost of service in that territory.
...
If all heat pump customers opted in to fair rates, this cross-subsidy would be removed. The volumetric delivery rate
...
Importantly, this is **not a "rate hike"** on customers without heat pumps.svg It is the removal of an unfair
...
## How should the heat pump rate evolve as the winter peak grows?

The heat pump rate proposed above has planned obsolescence built in: it is intended to be medium-term solution, to be gradually adjusted over time, and eventually replaced if the grid become fully winter-peaking.
...
Adding cost-reflectiveness to our fair rate improves gas-to-heat-pump bill outcomes modestly (@fig-quadrant-bar-natgas-tou-state).
...
### Over/underpayment by utility

::: {.column-page-inset-right}
{{< embed notebooks/analysis.qmd#tbl-bat-by-utility >}}
:::
