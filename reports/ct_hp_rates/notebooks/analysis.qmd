...
path_to_report <- file.path(repo_root, "reports", "ct_hp_rates")
...
# ****************** 100 sample homes ******************
#path_monthly_data <- file.path(path_to_data, "resstock", "2024_release2_tmy3", "load_curve_monthly_100")
# ****************** FULL DATASET ******************
...
path_fuel_oil_supply_rates <- file.path(path_to_data, "eia", "heating_oil", "ct_eia_heating_oil_prices_monthly.parquet")
path_propane_supply_rates <- file.path(path_to_data, "eia", "propane", "ct_eia_propane_prices_monthly.parquet")
...
url_delivery_rates <- "https://docs.google.com/spreadsheets/d/1bBAVdWwoUX6MgmxnclLzfvM8WmmI9U58ceMM2xSATLM/edit?gid=101079722#gid=101079722"

# SMI thresholds
url_smi_thresholds <- "https://docs.google.com/spreadsheets/d/187FwQj-aLVJXdm2qzZ4uENLqIsuoi47j5MqpYfowUHQ/edit?gid=0#gid=0"

load_tariffs_from_cache <- FALSE
load_thresholds_from_cache <- FALSE
...
load_or_download <- function(table_name, url, path_to_cache, load_from_cache=FALSE) {
  ...
  if (load_from_cache && file.exists(file_path)) {
      ...
      data <- data |>
        mutate(
          value = case_when(
            ...
            billing_unit == "dollars_per_ccf" ~ value / 29.3,
            TRUE ~ value
          ),
          billing_unit = case_when(
            billing_unit == "dollars_per_therm" ~ "dollars_per_kwh",
            billing_unit == "dollars_per_ccf" ~ "dollars_per_kwh",
            TRUE ~ billing_unit
          )
        )
    }
    ...
    if (str_detect(table_name, "tariffs")) {
      ...
      delivery_winter_months <- c(1, 2, 3, 4, 5, 10, 11, 12)
      delivery_summer_months <- c(6, 7, 8, 9)
      data <- data |>
...
# 2024 Connecticut state median income is $99,240 (https://fred.stlouisfed.org/series/MEHOINUSCTA646N)
# 60% -> $59,544
# 80% -> $79,392
# 100% -> $99,240
housing_units <- group_income_by_dollars(housing_units, dollar_tiers = c(1000, 59544, 99240))
...
ct_bldg_utility_crosswalk <- "/workspaces/reports2/data/ResStock/2024_release2_tmy3/utility_assignment/rs2024_bldg_utility_crosswalk_ct.feather"

bldg_utility_mapping_ct <- read_feather(ct_bldg_utility_crosswalk)

housing_units <- assign_utilities(
  housing_units,
  path_to_bldg_utility_crosswalk = ct_bldg_utility_crosswalk
)
...
housing_units_vacant <- housing_units |> filter(occupants_group == "Vacant")
...
print(paste0("This housing_units dataframe (use_these_states = ", use_these_states, ") has ", n_total_homes, " total bldg_ids"))
print(paste0("This housing_units dataframe (use_these_states = ", use_these_states, ") has ", n_non_vacant_homes, " non-vacant bldg_ids (", round(n_non_vacant_homes / n_total_homes * 100, 2), "%)"))

summarize_bldg_ids_by <- function(housing_units, column_to_summarize) {
  # Accepts:
  #   housing_units: dataframe
  #   column_to_summarize: unquoted column name (e.g., electric_utility)

  col_sym <- rlang::ensym(column_to_summarize)

  summarized <- housing_units |>
    filter(upgrade == 0) |>
    group_by(!!col_sym) |>
    summarise(
    ...
    mutate(
      pct = total_bldg_ids / sum(total_bldg_ids) * 100
    ) |>
    arrange(desc(pct))

  total_row <- tibble(
    !!col_sym := "Total",
    total_bldg_ids = sum(summarized$total_bldg_ids),
    pct = 100
  )

  # Print the summary
  print(bind_rows(summarized, total_row))
}

# Example usage:
summarize_bldg_ids_by(housing_units, electric_utility)
summarize_bldg_ids_by(housing_units, gas_utility)

View(housing_units |> select(bldg_id, upgrade, occupants, electric_utility, gas_utility, in.representative_income, dollar_tier, smi_tier, discount_rate_elec, discount_rate_gas))
...
# Electric
url_electric_supply_rates <- "https://docs.google.com/spreadsheets/d/1MWg8knT4F_pE1fo6mloAEZKDRFT6Em9EXSLiAt4eElA/edit?gid=0#gid=0"
...
if (load_results_from_cache) {
  ...
  # Monthly Bills
  print("Monthly bills - Electricity")
...
```{r}
#| label: explore natural gas costs
library(readxl)
path_eia_gas_prices <- file.path(path_to_data, "eia/natural_gas", "eia_us_ng_prices.xls")
eia_gas_prices <- read_excel(path_eia_gas_prices, sheet = "Data 1", skip = 2)


# Each of the columns is named like "Natural Gas Citygate Price in Alabama (Dollars per Thousand Cubic Feet)", but we want to extract just the state and rename using the 2-letter abbreviation. e.g "Natural Gas Citygate Price in Alabama (Dollars per Thousand Cubic Feet)" => "AL".
# Except, the first column, for the US average, is "U.S. Natural Gas Citygate Price (Dollars per Thousand Cubic Feet)". Handle this separately.
rename_gas_columns <- function(col_name) {
  # ignore Date, year, month, and day columns
  if (str_detect(col_name, "^Date|^Year|^Month|^Day")) {
    return(col_name)
  }

  # Keep original name if anything goes wrong
  if (str_detect(col_name, "^U\\.S\\.")) {
    return("US")
  }

  if (str_detect(col_name, "District of Columbia")) {
    return("DC")
  }

  state_full <- str_extract(col_name, "(?<=in ).*?(?= \\()")

  if (is.na(state_full)) {
    return(col_name)  # Keep original if no state found
  }

  idx <- match(state_full, state.name)

  if (is.na(idx)) {
    return(col_name)  # Keep original if state not in lookup
  }

  return(state.abb[idx])
}

# Rename columns FIRST, then add year/month/day
eia_gas_prices <- eia_gas_prices |>
  rename_with(~ sapply(.x, rename_gas_columns)) |>
  mutate(year = year(Date),
         month = month(Date),
         day = day(Date)) |>
  select(-Date) |>
  pivot_longer(
    cols = -c(year, month, day),  # Exclude all three date columns
    names_to = "state",
    values_to = "price"
  ) |>
...
eia_gas_prices_plot <- eia_gas_prices |> filter(
  state %in% c("CT", "RI", "US"),
  year > 2012
  )

# convert the natural gas prices from $/ccf to $/kwh
eia_gas_prices_plot <- eia_gas_prices_plot |>
  mutate(price = price / 1000 * 29.3)


# Load standard service prices
path_ct_standard_service_prices <- file.path(path_to_data, "datactgov", "ct_elec_standard_service_prices.xlsx")
# To specify a sheet when reading Excel files, use read_excel (from readxl), not read_csv.
ct_standard_service_prices <- read_excel(path_ct_standard_service_prices, sheet = "ct_elec_standard_service_prices")

ct_standard_service_prices <- ct_standard_service_prices |>
  mutate(
    ym = as.Date(sprintf("%04d-%02d-15", year, month))
  )

ggplot(
  eia_gas_prices_plot |>
    mutate(ym = as.Date(sprintf("%04d-%02d-01", year, month))),
  aes(x = ym, y = price, color = state)
) +
  geom_line() +
  geom_line(
    data = ct_standard_service_prices |> filter(utility == unique(utility)[1]),
    aes(x = ym, y = price, color = utility),
    linewidth = 1,
    inherit.aes = FALSE
  ) +
  geom_line(
    data = ct_standard_service_prices |> filter(utility == unique(utility)[2]),
    aes(x = ym, y = price, color = utility),
    linewidth = 1,
    inherit.aes = FALSE
  ) +
  scale_y_continuous(
    name = "Price ($/Thousand Cubic Feet)",
    sec.axis = sec_axis(~ ., name = "CT Std Service Price ($/kWh)"),
    limits = c(0, 25)
  ) +
  labs(
    title = "Natural Gas Citygate Prices by State",
    x = "Year-Month",
    y = "Price ($/Thousand Cubic Feet)",
    color = "State/Utility"
  ) +
  scale_x_date(
    date_breaks = "1 year",
...
annual_bills_elec_by_component <- monthly_bills_elec |>
  ...
  group_by(bldg_id, upgrade, version, hvac) |>
  summarise(supply_charge = sum(supply_charge),
            delivery_charge = sum(delivery_charge),
            customer_charge = sum(customer_charge),
            .groups = "drop") |>
  ungroup() |>
  filter(
    (!(hvac %in% c("hp_high")) & version == "baseline") |
    (hvac == hp_option & version == "baseline")
  ) |>
  # take the median for each group
  group_by(upgrade, version, hvac) |>
...
annual_bills_gas_median <- annual_bills_gas |>
  filter(baseline_heating_type == "Natural Gas") |>
  mutate(
    hvac = ifelse((!hvac %in% c("hp_high")), "current", hvac)
  ) |>
  group_by(upgrade, hvac) |>
  summarise(annual_gas_bill = median(annual_bill),
            .groups = "drop") |>
  ungroup()
...
y_rightmost_customer_charge <- annual_bills_by_component_plot |> filter(scenario == "hp_high_baseline", charge_type == "customer_charge") |> pull(amount)
y_rightmost_delivery_charge <- annual_bills_by_component_plot |> filter(scenario == "hp_high_baseline", charge_type == "delivery_charge") |> pull(amount)
y_rightmost_supply_charge <- annual_bills_by_component_plot |> filter(scenario == "hp_high_baseline", charge_type == "supply_charge") |> pull(amount)
y_rightmost_gas_bill <- annual_bills_by_component_plot |> filter(scenario == "hp_high_baseline", charge_type == "annual_gas_bill") |> pull(amount)
...
fig_bill_stacked_bar_gas_on_top <- ggplot(annual_bills_by_component_plot,
  ...
  scale_y_continuous(labels = scales::dollar_format(), limits = c(0, 5000), breaks = seq(0, 5000, by = 1000), expand = c(0,0)) +
...
monthly_bills_elec_by_component <- monthly_bills_elec |>
  ...
  # take the median for each group
  group_by(upgrade, version, hvac, month) |>
  summarise(supply_charge = median(supply_charge),
            delivery_charge = median(delivery_charge),
            customer_charge = median(customer_charge),
            .groups = "drop") |>
...
annual_bills_fuel_oil_median <- annual_bills_fuel_oil |>
  ...
  group_by(upgrade, hvac) |>
  summarise(annual_fuel_oil_bill = median(annual_bill),
            .groups = "drop") |>
  ungroup()
...
y_rightmost_delivery_charge <- annual_bills_by_component_plot |> filter(scenario == "hp_high_baseline", charge_type == "delivery_charge") |> pull(amount)
y_rightmost_supply_charge <- annual_bills_by_component_plot |> filter(scenario == "hp_high_baseline", charge_type == "supply_charge") |> pull(amount)
y_rightmost_fuel_oil_bill <- annual_bills_by_component_plot |> filter(scenario == "hp_high_baseline", charge_type == "annual_fuel_oil_bill") |> pull(amount)
...
monthly_bills_combined_resistance_customers_long <- monthly_bills_combined_resistance_customers |>
  pivot_longer(cols = c(customer_charge, delivery_charge, supply_charge, total_elec_bill, total_bill),
               names_to = "component",
               values_to = "amount")
...
#| fig-cap: "Proportion of CT homes by building type."
#| fig-width: 8
#| fig-height: 2
...
#| fig-cap: "Proportion of CT homes by baseline cooling type."
#| fig-width: 8
#| fig-height: 2
...
burden_hists_all_combined <- burden_hists_all_current_plot / burden_hists_all_hp_high_plot +
  ...
  plot_annotation(
    title = "Winter Energy Burdens in Connecticut: Current HVAC vs. Cold Climate Heat Pump",
...
#| fig-cap: "Proportion of CT homes by dollar tier."
#| fig-width: 8
#| fig-height: 2
...
#| fig-cap: "Proportion of CT homes by State Median Income (SMI) tier. SMI considers both the household's income and the number of occupants in the household."
#| fig-width: 8
#| fig-height: 2
...
smi_tier_pct <- smi_tier_pct |>
  mutate(
    smi_tier_label = recode(
      ...
      "Moderate Income" = "Moderate Income (60–100% SMI)",
      "Not LMI" = "Not LMI (>100% SMI)"
    ),
    smi_tier_label = factor(
      smi_tier_label,
      levels = c("Low Income (<60% SMI)", "Moderate Income (60–100% SMI)", "Not LMI (>100% SMI)")
    )
  ) |>
...
burden_hists_3_smi_tiers_current_combined <- burden_hists_low_smi_current_plot / burden_hists_moderate_smi_current_plot / burden_hists_not_lmi_current_plot +
  ...
  plot_annotation(
    title = "Annual Energy Burdens in Connecticut: Current HVAC for 3 SMI tiers",
    subtitle = paste0("Among households of annual income <", scales::dollar(income_threshold_upper)),
    theme = theme(
      plot.title = element_text(hjust = 0.5, size = 9, face = "bold"),
      plot.subtitle = element_text(hjust = 0.5, size = 8),
...
burden_hists_3_smi_tiers_hp_high_combined <- burden_hists_low_smi_hp_high_plot / burden_hists_moderate_smi_hp_high_plot / burden_hists_not_lmi_hp_high_plot +
  ...
  plot_annotation(
    title = "Annual Energy Burdens in Connecticut: Cold Climate Heat Pump for 3 SMI tiers",
    subtitle = paste0("Among households of annual income <", scales::dollar(income_threshold_upper)),
    theme = theme(
      plot.title = element_text(hjust = 0.5, size = 9, face = "bold"),
      plot.subtitle = element_text(hjust = 0.5, size = 8),
...
#   supply_year = supply_year,
#   second_subtitle = "Connecticut | Baseline Rates",
