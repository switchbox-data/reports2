...
- **ResStock 2024.2** — EnergyPlus physics-based simulations; each modeled dwelling ≈ 252 real-world homes; AMY 2018 weather; calibrated to 2020 RECS
- **Upgrade 0** — current HVAC (gas furnace, oil boiler, propane, electric resistance, existing HP)
- **Upgrade 2** — cold-climate ASHP retrofit on the same homes; HP performance modeled as a function of hourly outdoor temperature
- Bill change = Upgrade 2 total energy bill minus Upgrade 0 total energy bill

#### Energy prices and tariffs

- **Electricity** — RIE A-16 (2025): delivery (fixed charge + quarterly volumetric) + supply (Last Resort Service seasonal rates); source: Genability
- **Natural gas** — Rate 12 Residential Heating (2025): fixed charge + per-therm volumetric; source: RateAcuity
- **Delivered fuels** — EIA 2025 monthly residential prices for RI (heating oil, propane), applied to each building's modeled consumption
...
- **Current LIDR** — 25% or 30% off total bill, tied to categorical program enrollment (SNAP, LIHEAP, SSI, Medicaid, etc.)
- **Proposed LIDR+** (Docket 25-45-GE, electric only): income-based tiers — ≤75% FPL → 60%, 76–150% FPL → 30%, 151–250% FPL → 10%; gas LIDR unchanged
- **Modeling** — FPL% used as proxy for enrollment (no enrollment data in ResStock); electric = LIDR+ tiers; gas = flat LIDR by FPL band
- **Participation** — baseline = ~32% of eligible households enrolled (36,320 A-60 enrollees as of July 2024); also model 100% enrollment
...
|-----------|-----------------|--------|
| Energy | Short-run generation cost each hour | ISO-NE real-time LMP |
| Generation capacity | Next MW of adequacy | ISO-NE FCM auction prices |
| Bulk transmission | Next MW of regional TX capacity | AESC 2024 avoided PTF cost |
...
- **Energy** — all 8,760 hours non-zero; varies continuously with grid conditions
- **Generation capacity** — top 8 hours/month by ISO-NE system load; exceedance weights; ≤96 non-zero hours/year
- **Bulk transmission** — top 40 hours/season; exceedance weights; currently summer-dominated
- **Sub-TX + distribution** — MCOS capital annualized + levelized over 7 years → `$/kW-yr`; allocated to top 100 hours of RI balancing area load (EIA Form 930) via probability-of-peak weights
- **Economic burden** per customer: $EB_i = \sum_h L_{i,h} \times MC_h$

#### Why there is a residual

- Marginal costs capture only forward-looking incremental cost of new investment
- Excluded: carrying charges on existing rate base, O&M on sunk assets, regulatory return, customer costs, policy costs
- Residual = total revenue requirement minus revenue recoverable by MC pricing
- For RIE: delivery residual is `{python} pct(v.delivery_residual_pct)` of total delivery revenue

#### How we allocate the residual

- **EPMC** — each customer's residual share proportional to their share of total marginal costs
...
- Calibrated so HP subclass BAT = 0 (their delivery revenue equals their cost of service)
- Formula: $r_{\text{HP}} = K \times EB_{\text{HP}} / Q_{\text{HP}}$
- Non-HP customers remain on a slightly recalibrated default rate that absorbs the revenue no longer cross-subsidized by HP customers
...
- **RI Peak Report 2024** — system-wide summer and winter net peak MW under extreme weather (95/5 scenario) through 2039; EH contribution to winter peak; cumulative HP adoption counts (Base and High scenarios)

#### Deriving winter utilization

- Feeder data is summer-only — no feeder-level winter measurements available
- Winter demand derived from summer demand using an evolving system-wide winter/summer ratio from the Peak Report, anchored at $W_0 = 0.70$ in 2024
- Winter rated capacity = summer nameplate (thermal limits are season-independent)
