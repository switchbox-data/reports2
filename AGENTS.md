...
| `lib/great_tables/`            | Switchbox Great Tables theme (`get_switchbox_gt_tab_options`) — table title aligned with plotnine (GT Planar Bold 15px left). Also hosts `icml.py` (`render_gt_to_icml`) which emits GT tables as native InDesign `<Table>` XML for `just typeset`. |
...
Quarto's manuscript mode truncates raw `text/html` MIME outputs during the Pandoc `--to ipynb` conversion, so GT tables displayed normally would be reduced to a stray `</div>`. Use **`lib.quarto.display_gt`** to base64-encode the HTML so it survives the embed pipeline. The helper is format-aware: HTML output gets the base64-decoded `<div>`, `just draft` (DOCX, `SWITCHBOX_GT_AS_IMAGE=1`) gets a high-res PNG screenshot, and `just typeset` (ICML, `SWITCHBOX_TYPESET=1`) emits a **native InDesign `<Table>`** (via `lib.great_tables.icml`, base64-packaged inside a 1×1 placeholder SVG carrier and unwrapped by `icml_gt.lua`) that references the designer's named styles and reflows in layout — no raster, no placed SVG.
...
`display_figure` is format-aware: it emits SVG for HTML and ICML (InDesign places SVG natively) and high-res PNG (300 DPI) only for DOCX, where Pandoc's default 96-DPI rsvg-convert would otherwise fuzz out the chart. It closes the figure to free memory. **Always use this helper** instead of inlining the `savefig`/`plt.close`/`display(SVG(...))` three-liner.
...
| `lib.rates_analysis.rate_case_funcs`  | Generalized rate-case analysis for Python reports (bill-change incidence, BAT/cross-subsidy, cost of service by subclass, tariff introspection), parameterized by state/batch/scenario. `load_master_bat_for_utility` filters a master BAT segment to one utility and optionally validates/selects required columns while preserving lazy execution. `segment_upgrade` reads the ResStock upgrade ID stored on a master-bills segment. `monthly_bill_components`/`annual_bill_components` pull one building's electric+gas bill (monthly, or annual with delivery split into fixed/volumetric) for representative-household charts; `load_delivery_mc_heatmap` loads and joins a utility's hourly bulk-tx/dist-sub-tx marginal costs into one 8760-row frame; `tariff_month_rate_table` builds a month-by-month volumetric-rate comparison table across any number of tariffs; `bat_component_delta` diffs arbitrary master-BAT columns between two segments (optionally filtered to one `utility`), for per-building revenue/marginal-cost/etc. deltas. `weighted_range_pcts` bins a value column into caller-defined `[lo, hi)` intervals (use ±inf for tails); `quadrant_pcts` is the fixed +/- $1k wrapper used for bill-change quadrant bars. Plotting helpers include `plot_bill_change_quadrants` (savings/loss quadrants by heating type), `plot_weighted_bill_change_hist` (weighted histogram of annual bill change, colored by quadrant), `plot_weighted_hist_by_group` (weighted histogram of any value column faceted by a group column, e.g. incremental kWh by heating type), `plot_mc_heatmap` (8760 day-of-year x hour-of-day marginal-cost heatmap, given any dataframe with `day_of_year`/`hour`/value columns), `plot_monthly_bill_components` (stacked monthly delivery/supply bars for one building), `plot_decomposed_bill_3bar` (three-bar decomposed annual bill: gas furnace vs. HP on the default rate vs. HP on a reform rate), and `plot_ratedesign_monthly_hist`/`plot_ratedesign_cdf` (rate-design-only, pre-retrofit monthly bill-change histogram/CDF). Single-fuel variants: `plot_bill_change_quadrant_single` (deprecated — one horizontal bar for one heating type under one rate; use `plot_bill_change_quadrant_rate_comparison` with a single `rates` entry instead) and `plot_bill_change_quadrant_rate_comparison` (one heating type across any number of rate scenarios including a single rate, convenience wrapper around `plot_bill_change_quadrant_comparison` that handles segment loading and filtering internally; accepts a `title` kwarg to override the auto-generated title). |
| `lib.rates_analysis.cos_funcs`        | Incremental delivery revenue vs. marginal-cost analysis. `bat_component_delta_with_gap` diffs `annual_bill_delivery` and `economic_burden_delivery` between two `{scenario}_{stage}` segments for one utility, adds `delta_gap`, and joins in per-building incremental grid kWh (`rate_case_funcs.load_billing_kwh_annual`); `bat_component_summary_by_heating_type` summarizes that result by baseline heating type (plus an "All non-HP" total), including weighted-sum-ratio `$`-per-incremental-kWh for both revenue and marginal cost.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `lib.rates_analysis.ncp_funcs`        | Subclass non-coincident-peak (NCP) cost-causation analysis, computed purely from ResStock hourly load curves (no utility ECOSS dollar data needed). `grid_consumption_expr` replicates CAIRO's `max(total - abs(pv), 0)` "grid consumption" formula (not `out.electricity.net`, which drifts from `total`); `load_subclass_population` pulls one utility's per-building heating-type-v2 subclass and weight from CAIRO's master bills; `scan_hourly_loads_for_buildings` constructs per-building parquet paths directly for a given ResStock release/state/upgrade; `validate_hourly_against_annual_kwh` empirically discovers any uniform per-utility `resstock_kwh_scale_factor` CAIRO applied (median per-building ratio) and warns on buildings that deviate from it; `aggregate_hourly_by_subclass` produces one weighted hourly load curve per subclass; `compute_subclass_ncp_shares` gives each subclass its own peak magnitude/timestamp and share of a hypothetical NCP allocation; `subclass_ncp_report_vars` extracts the heat-pump/natural-gas customer-share and NCP-share scalars used in testimony.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `lib.rates_analysis.fair_default`     | Closed-form fair-default rate design. `fair_default_fixed_charge_only()` uses the V^0-based Cramer formula to eliminate a subclass's static BAT cross-subsidy. `fair_default_incremental_fixed_charge()` instead targets paired pre/post-retrofit overpayment: it scales selected volumetric charges until their retrofit increment equals incremental marginal cost, then raises the class fixed charge to preserve revenue. Both return F\*, Δ, λ\*, and feasibility.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `lib.rates_analysis.hh_summary_funcs` | Household-level base table for `<state>_hp_rates` "summary by heating type" notebooks: ResStock metadata (the heating/fuel attributes CAIRO's post-processing already carries onto master bills, including `postprocess_group.heating_type_v2`, the three `in.hvac_cooling_*` equipment columns, `in.income`/`in.representative_income`, `in.federal_poverty_level`) joined to the `is_lmi_any` LMI-eligibility flag from CAIRO's master bills. `resstock_metadata_path`/`utility_assignment_path` resolve local-EBS-mirror-then-S3 paths; `scan_cooling_columns` scans just the cooling trio for exploring distinct values before committing to them; `load_household_metadata`/`join_lmi_flag`/`load_household_summary_base` build the per-building table (optionally restricted to one electric utility via `scan_utility_bldg_ids`'s semi-join against `utility_assignment.parquet`, since ResStock metadata itself carries no utility column), parameterized by state/batch/release/utility so any state's report can reuse this as a template. `cooling_coverage_share_by_heating_type` aggregates the base table into weighted cooling-coverage shares per heating type -- relabeling `heating_type_v2` codes via `rate_case_funcs.add_heating_label`/`HEATING_ORDER` rather than a separate copy, so categories match every other heating-type breakdown in a report; `plot_cooling_coverage_by_heating_type` renders that as a stacked bar (single-hue sequential fill, in-bar % labels). `bill_delta_direction_by_cooling_coverage` joins a `rate_case_funcs.bill_delta_between_segments` result (e.g. a natural-gas-to-heat-pump bill delta) to the base table's cooling-coverage attribute and runs `bill_change_incidence` within each coverage level, yielding weighted bill-increase/no-change/decrease shares per level; `plot_bill_delta_direction_by_cooling_coverage` renders that as a stacked bar (carrot/gray/sky fill, short x-axis labels via `COOLING_COVERAGE_LABELS`).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| `lib.great_tables`                    | Switchbox Great Tables helpers (`get_switchbox_gt_tab_options`) for `GT.tab_options`: table title matches plotnine (GT Planar Bold 15px left); IBM Plex Sans on body/stub/source notes. `render_gt_to_icml` converts a `GT` to a native InDesign `<Table>` XML fragment referencing the designer's named styles. `set_icml_table_style(gt, "Table")` overrides the default `Table Inline` style per table.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `lib.quarto`                          | Quarto Manuscript helpers. `display_figure(fig)` renders a matplotlib figure as SVG (HTML and ICML) or high-res PNG (DOCX only, to dodge Pandoc's 96-DPI rsvg-convert) for safe embedding (avoids multi-MIME bug); `display_svg` is an alias. `display_gt(table)` base64-encodes a GT table for HTML, PNG-screenshots for DOCX (`SWITCHBOX_GT_AS_IMAGE=1`), and emits a native InDesign `<Table>` for ICML (`SWITCHBOX_TYPESET=1`, via `lib.great_tables.icml` + SVG carrier + `icml_gt.lua`). Use `display_figure` for raw matplotlib figures; `display_gt` for all GT tables embedded via `{{< embed >}}`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
filters:
  - at: pre-quarto
    path: ../../lib/quarto_extensions/icml_gt/icml_gt.lua
  - at: pre-quarto
...
| `icml_gt/`     | ICML-only: extracts base64-encoded ICML `<Table>` XML from the 1×1 placeholder SVG emitted by `display_gt` and promotes it to a raw ICML block, so GT tables land as native editable InDesign tables. Must load **before** `icml_floats` to intercept carriers. |
