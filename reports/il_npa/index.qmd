...
For multi-family parcels, which can contain or intersect multiple building footprints, we attribute the parcel's unit count to the single building with the largest absolute footprint-overlap area. (Single-family parcels are assigned one unit by definition.) Because the Cook County parcel boundaries and Chicago building footprints are produced by independent surveys with different precisions, this attribution is occasionally noisy at parcel edges: a building that mostly belongs to a neighboring parcel can bleed across the shared boundary and, if that bleed area is larger than any of the parcel's own building overlaps, be selected as the match for the wrong parcel. As a sensitivity check — not because it is inherently more correct — we re-counted multi-family units under an alternative rule that only credits a parcel with a building when the building's footprint is majority-inside that parcel's boundary. Neither rule is definitive: both are reasonable ways to reconcile two independently surveyed datasets, and each makes different tradeoffs (for example, the alternative rule leaves buildings that straddle legal parcel lines unattributed to any parcel). Comparing the two gives a sense of how sensitive our unit counts are to this methodological choice. At the individual parcel level, the two rules disagree in both directions — some parcels are over-counted by the production rule, others are under-counted — so the effect on any single parcel is unpredictable. Summed across all multi-family parcels in our scope, those individual discrepancies do not fully cancel: the alternative rule returns about 5% fewer units in aggregate. At the block level (our final unit of analysis), 80% of fully-residential blocks have identical unit counts under the two rules, and the remaining blocks differ by no more than two units.
