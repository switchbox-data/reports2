...
| `lib.rates_analysis.rate_case_funcs` | Python rate-case analysis over CAIRO master tables: loading, bill-change incidence, BAT over/underpayment, representative-household decomposition, and the charts built from them. `weighted_range_pcts` returns weighted percents of a value column in caller-defined low-end, middle (one or more `[lo, hi)` intervals), and high-end ranges; `quadrant_pcts` is the fixed +/- $1k wrapper used for bill-change quadrant bars.                                                                                                                                                                             |
