...
- **Fixed:** a set monthly charge for being connected to the grid (e.g. $16 per month), similar to a rent or lease payment
- **Volumetric:** a per kWh charge for every unit of electricity consumed (e.g. 10¢/kWh), similar to pumping gasoline at a gas station
...
And therein lies the problem: heat pumps use significantly more electricity than the fossil heating systems they replace. Their supply cost of service goes up — they are consuming more electricity, after all — and their supply bill rises to match it.[^er] But because delivery bills are also largely calculated using volumetric rates, the amount they pay for poles and wires goes up significantly as well, even though their delivery cost of service only increases slightly — and in some cases actually decreases.[^cooling]
...
In this report, we call a rate {{< glossary "Cost-based rate" display="cost-based rate" >}} if it eliminates the cross-subsidy group of customers is paying into — in other words, if the rate collects total delivery revenue from that group equal to the group's delivery cost of service. A rate that is **cost-based** for a group of customers solves the fairness problem for that group.
...
In this report, we define a {{< glossary "Time-varying rate" display="time-varying rate" >}} as a volumetric rate whose ¢/kWh price changes over time. Examples include {{< glossary "Seasonal rate" display="seasonal rates" >}}, {{< glossary "Time-of-use rate (TOU)" display="time-of-use (TOU)" >}}, critical peak pricing (CPP), and real-time pricing (RTP) (see @sec-cost-reflective-rates for details).

We define a {{< glossary "Cost-reflective rate" display="cost-reflective rate" >}} as a time-varying rate whose ¢/kWh prices track the underlying costs of generating and delivering electricity to a group of customers — costs which themselves change over time.[^underlying-costs]
...
The same four scenarios apply to LMI households that heat with **heating oil or propane** (delivered fuels the utility discount programs do not reduce):

::::: {.column-page-inset-right}
{{< embed notebooks/analysis.qmd#fig-burden-bar-lmi-current-op >}}
:::::

- Today **`{python} pct(v.pct_burdened_current_op_lmi40)`** of this cohort is highly energy burdened — well above the **`{python} pct(v.pct_burdened_current_lmi40)`** share for the gas-heated LMI population — reflecting the cost of oil and propane relative to (discounted) electricity and natural gas in our bills.
- After a heat-pump switch on default rates, **`{python} pct(v.pct_burdened_hp_default_op_lmi40)`** would be highly burdened — a **decrease** of `{python} f"{(v.pct_burdened_current_op_lmi40 - v.pct_burdened_hp_default_op_lmi40) * 100:.0f}"` percentage points from today, unlike the increase for gas (because heating load shifts from expensive delivered fuel to electricity and total energy bills can fall even before rate reform).
- The seasonal heat pump rate plus 40% EAP enrollment (third bar) brings the highly burdened share to **`{python} pct(v.pct_burdened_hp_seasonal_op_lmi40)`**; the fourth bar (seasonal rate + 100% EAP enrollment) drops it to **`{python} pct(v.pct_burdened_hp_seasonal_op_lmi100)`** — down `{python} f"{(v.pct_burdened_current_op_lmi40 - v.pct_burdened_hp_seasonal_op_lmi100) * 100:.0f}"` percentage points from today's delivered-fuel baseline.

And the same for LMI households on **baseline electric resistance** heating (no fossil fuel, very high winter electricity use):

::::: {.column-page-inset-right}
{{< embed notebooks/analysis.qmd#fig-burden-bar-lmi-current-er >}}
:::::

- Today **`{python} pct(v.pct_burdened_current_er_lmi40)`** of LMI electric-resistance households is highly energy burdened — well above the gas LMI share — because resistance heating is electricity-intensive and expensive to run.
- On heat pumps and default rates, the highly burdened share falls to **`{python} pct(v.pct_burdened_hp_default_er_lmi40)`** — a decrease of `{python} f"{(v.pct_burdened_current_er_lmi40 - v.pct_burdened_hp_default_er_lmi40) * 100:.0f}"` percentage points (a modern heat pump delivers the same heat for far less electricity than resistance equipment).
- Under the seasonal heat pump rate, **`{python} pct(v.pct_burdened_hp_seasonal_er_lmi40)`** remain highly burdened with ~40% EAP enrollment; with 100% enrollment, **`{python} pct(v.pct_burdened_hp_seasonal_er_lmi100)`** — a decline of `{python} f"{(v.pct_burdened_current_er_lmi40 - v.pct_burdened_hp_seasonal_er_lmi100) * 100:.0f}"` percentage points vs. today's electric-resistance baseline.
...
Compared to {{< glossary "Default rates" display="default rates" >}}, a {{< glossary "Seasonal rate" display="seasonal" >}} {{< glossary "Time-of-use rate (TOU)" display="TOU rate" >}} would dramatically improve gas-to-heat-pump bill outcomes (@fig-quadrant-bar-natgas-tou-state).
...
These show when **bulk-transmission** and **distribution+sub-transmission** marginal *capacity* costs (the delivery side of the CAIRO marginal-cost stack) are positive across the 8760 hours, by utility. Each map uses a separate color scale for its two T/D components so the seasonal and diurnal patterns in each are readable even when typical magnitudes differ. See the companion notebook for methodology.
