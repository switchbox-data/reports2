...
# Introduction
This memo examines the impact of switching to heat pumps on the energy bills of homes in Rhode Island, under today's electric rates.
...
- Unless otherwise noted, the analysis only evaluates households served by **investor-owned electric utilities**. Municipal utility customers are out of scope.
...
- The **heating season**, which is also called "**winter**" throughout the report, is defined as November to April for Rhode Island Energy.
...
- Gas and electricity **supply rates** are from **2024**, as well as state average propane and heating oil prices from the same year, are assumed throughout.
...
- Electricity supply rates **do not reflect municipal aggregation** programs. Many of Rhode Island's residential electricity customers are served by these programs, which tend to offer lower rates than basic service from investor-owned utilities, so our analysis is likely overestimating heat pump operating costs for these customers.
- We model Rhode Island Energy's **default electric delivery rates** only. Switchbox has not yet developed heat pump rates for Rhode Island, so analyzing their impact is out of scope.
...
[^note]: This is an industry-standard dataset from the National Renewable Energy Laboratory (NREL) that simulates the energy consumption of a statistically representative sample of homes in Rhode Island [@nrel_ResStock_2024].
...
In this section, we look at the impact of switching to heat pumps on the annual energy bills of a single, representative home in Rhode Island.
...
This section looks the _median_ annual energy bill for each type of customer, and how that changes after switching to heat pumps. In a later section, we will look at the _range_ of bill impacts for all homes in Rhode Island.
...
With this dramatic increase in the electric bill, the overall bill increases by `r ((post_hp_total_bill_gas / pre_hp_total_bill_gas)) |> scales::percent()` to `r post_hp_total_bill_gas |> scales::dollar(accuracy = 1)`.
...
This likely results in the household overpaying for electrical delivery, as their new winter time heat pump load does not contribute to the grid's summertime peak load, and thus does not trigger grid capacity upgrades that they would need to pay for (via higher delivery charges).
...
As expected, the same seasonal pattern holds, with wintertime gas use now converted to electricity. The heat pump installation has a variable impact on the monthly bills; in the case of Rhode Island Energy, where per-kWh delivery rates are about 3.4x higher for electricity than for gas[^note_spark_gap], the total home energy bills are higher in the winter months, in proportion to the increase in electricity consumption.

[^note_spark_gap]: The Rhode Island Energy all-in per-kWh [electric delivery rate](https://www.documentcloud.org/documents/26078348-2025-7-1-summary-of-rates-2095/) used in this report is $0.14071/kWh, while the all-in per-kWh [gas delivery rate](https://www.documentcloud.org/documents/26078485-2025-rie-gas-rates/) for heating customers is $1.234/therm, equivalent to $0.0421/kWh.
...
The switch to heat pumps results in an annual energy bill savings of `r (pre_hp_total_bill_fuel - post_hp_total_bill_fuel) |> scales::dollar(accuracy = 1)`.
...
The heat pump installation leads to savings in all the winter months, which drives the annual energy bill savings compared to the fuel oil heating system.
...
Since they are not switching _fuels_, only technologies, the efficiency advantage from heat pumps lead directly to electrical bill savings. As expected, both supply and delivery charges decrease, in proportion to the lower electricity consumption.
...
@fig-hist_1_winter_bill_impact shows the distribution of energy bill _changes_ during the winter for every home in Rhode Island.

:::{.column-page-inset-right}
{{< embed notebooks/analysis.qmd#fig-hist_1_winter_bill_impact >}}
:::

`r winter_changes_all_fuels_category_percentages |> filter(bill_category == "pct_that_save") |> pull(percentage)`% of homes would cut their wintertime energy bills by switching to heat pumps. That savings would be modest, however: only `r winter_changes_all_fuels_category_percentages |> filter(bill_category == "large_savings") |> pull(percentage)`% of homes would see a bill decrease of more than $1,000 per year. Meanwhile, `r winter_changes_all_fuels_category_percentages |> filter(bill_category == "pct_that_lose") |> pull(percentage)`% of homes would see a bill increase, with `r winter_changes_all_fuels_category_percentages |> filter(bill_category == "large_increase") |> pull(percentage)`% of them facing an increase of more than $1,000 per year.
...
Just over half of households would lower their summer energy bills by switching to heat pumps, while the rest would raise them. What's going on here?
...
A majority (`r round(cooling_type_pct |> filter(baseline_cooling_type_group == "Room AC") |> pull(pct) * 100)`%) of Rhode Island homes use have room-by-room AC (e.g. window units), which may not provide cooling to the entire home.

A significant minority (`r round(cooling_type_pct |> filter(baseline_cooling_type_group == "Central AC") |> pull(pct) * 100)`%) have central AC (e.g. through ducts), which does. There is also a small but important group (`r round(cooling_type_pct |> filter(baseline_cooling_type_group == "None") |> pull(pct) * 100)`%) of homes that have no cooling.
...
The findings in preceding sections help us assess the overall impact of default electric rates on heat pump operating costs, compared to current HVAC systems: statewide, most households would lower their existing heating bills by switching to heat pumps. During the summer, households that already have added cooling would cut their bills by switching to heat pumps, while those without would see an increase, reflecting the (optional) gain of function of adding air conditioning.
...
Most homes (`r annual_change_stats_fuel_oil_baseline_hp_high |> filter(bill_category == "pct_that_save") |> pull(percentage)`%) currently heating with fuel oil would save by switching to a heat pump under today's electric rates. Only `r annual_change_stats_fuel_oil_baseline_hp_high |> filter(bill_category == "pct_that_lose") |> pull(percentage)`% would see bill increases, with very few (`r annual_change_stats_fuel_oil_baseline_hp_high |> filter(bill_category == "large_increase") |> pull(percentage)`%) facing an increase of more than $1,000 per year.
...
Savings are indeed significant, and nearly universal: 60% of these customers would save over $1,000 per year by switching to heat pumps.

Why would `r annual_change_stats_resistance_baseline_hp_high |> filter(bill_category == "pct_that_lose") |> pull(percentage)`% of these customers see an a small increase in their annual energy bills?
...
**@fig-burden_hists_all_smi_tiers** shows the distribution of energy burdens for all househods in the state, on their current HVAC systems and after switching to heat pumps—under today's electric rates.
...
On the one hand, these findings should allay concerns that heat pums will systematically worsen Rhode Island's energy affordability crisis. But on the other hand, these results make clear that heat pump rates are still needed to turn heat pump adoption into an engine of energy affordability.
...
**@fig-bar_smi_tiers** shows the distribution of households incomes in Rhode Island, grouped into income tiers by State Median Income (SMI).
...
@fig-burden_hists_3_smi_tiers_hp_high shows that low-income burdens go down slightly, moderate-income burdens stay essentially the same, and the number of highly burdened non-LMI households goes up by eight percentage points.
...
ResStock 2024 has 3 different heat pump models, specified by HSPF: 9.2, 11, and 13. We refer to these as "low", "high", and "best" efficiency heat pumps.

Under baseline rates, about 30% of homes would save by switching to low efficiency heat pumps, 46% of homes would save by switching to high efficiency heat pumps, and 56% of homes would save by switching to best efficiency heat pumps.

In other words, heat pump efficiency can have a large impact on bill savings.


# Appendix: Data and Methods {#sec-data-and-methods}
Our analysis calculates the monthly electricity, natural gas, fuel oil, and propane bills—before and after switching to cold-climate air-source heat pumps—for a set of several thousand virtual homes, which comprise a statistically representative sample of the Rhode Island residential housing stock.
...
Delivery tariffs are drawn from the directly from utilities' filings with the Rhode Island Public Utilities Commission (PUC).
