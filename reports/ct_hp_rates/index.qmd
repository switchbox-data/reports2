---
title: "Heat Pump Operating Costs in Connecticut"
subtitle: "A Preliminary Analysis of the Bill Impacts from Switching to Heat Pumps"
date: 2025-10-30
author:
  - name: Bryan Murray
...
- This memo focuses only on **cold-climate air-source heat pump** (ccASHP) retrofits; ground-source heat pumps (GSHPs) are out of scope.
...
As expected, the same seasonal pattern holds, with wintertime gas use now converted to electricity. This drives the annual energy bill increase compared to the natural gas home.
...
The most common heating types in Connecticut are fuel oil (`r round(fuel_types_pct |> filter(hvac_group == "Fuel Oil") |> pull(pct) * 100)`%) and natural gas (`r round(fuel_types_pct |> filter(hvac_group == "Natural Gas") |> pull(pct) * 100)`%), with a significant portion using electric resistance (`r round(fuel_types_pct |> filter(hvac_group == "Electric Resistance") |> pull(pct) * 100)`%).
...
The situation is reversed for fuel oil customers: `r annual_change_stats_fuel_oil_baseline_hp_high |> filter(bill_category == "pct_that_save") |> pull(percentage)`% would save on total annual energy bills, while only `r annual_change_stats_fuel_oil_baseline_hp_high |> filter(bill_category == "pct_that_lose") |> pull(percentage)`% would see bill increases.
