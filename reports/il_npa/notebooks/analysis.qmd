...
pg_summary["elec_cost_total"] = (
    pg_summary["elec_sf_cost"]
    + pg_summary["elec_mf_cost"]
    + pg_summary["elec_comm_cost"]
    + pg_summary["street_miles"] * cost_decomm_mile
)

pg_summary["pct_residential"] = (pg_summary["sf_parcels"] + pg_summary["mf_parcels"]) / pg_summary["non_vacant_parcels"]

pg_summary["comm_ind_total_count"] = pg_summary["commercial_parcels"] + pg_summary["industrial_parcels"]
...
pg_summary["npa_over_prp"] = pg_summary["elec_cost_total"] / pg_summary["prp_cost"]
...
pg_summary.loc[pg_summary["comm_ind_flag"], "comp_cost_discrete_res_only"] = "not eligible"

```

```{python}

res_only = pg_summary.loc[pg_summary["comm_ind_flag"] == False]
...
n_res_blocks = (pg_summary["comm_ind_flag"] == False).sum()

n_res_blocks_cheaper = (res_only["npa_over_prp"] <= 1).sum()

pct_res_blocks_cheaper = (n_res_blocks_cheaper / n_res_blocks) * 100

```

```{python}
# how many fully residential blocks  are within 1 to 1.5 times the cost of pipeline replacement  (total & number percent)
n_res_blocks_1_to_1_5 = ((res_only["npa_over_prp"] > 1) & (res_only["npa_over_prp"] < 1.5)).sum()

n_res_blocks_0_to_1_5 = (res_only["npa_over_prp"] < 1.5).sum()

pct_res_blocks_1_to_1_5 = (n_res_blocks_1_to_1_5 / n_res_blocks) * 100

pct_res_blocks_0_to_1_5 = (n_res_blocks_0_to_1_5 / n_res_blocks) * 100
```

```{python}
# 1.0 threshold cost difference
prp_total_ce_1_0 = res_only[res_only["npa_over_prp"] < 1.0].prp_cost.sum()

npa_total_ce_1_0 = res_only[res_only["npa_over_prp"] < 1.0].elec_cost_total.sum()

total_cost_diff_1_0 = npa_total_ce_1_0 - prp_total_ce_1_0

# 1.5 threshold cost difference
prp_total_ce_1_5 = res_only[res_only["npa_over_prp"] < 1.5].prp_cost.sum()

npa_total_ce_1_5 = res_only[res_only["npa_over_prp"] < 1.5].elec_cost_total.sum()
...
n_res_units = res_only["total_residential_units"].sum()
n_sf = res_only["sf_parcels"].sum()
n_mf = (res_only["total_residential_units"] - res_only["sf_parcels"]).sum()

n_res_units_1_0 = res_only[res_only["npa_over_prp"] < 1.0]["total_residential_units"].sum()
n_sf_1_0 = res_only[res_only["npa_over_prp"] < 1.0]["sf_parcels"].sum()
n_mf_1_0 = (res_only[res_only["npa_over_prp"] < 1.0]["total_residential_units"] - res_only[res_only["npa_over_prp"] < 1.0]["sf_parcels"]).sum()

n_res_units_1_5 = res_only[res_only["npa_over_prp"] < 1.5]["total_residential_units"].sum()
n_sf_1_5 = res_only[res_only["npa_over_prp"] < 1.5]["sf_parcels"].sum()
n_mf_1_5 = (res_only[res_only["npa_over_prp"] < 1.5]["total_residential_units"] - res_only[res_only["npa_over_prp"] < 1.5]["sf_parcels"]).sum()
