...
```{python}
    ...
    current_adoption_rate = sbmeta.group_by("postprocess_group.heating_type").agg(pl.col("weight").sum().alias("total_homes")).pipe(lambda df: df.filter(pl.col("postprocess_group.heating_type") == "heat_pump")["total_homes"] / df["total_homes"].sum()).item()

    if DEBUG_MODE:
        print(current_adoption_rate) # NOTE: DEBUGGING ONLY
        scaled_total = sbmeta["weight_scaled"].sum()
        if abs(scaled_total - total_num_homes) > 1e-6 * total_num_homes:
            raise ValueError(
...
```{python}
    bau_hp_count_lag = np.array(get_adoption_forecast_num_homes([2028, 2029, 2030, 2031]))- np.array(get_adoption_forecast_num_homes([2027, 2028, 2029, 2030]))
...
```{python}
    ...
    def _draw_year_built(rng: np.random.Generator, vintage: str | None) -> int:
        ...
        if lo_hi is None:
            # Fallback: put unknowns in a reasonable mid-century bucket.
            lo_hi = (1950, 2000)
        lo, hi = lo_hi
...
```{python}
    ...
    def _get_load_curves_aggregated(scenario, scenario_name):
        ...
        for yr in range(n_years):
            ...
            df = adjust_timestamp(df.lazy(), start_year=start_year).collect()
