...
- Electric-resistance homes staying on ER: `{python} pct(v.pct_er_stay_save_ehrate)` save under the electric heating rate, averaging `{python} dollar(abs(v.mean_monthly_er_stay_delta_ehrate), 2)`/month.
...
- ER → HP economics under three rate regimes: `{python} pct(v.pct_er_switch_save_default)` of ER homes save by upgrading equipment to HP under today's default rate (average `{python} dollar(abs(v.mean_er_switch_delta_default))`/yr). Moving new HP customers onto the HP seasonal rate lifts that to `{python} pct(v.pct_er_switch_save_default_to_hprate)` (average `{python} dollar(abs(v.mean_er_switch_delta_default_to_hprate))`/yr). For ER customers already on the electric heating rate, upgrading to HP while staying on the EH rate saves `{python} pct(v.pct_er_switch_save_ehrate)` of them (average `{python} dollar(abs(v.mean_er_switch_delta_ehrate))`/yr).
