...
```{python}
    def get_adoption_forecast_num_homes(projection_years: list) -> list[float]:
        ...
        df = pl.DataFrame([row[:num_cols] for row in rows[start_idx+1:]], schema=rows[start_idx][:num_cols])

        return df.filter(pl.col("Year").is_in([str(x) for x in projection_years])).select(pl.col("Cumulative_Base")).to_series().to_list()
```

```{python}
    sbmeta = pl.scan_parquet("s3://data.sb/nrel/resstock/res_2024_amy2018_2/metadata/state=RI/upgrade=00/metadata-sb.parquet")
    # TODO: remove this hack once weight column is in the metadata
    sbmeta = sbmeta.with_columns(
        pl.lit(252.301639).alias("weight")
    )

    # TODO: fix to make sure you get the "true" case
    current_adoption_rate = sbmeta.collect().group_by("postprocess_group.has_hp").agg(pl.col("weight").sum().alias("total_homes")).pipe(lambda df: df.filter(pl.col("postprocess_group.has_hp") == True)["total_homes"] / df["total_homes"].sum()).item()
    ...
    # TODO: come up with better hp rate adoption projections
    hp_pct_total = [1.5 * x / total_num_homes for x in bau_hp_counts]
