...
if parcels_classified is not None and buildings is not None and len(buildings) > 0:
    ...
    if len(test_parcel) > 0:
        ...
        if test_parcel['matched_building_id'].values[0] == "631646":
            ...
            print(f"   ⚠️  Match differs from expected")
...
if pg_polygons is not None:
    ...
    summary = summary.merge(
        street_miles_by_polygon.reset_index().rename(columns={'length_miles': 'street_miles'}),
        on='OBJECTID',
        how='left'
    ).merge(
        ...
        unit_counts.reset_index().rename(columns={'building_units': 'total_residential_units'}),
        on='OBJECTID',
        how='left'
    )
    ...
    int_cols = ['total_parcels', 'sf_parcels', 'mf_parcels',
