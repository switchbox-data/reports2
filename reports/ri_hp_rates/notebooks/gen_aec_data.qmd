...
```{python}
    ...
    _sum_weight = sbmeta.select(pl.col("weight").sum()).collect().item()
    ...
    if DEBUG_MODE:
        ...
        scaled_total = sbmeta.select(pl.col("weight_scaled").sum()).collect().item()
...
```{python}
    ...
    DEVICE_LIFETIME_PARAMS = {
        # TODO: confirm / update these params + names
    ...
    def append_hvac_failure_year(
        ...
        *,
        ...
        df = (
            sbmeta.select(["bldg_id", col_heating_type, col_vintage])
            .collect()
            .rename({col_heating_type: "heating_type", col_vintage: "vintage"})
        )

        # Fill null/unknown heating types to "other"
        df = df.with_columns(
        ...
        return sbmeta.join(failure_df.lazy(), on="bldg_id", how="left")

    # Attach the column to sbmeta for downstream use
    sbmeta = append_hvac_failure_year(sbmeta, current_year=2025, seed=0)
