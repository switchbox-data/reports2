...
(i) Single-family electrification: `r sample_sf` single family units at an cost of `r dollar(cost_elec_sf, scale = 1e-3, suffix = "K", accuracy = 1)` to fully electrify each unit, totaling `r dollar(sample_elec_sf_cost, scale = 1e-3, suffix = "K", accuracy = 1)`.
...
(iii) Electric distribution upgrade: the incremental load associated with electrifying `r sample_total_units` residential units would prompt electric distribution upgrades costing `r dollar(grid_upgrade_cost_hh, accuracy = 1)` per unit, totaling `r dollar(sample_grid_cost, scale = 1e-3, suffix = "K", accuracy = 1)`. (This particular block may or may not distribution upgrades—the marginal cost of distribution upgrades we use here is a citywide average.)

(iv) Pipe decommissioning: the same approximately `r round(sample_attributed_pipe, 3)` miles of estimated pipe would cost approximately `r dollar(cost_decomm_mile, scale = 1e-3, suffix = "K", accuracy = 1)` per mile to decomission, totaling `r dollar(sample_decomm_cost, scale = 1e-3, suffix = "K", accuracy = 1)`.
...
The block's TE-to-PRP cost ratio is approximately `r percent(sample_ratio, accuracy = 1)` — so it is more expensive to electrify than to replace pipe on this particular block block. It is not part of the cost-effective set under Perspective 1.

The same methodology, applied  all `r comma(n_res_blocks)` in-scope residential blocks, drive every finding in this section.
...
**Q.** If PGL undertook TE on these `r comma(n_res_blocks_cheaper)` cost-effective blocks, how many housing units would be upgrades, how many miles of pipe would be decommissioned, and how much upfront capital would be saved?
...
2. **Leak reduction.** The SMP Investigation Order's leak-reduction objective — to replace PGL's leak-prone CI/DI pipe[^smp-leak-objective] — is met identically by removing the gas line on a block as by replacing it. On the cheaper-to-electrify blocks, removal achieves the same safety outcome at lower upfront capital cost than replacement, while simulatenously advancing Chicago's residential electrification goals. (Other non-pipeline alternatives — lining, repair, smaller-scope retirements — are not in scope for this testimony but could in principle reduce leak risk at still lower cost on these blocks.)
...
Moving from the left, the cumulative-savings curve rises as more cheaper-to-electrify blocks are retrofitted and peaks at the Perspective 1 threshold of approximately `r percent(pct_res_blocks_cheaper, accuracy = 0.1)` — in other words, at the percentage of blocks for which it is strictly cheaper to electrify than to replace pipe. The curve then declines as more-expensive-to-electrify blocks are retrofitted and the saving are "spent down". The curve crosses zero at approximately `r percent(zero_crossing, accuracy = 0.1)` of the blocks in the portfolio.
...
**Q.** Can you illustrate the Perspective 3's methodology through an example?
...
Adding the net present value of that scattershot electrification to the PRP side of the comparison adds approximately `r dollar(sample_scattershot_npv, scale = 1e-3, suffix = "K", accuracy = 1)` in up-front capital cost. Once it is added, the same block that was not cost-effective under Perspective 1 becomes cost-effective: the TE-to-(PRP+scattershot) ratio falls to approximately `r percent(sample_ratio_ss, accuracy = 1)`.
...
This bar chart is comparing the following plain-English scenarios:
...
For the same total cost as replacing the pipe and electrifying only a third of the phomes, TE would deliver the same leak-reduction outcome and electrify 100% of the block.
...
While the size of these capital expenditures is comparable, their impact is not: under PRP + scattershot, the pipe on all blocks would be replaced at the same time that a third of the homes served by this pipe exit the gas sytem. Under TE, all the pipe would be decomissioned, and all households would be electrified.
...
The reason is structural: it lies in the exclusive use of riskiest-first project-selection PRP. Two problems follow from that approach, and neither can be fixed without adding an additional "track" of pipe retirement projects that runs parallel to riskiest-first.
...
However, this hydraulic feasibility questions depends on which adjacent segments have already been decommissioned. Decommissioning is thus a _network_ problem, not a project-by-project problem.
...
Risk-first project selection is oblivious to that network context. It selects the segment scoring highest on probabilistic risk, not the segments that are hydraulically feasible to decommission, and which would enable other segments to be decommissioned. The realized share of pipe that turns out to be hydraulically decommissionable under risk-first selection will necessarily fall below the share that could have been decommissionable under planning exercise that identifies a decomisioning sequence that maximizes the overall hydraulic feasibility of TE. That is true mathematically, before any field data is collected.

I am not suggesting that PGL stop addressing the highest risk pipe. I am simply pointing out that maximing the hydraulic feasibility of TE requires system-wide planning and proactive decommissioning, not a reactive project-by-project NPA-evaluation.

<br>

**Q.** What is the consequence of these two problems for large-scale TE?

**A.** If TE is only ever evaluated project-by-project for riskiest pipe segments, PGL is likely to rarely, if ever, find it to be a timely and hydraulic-feasible option—even if it is cost-effective.

<br>

**Q.** What would the right structural fix look like?

**A.** Capturing a meaningful share of the cost-effective TE potential identified by this testimony requires a parallel "second track" of the PRP program based on hydraulic feasibility planning and proactive decomissioning and electrification of entire sections of the gas network. This second track would not be a replacement for the existing process, but an addition to it.

- Track 1 (updated current process): continue risk-first retirement on the truly highest-risk pipe — those segments that genuinely cannot wait two to three years for a coordinated block-level electrification project to organize. In this track, pipe repair and relining should be preferred to repair whenever feasible.

- Track 2 (new): plan proactively how to decommission the less-risky neighboring segments, in a sequence that maximize hydraulic decommissioning feasibility, on the subset of blocks where TE is cost-effective under whatever cost-effectiveness perspective the Commission adopts. This track would retire pipe segments "from the ends in" in addition to "riskiest-first."
...
The independence requirement is essential. The most useful precedent is the Safety Monitor mechanism the Commission established in the SMP investigation,[^smp-safety-monitor] which directed Commission Staff — not PGL — to issue an RFP and hire an outside auditor to oversee PGL's CI/DI retirement work, with the cost borne initially by PGL but recoverable through ratemaking. If PGL conducts the Part 1 study itself, it is at substantial risk of producing an endorsement of the status quo, given that its maximizes shareholder value by replacing pipe segments.
