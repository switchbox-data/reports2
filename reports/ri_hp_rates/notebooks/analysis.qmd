...
load_monthly_consumption_from_cache <- FALSE
...
if (load_results_from_cache) {
  ...
  monthly_bills_elec <- calc_monthly_bills(
    monthly_consumption = monthly_consumption,
    consumption_column = "electricity_total_consumption_kwh",
  ...
  left_join(housing_units |> select(all_of(cols_from_housing_units)), by = c("bldg_id", "upgrade"))

  # Monthly Change
  monthly_changes_elec <- calc_monthly_changes(monthly_bills_elec, "electricity") |>
    left_join(housing_units |> filter(upgrade == 0) |> select(bldg_id, hvac_primary, baseline_heating_type), by = "bldg_id")

  # BILLS by season
  winter_bills_elec <- calc_annual_bills_from_monthly(monthly_bills_elec, "electricity", months = winter_months) |>
    # add metadata about each bldg_id
    left_join(housing_units |> select(all_of(cols_from_housing_units)), by = c("bldg_id", "upgrade"))
...
if (load_results_from_cache) {
  ...
  monthly_bills_fuel_oil <- calc_monthly_bills(
    monthly_consumption = monthly_consumption,
    consumption_column = "fuel_oil_total_consumption_kwh",
    fuel_type = "fuel_oil",
...
if (load_results_from_cache) {
  ...
  monthly_bills_propane <- calc_monthly_bills(
    monthly_consumption = monthly_consumption,
    consumption_column = "propane_total_consumption_kwh",
    fuel_type = "propane",
...
if (load_results_from_cache) {
  ...
  # Monthly Gas Bills
  monthly_bills_gas <- calc_monthly_bills(
    monthly_consumption = monthly_consumption,
    consumption_column = "natural_gas_total_consumption_kwh",
...
annual_bills_total <- calc_annual_bills_total(annual_bills_elec, annual_bills_gas, annual_bills_fuel_oil, annual_bills_propane, housing_units=housing_units) |>
  left_join(housing_units |> select(all_of(cols_from_housing_units)), by = c("bldg_id", "upgrade"))

winter_bills_total <- calc_annual_bills_total(winter_bills_elec, winter_bills_gas, winter_bills_fuel_oil, winter_bills_propane, housing_units=housing_units) |>
  left_join(housing_units |> select(all_of(cols_from_housing_units)), by = c("bldg_id", "upgrade"))

summer_bills_total <- calc_annual_bills_total(summer_bills_elec, summer_bills_gas, summer_bills_fuel_oil, summer_bills_propane, housing_units=housing_units) |>
  left_join(housing_units |> select(all_of(cols_from_housing_units)), by = c("bldg_id", "upgrade"))
```
...
cols_from_housing_units_for_analysis <- c(
  "bldg_id",
  "upgrade",
  "baseline_heating_type",
  "hvac_primary",
...
annual_changes_total <- calc_annual_changes_total(annual_changes_elec, annual_changes_gas, annual_changes_fuel_oil, annual_changes_propane) |>
  left_join(housing_units |> filter(upgrade %in% c(1,2,3)) |> select(all_of(cols_from_housing_units_for_analysis)), by = c("bldg_id", "upgrade"))
...
annual_bills_elec_by_component <- monthly_bills_elec |>
  ...
  mutate(
    hvac = ifelse((!hvac_primary %in% c("hp_low", "hp_high", "hp_best")), "current", as.character(hvac_primary))
  ) |>
  summarize(
...
fuel_types_pct <- housing_units |>
  filter(upgrade == 0) |>
  group_by(hvac_primary) |>
  summarize(count = n()) |>
  mutate(percentage = count / sum(count) * 100) |>
  arrange(desc(count)) |>
  mutate(hvac_primary = factor(hvac_primary, levels = rev(hvac_primary)))

fuel_types_pct <- fuel_types_pct |>
  mutate(hvac_group = factor(hvac_primary,
...
burden_hists_all_current <- plot_energy_burden_histogram_standalone(
  ...
  mutate(
    hvac = ifelse((!hvac_primary %in% c("hp_low", "hp_high", "hp_best")), "current", hvac_primary)
  ) |>
  filter(
...
# # Monthly Combined
# monthly_changes_heating <- monthly_changes_heating_elec |>
#   filter(version == "baseline") |>
#   select(bldg_id, upgrade, year, month, monthly_change) |>
#   rename(monthly_change_elec = monthly_change) |>
#   left_join(
#     monthly_changes_heating_gas |>
#     select(bldg_id, upgrade, year, month, monthly_change) |>
#     rename(monthly_change_gas = monthly_change),
#     by = c("bldg_id", "upgrade", "year", "month")
#   ) |>
#   mutate(monthly_change = monthly_change_elec + monthly_change_gas) |>
#   left_join(housing_units |> filter(upgrade == 0) |> select(bldg_id, afue_value, hvac_primary, baseline_heating_type), by = "bldg_id") |>
#   filter(afue_value != 'NA')
# View(monthly_changes_heating)
...
View(annual_change_stats_heating_baseline_hp_high)
print(result[[1]])
