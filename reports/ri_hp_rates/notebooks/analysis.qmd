...
housing_units <- add_building_type_group(housing_units)
...
housing_units <- add_hvac_primary(housing_units) |>
  add_hvac_backup(housing_units) |>
  add_appliances(housing_units) |>
  add_shell(housing_units) |>
  right_join(housing_units, by = c("bldg_id", "upgrade"))

# Factors separately for each
hvac_levels <- c("current", "hp_low", "hp_high", "hp_best", "hp_geo", "electric_resistance", "none")
housing_units$hvac <- forcats::fct(housing_units$hvac, levels = hvac_levels)
```
...
housing_units <- add_heat_non_heat_flag(housing_units)
