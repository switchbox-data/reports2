...
# System-wide PRP miles per PGL's filing (Eldringhoff/Dickson, NSG-PGL Ex. 3.0).
prp_total_miles    <- 1020
n_prp_projects     <- 179
...
# Portfolio totals across all in-scope residential blocks.
total_prp_cost_portfolio    <- sum(res_only_df$prp_cost,        na.rm = TRUE)
total_npa_cost_portfolio    <- sum(res_only_df$elec_cost_total, na.rm = TRUE)
portfolio_savings_vs_prp    <- total_prp_cost_portfolio - total_npa_cost_portfolio

# Portfolio totals with scattershot baseline (§VII).
total_ss_npv_portfolio      <- sum(res_only_df$scattershot_npv, na.rm = TRUE)
...
Juan-Pablo Velez. Co-founder and Executive Director of Switchbox. 1 Whitehall Street, 17th Floor, New York, NY 10004.

**Q.** By whom are you employed and in what capacity?

**A.**

I am currently the Executive Director of Switchbox, a non-profit policy think tank building open analyses of state climate and energy policy for advocates, regulators, and the public.

**Q.** On whose behalf are you submitting this testimony?

**A.**

I am testifying on behalf of the Citizens Utility Board ("CUB").

**Q.** Please summarize your professional and educational background.

**A.**

I have a BA in Sociology from the University of Chicago. I took post-graduate math courses in probability theory, mathematical statistics, and vector calculus at Columbia University and NYU.
...
Yes. I sumbitted prefiled expert testimony before the Rhode Island Public Utilities Commission (RIPUC) on heat pump rates and bill-alignment analyses on 04/16/2026.
...
(iii) Replace the single-average cost-per-mile assumption used in this testimony with the **actual cost distribution by project type** under that sequence — addressing PGL's own observation that "they don't all cost the same."[^prp-cost-variation]

[^prp-cost-variation]: Direct Testimony of Polly M. Eldringhoff and Jerry Dickson, NSG-PGL [Ex. 3.0](https://www.icc.illinois.gov/docket/P2026-0065/documents/375324/files/657976.pdf) at 106–107.
...
**A.** No. My silence with respect to any position taken by the Companies or any other party in this case should not be construed as agreement with that position.
...
[^prp-traditional]: NSG-PGL [Ex. 3.0](https://www.icc.illinois.gov/docket/P2026-0065/documents/375324/files/657976.pdf) at 38–39.
...
[^prp-scope]: NSG-PGL [Ex. 3.0](https://www.icc.illinois.gov/docket/P2026-0065/documents/375324/files/657976.pdf) at 8, 105.
[^prp-project-spending]: NSG-PGL [Ex. 3.0](https://www.icc.illinois.gov/docket/P2026-0065/documents/375324/files/657976.pdf) at 106–107.
[^prp-total-spending]: Direct Testimony of Theodore Eidukas, NSG-PGL [Ex. 1.0](https://www.icc.illinois.gov/docket/P2026-0065/documents/375324/files/657962.pdf) at 4–6.
...
[^sc1-decline]: NSG-PGL [Ex. 1.0](https://www.icc.illinois.gov/docket/P2026-0065/documents/375324/files/657962.pdf) at 21–22.
...
[^prp-npa-options]: NSG-PGL [Ex. 3.0](https://www.icc.illinois.gov/docket/P2026-0065/documents/375324/files/657976.pdf) at 99.
...
[^gfs-not-feasible]: Direct Testimony of Frank C. Graves, Josh Figueroa, and Ragini Sreenath, NSG-PGL [Ex. 7.0](https://www.icc.illinois.gov/docket/P2026-0065/documents/375328/files/658043.pdf) at 8–9.
...
The `r comma(n_res_blocks)` in-scope residential blocks contain `r comma(n_res_units)` residential units, comprising `r comma(n_sf)` single-family parcels and `r comma(n_mf)` multi-family units. Approximately `r round(n_miles)` miles of mains run beneath these blocks. That is roughly `r percent(pct_miles_in_scope, accuracy = 0.1)` of the approximately `r comma(prp_total_miles)` miles of CI/DI mains PGL plans to retire system-wide under the PRP.[^prp-scope]

@fig-blocks-by-type shows the spatial distribution of in-scope and excluded blocks within the project areas I mapped. @fig-miles-of-prp-3seg shows how the approximately `r comma(prp_total_miles)` system-wide PRP miles break down across the in-scope residential blocks, the excluded mixed-zoning blocks, and the remaining miles outside the project areas I mapped. @fig-prp-spending-by-year-all shows the estimated total planned PRP capital spending across the project areas by project start year, broken out by block type — my analysis focuses on the fully residential portion (the dark bars).
...
[^smp-replacement-cost]: Peoples Gas, [*System Modernization Plan* at 42](https://www.documentcloud.org/documents/26371764-p2024-0081-pgl-ex-20-220-part-1-47633300/#document/p42/a2775095) (Docket 24-0081, replacement).
...
**A.** The analysis excludes seven categories of cost, benefit, and feasibility consideration.
...
**A.** I evaluate the in-scope residential blocks under three different perspectives on cost-effectiveness.
...
The three perspectives are not competing hypotheses. They are progressively less restrictive comparisons against PGL's planned PRP, each adding an element of realism to what came before.
...
Approximately `r round(sample_total_pipe, 2)` miles of CI/DI gas main run along the block, as labeled in the figure above. Because gas mains in this corridor serve blocks on both sides of the street, the analysis allocates the pipe along this block proportionally — by each block's share of the construction-polygon perimeter — yielding approximately `r round(sample_attributed_pipe, 3)` miles of pipe attributed to this block for cost-accounting purposes.
...
[^bdc-future-gas-il]: Building Decarbonization Coalition, *The Future of Gas in Illinois* (2024). *TK: confirm full citation, exact title, and pin-cite page(s).*
...
3. pproximately a third of the homes on the block are electrified one at a time at the end of the decade.
...
**A.** On PGL's rate base, yes — coordinated TE on the in-scope residential portfolio would cost ratepayers approximately `r dollar(total_npa_cost_portfolio - total_prp_cost_portfolio, scale = 1e-6, suffix = " million", accuracy = 1)` more than pipe replacement alone. But that figure does not represent the new cost to society. It represents the transfer of an existing customer-side expenditure (the electrification of individual homes) onto the rate base, and the bridging mechanisms available to the Commission can shift a meaningful share of it back off.

To see why, it helps to compare the two scenarios on different ledgers: what PGL's ratepayers pay through their gas bills, and what individual households pay out of pocket and through federal incentives.

Under PRP-plus-scattershot, ratepayers fund approximately `r dollar(total_prp_cost_portfolio, scale = 1e-6, suffix = " million", accuracy = 1)` of pipe replacement and households fund approximately `r dollar(total_ss_npv_portfolio, scale = 1e-6, suffix = " million", accuracy = 1)` of their own scattershot electrification — approximately `r dollar(total_prp_ss_portfolio, scale = 1e-6, suffix = " million", accuracy = 1)` in total.

Under coordinated TE, ratepayers fund approximately `r dollar(total_npa_cost_portfolio, scale = 1e-6, suffix = " million", accuracy = 1)` for the full coordinated electrification, and households no longer pay separately because the utility is doing the work. Total societal cost falls by approximately `r dollar(portfolio_savings_vs_prp_ss, scale = 1e-6, suffix = " million", accuracy = 1)`.

The two ledgers move in opposite directions. Ratepayers pay approximately `r dollar(total_npa_cost_portfolio - total_prp_cost_portfolio, scale = 1e-6, suffix = " million", accuracy = 1)` more than they would under PRP alone. Households pay approximately `r dollar(total_ss_npv_portfolio, scale = 1e-6, suffix = " million", accuracy = 1)` less than they would under scattershot. The difference between those two movements is the approximately `r dollar(portfolio_savings_vs_prp_ss, scale = 1e-6, suffix = " million", accuracy = 1)` of net societal savings. The rate-base gap is not new spending. It is most of the customer-side spending on electrification that coordination removes, routed through the utility to the customer's gas bills.
...
<!-- QUESTION FOR JPV AND ALEX

This Q4 uses five portfolio totals (total_prp_cost_portfolio,
total_npa_cost_portfolio, total_ss_npv_portfolio, total_prp_ss_portfolio,
portfolio_savings_vs_prp_ss) that live in working_draft_testimony.qmd's
setup chunk as locals rather than in analysis.qmd as report-variables.
The prose renders correctly today either way.

Leave as-is or promote to analysis.qmd? Alex wrote it so I just wanted to check. -->
...
- Track 1 (updated current process): continue risk-first replacement on the truly highest-risk pipe — those segments that genuinely cannot wait two to three years for a coordinated block-level electrification project to organize. This is a meaninfully smaller number than the number of replacements per year PGL currently performs. *TK: confirm with CUB how this would actually be evaluated.*
...
The two tracks together handle real safety risk and capture the cost-effective TE that the PRP investigation in [Docket 24-0081](https://www.icc.illinois.gov/docket/P2024-0081/documents/361580/files/633326.pdf) was already looking for. Neither track alone does both.

This is the strategic destination. It is not what I am asking the Commission to order in this case — see §X. What I am asking the Commission to order is the analysis that would tell PGL, the Commission, and stakeholders whether Track 2 is feasible and at what scale.
...
(iii) **Capability requirements.** What would PGL need to do organizationally to decomission at this scale? For instacne, what engineering planning, contractor staging, customer engagement, and procurement capabilities would PGL need to execute a Track 2 program at the cadence implied by the 2035 retirement deadline,[^smp-final-order] and what is the gap between those capabilities and what PGL operates today? Decommissioning at this scale is not a planning exercise alone. It requires signing up entire blocks of households at once rather than one customer at a time, coordinating gas decommissioning with electrification and ComEd grid upgrades on a single project schedule, and procuring electrification equipment in volume. None of these are capabilities PGL operates today, and the study should report what it would take to build them.
...
6. It is consistent with the Commission's own stated direction in opening the NPA workshops in [Docket 24-0081](https://www.icc.illinois.gov/docket/P2024-0081/documents/361580/files/633326.pdf). The Commission already asked the question. This ask just operationalizes it.
...
**Q.** What is simplest version of your ask?
...
1. Hydraulic feasibility is unmodeled. This testimony does not estimate the share of the in-scope mileage that could be physically decommissioned under a coordinated program. That is exactly what the Part 1 study in §X is designed to answer.
...
**A.** No. My silence with respect to any position taken by the Companies or any other party in this case should not be construed as agreement with that position. (Re-affirmed from §II.)
