...
```{python}
# Calculate the percent of polygons where lpp_over_npa > 1 and com_ind_flag == False
mask = (pg_summary["lpp_over_npa"] > 1) * (pg_summary["comm_ind_flag"] == False)
percent_gt_1_and_comm_ind_false = (mask.sum() / len(pg_summary)) * 100
print(f"{percent_gt_1_and_comm_ind_false:.1f}% of construction polygons have lpp_over_npa > 1 and no commercial or industrial parcels")
```

## Industrial only

```{python}
# Calculate the percent of polygons where lpp_over_npa > 1 and ind_flag == False
...
```{python}
# Calculate the percent of polygons where npa_over_lpp > 1 (with _dc suffix)
num_gt_1_dc = (pg_summary['lpp_over_npa_w_decomm'] > 1).sum()
total_polygons_dc = len(pg_summary)
percent_gt_1_dc = (num_gt_1_dc / total_polygons_dc) * 100

print(f"{percent_gt_1_dc:.1f}% of construction polygons have npa_over_lpp > 1")
...
```{python}
# Calculate the percent of polygons where lpp_over_npa > 1 and com_ind_flag == False (with _dc suffix)
...
print(f"{percent_gt_1_and_comm_ind_false_dc:.1f}% of construction polygons have lpp_over_npa > 1 and no commercial or industrial parcels")
```
...
```{python}
# Calculate the percent of polygons where lpp_over_npa > 1 and ind_flag == False (with _dc suffix)
...
print(f"{percent_gt_1_and_ind_false_dc:.1f}% of construction polygons have lpp_over_npa > 1 and ind_flag == False")
```

# Print summary statistics for construction polygons no decommissioning costs
```{python}
print(f"{percent_gt_1:.1f}% of construction polygons have npa_over_lpp > 1")
print(f"{percent_comm_ind_false:.1f}% of construction polygons have comm_ind_flag == False")
print(f"{percent_gt_1_and_comm_ind_false:.1f}% of construction polygons have lpp_over_npa > 1 and no commercial or industrial parcels")
...
# Print summary statistics for construction polygons with decommissioning costs
```{python}
print(f"{percent_gt_1_dc:.1f}% of construction polygons have npa_over_lpp > 1")
print(f"{percent_gt_1_and_comm_ind_false_dc:.1f}% of construction polygons have lpp_over_npa > 1 and no commercial or industrial parcels")
print(f"{percent_gt_1_and_ind_false_dc:.1f}% of construction polygons have lpp_over_npa > 1 and ind_flag == False")
...
m = plot_pg_polygons_map(pg_summary, 'street_miles', discrete=False, cmap='YlOrRd', title = "Street miles in construction polygons")
m
```

```{python}
# Create interactive map showing count of  street miles < 1mi
m = plot_pg_polygons_map(
    pg_summary[pg_summary["street_miles"] <0.1], "street_miles", discrete=False, cmap="YlOrRd", title="Street miles in construction polygons"
)
m
```
...
m = plot_pg_polygons_map(pg_summary, 'ind_flag', discrete=True, color_dict={
                         True: '#ff0000', False: '#0000ff'}, title = "Construction polygons with 1 or more industrial parcels")
m
```
