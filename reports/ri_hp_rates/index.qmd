...
- Unless otherwise noted, the analysis only evaluates households served by **investor-owned electric utilities**. Municipal utilities are out of scope.
...
- The **heating season**, which is also called "**winter**" throughout the report, is defined as October to March for Rhode Island Energy.
...
Despite this dramatic increase in the electric bill, the overall household energy bill drops by `r (1 - (post_hp_total_bill_gas / pre_hp_total_bill_gas)) |> scales::percent()` to `r post_hp_total_bill_gas |> scales::dollar(accuracy = 1)`.
...
The customer and delivery charges are what they pay for the poles and wires that deliver this power to their home. Unlike the customer charge, which stays the same, the delivery charge is a volumetric charge that, like the supply charge, grows in proportion to household electricity consumption.
...
As expected, the same seasonal pattern holds, with wintertime gas use now converted to electricity. During the six months of the heating season, the heat pump home has lower bills every month but January, which drives the annual energy bill savings compared to the natural gas home.
...
The results for fuel oil customers are similar to the gas customers: fossil fuel consumption is nearly eliminated—except for a residual amount used as backup heating (in this example), while the electricity bills jumps by `r (post_hp_total_elec_bill_fuel / pre_hp_total_elec_bill_fuel) |> round(1)`x (@fig-stacked_bar_median_before_after_fuel_oil).

:::{.column-page-inset-right}
{{< embed notebooks/analysis.qmd#fig-stacked_bar_median_before_after_fuel_oil >}}
:::

The switch to heat pump results in an annual energy bill savings of `r (pre_hp_total_bill_fuel - post_hp_total_bill_fuel) |> scales::dollar(accuracy = 1)`.
...
As expected, the savings are mostly during the heating season, with no significant savings in the summer months.
...
80% of homes would cut their wintertime energy bills by switching to heat pumps. That savings would be modest, however: only 12% of homes would see a bill decrease of more than $1,000 per year.
...
That said, no household would save or lose more than $500 per summer. Why is the spread of bill impacts so much tighter in the summer than in the winter? Because, as we saw earlier, cooling bills are much smaller than heating bills, and thus less impactful to annual household budgets.
...
Fuel oil is more expensive than natural gas, so a larger share of the households would save by switching, and the savings would be larger on average.
...
Why would (`r annual_change_stats_resistance_baseline_hp_high |> filter(bill_category == "pct_that_lose") |> pull(percentage)`%) of these customers see an a small increase in their bills?

We're looking at annual bills here, so these are households that are gaining cooling from the first time, and the increase in their summer bill just manages to offset the savings on their winter bill.
...
Around a fifth (`r annual_change_stats_small_multifamily_baseline_hp_high |> filter(bill_category == "pct_that_lose") |> pull(percentage)`%) of homes in 2-4 unit multifamily buildings would see bill increases, but virtually none (`r annual_change_stats_small_multifamily_baseline_hp_high |> filter(bill_category == "large_increase") |> pull(percentage)`%) would face an increase of more than $500 per year.

These findings are consistent with what we've seen in other states.
...
Homes in larger multifamily buildings would fare very similarly: roughly one-third (`r annual_change_stats_small_multifamily_baseline_hp_high |> filter(bill_category == "pct_that_lose") |> pull(percentage)`%) would see bill increases. Hardly any would see bill increases of more than $250 per year, an amount they'd be unlikely to notice.
...
Under baseline rates, about 50% of homes would save by switching to low efficiency heat pumps, 75% of homes would save by switching to mid efficiency, and 90% of homes would save by switching to best efficiency.
