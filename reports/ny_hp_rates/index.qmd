...
And here is what it would do to the cross-subsidy picture.

::::: {.column-screen-inset-right}
{{< embed notebooks/cost_of_service_by_subclass.qmd#tbl-eh-seasonal-sw-avg-pc >}}
:::::

- That's fairness, what about bills?
...
Yes. While the bill savings would be slightly reduced compared to switching under the heat pump-only seasonal rate, `{python} pct(v.pct_natgas_save_ehrate_lmi40)` would save under the electric heating seasonal rate, compared to `{python} pct(v.pct_natgas_save_default_lmi40)` under the default rate. And the share losing more than $1,000/yr would change from `{python} pct(v.pct_natgas_lose_1k_default_lmi40)` under the seasonal rate to `{python} pct(v.pct_natgas_lose_1k_ehrate_lmi40)` under the electric heating rate.
...
All households that currently heat with electric resistance would save by switching from the default to the electric heating rate, and their savings would average TK a year, or `{python} dollar(abs(v.mean_monthly_er_stay_delta_ehrate), 2)` a month.
...
Under default rates, `{python} pct(v.pct_er_switch_save_default)` of ER homes save by upgrading equipment to HP, and they would save an average `{python} dollar(abs(v.mean_er_switch_delta_default))`/yr. (The fact that not 100% of ER customers save annually reflects the fact that some get cooling).

Unsurprisingly, as we saw earlier in the report, removing the overpayment for HP customers via the HP-only seasonal rate makes those savings even bigger: `{python} pct(v.pct_er_switch_save_default_to_hprate)` of ER heating customers would save, and they would save an average `{python} dollar(abs(v.mean_er_switch_delta_default_to_hprate))`/yr.

If those ER had access to the EH rate before and after they switched, however, the number that would save by switching would drop to `{python} pct(v.pct_er_switch_save_ehrate_to_hprate)`, and the average savings would be `{python} dollar(abs(v.mean_er_switch_delta_ehrate_to_hprate))` per year. These savings would still amount to TK per month during the heating months, and many of these customers would gain cooling for the first time, so there is still significant incentive to switch to heat pumps, and upfront costs remain the biggest barrier.

- Ok so an EH rate would remove the cross-subsidy for both HP and ER customers, without ruining the incentive of ER to switch to heat pumps. But by removing MORE overpayements, it would have a bigger impact on the fossil heating electric customers left on the defautl rate, if all HP + ERcustomers were enrolled. How big would the impact be?
...
- Non-ER, non-HP homes (gas, oil, propane) under the recalibrated flat rate: average bill change of `{python} dollar(v.mean_monthly_fossil_delta_ehrate, 2)`/month, with `{python} pct(v.pct_fossil_monthly_under_5_ehrate)` seeing a monthly change of less than $5 and TK seeing am onthly change of less than $15 a month.
