...
smi_tier_label_colors = c(
  ...
  "Moderate Income (60–100% SMI)" = "#7D9D81",       # medium dusty green
...
### Filter Housing Units for analysis
```{r}
#| label: filter_housing_units_for_analysis

cat("Rows in housing_units (all of RI):", nrow(housing_units |> filter(upgrade == 0)), "\n")

housing_units <- housing_units |>
  filter(in.sqft <= 5000)
cat("Rows in housing_units (all of RI, <= 5000 sqft):", nrow(housing_units |> filter(upgrade == 0)), "\n")


# Filter to weather file city of Pawtucket  Awos
housing_units <- housing_units |>
  filter(in.weather_file_city == "Pawtucket  Awos")
cat("Rows in housing_units (all of RI, <= 5000 sqft, Pawtucket  Awos):", nrow(housing_units |> filter(upgrade == 0)), "\n")
...
cat("Rows in housing_units (all of RI, <= 5000 sqft, Pawtucket  Awos, not vacant):", nrow(housing_units |> filter(upgrade == 0)), "\n")

n_non_vacant_homes <- housing_units |> filter(upgrade == 0) |> filter(occupants_group != "Vacant") |> nrow()
```

### Count by Baseline Heating Type
```{r}
#| label: count_by_baseline_heating_type

# Filter to baseline homes (upgrade == 0) and give counts by baseline_heating_type
counts_baseline_heating_type <- housing_units |>
  filter(upgrade == 0) |>
  count(baseline_heating_type) |>
  arrange(desc(n))

#View(counts_baseline_heating_type)
```
...
if (file.exists(path_monthly_consumption)) {
  ...
  monthly_consumption <- monthly_consumption |>
    filter(bldg_id %in% (housing_units |> filter(upgrade == 0) |> pull(bldg_id)))
...
```{r}
#| label: heating_bill_calculations
...
# ------------------------------------------------------------------------------
# Section: Device Efficiency
# ------------------------------------------------------------------------------
## Plot: fig-plot_heating_bill_change_vs_afue
```{r}
#| label: fig-plot_heating_bill_change_vs_afue
#| fig-cap: "Median Annual Total Bill Change by Baseline Gas Heating Efficiency (AFUE Value)"

annual_changes_y  <- annual_changes_heating |>
  filter(afue_value != 'NA') |>
  filter(upgrade == 2) |>
  filter(version_elec == "baseline") |>
  filter(baseline_heating_type == "Natural Gas")

#View(annual_changes_y)s
...
plot_bill_change_vs_afue <- ggplot(annual_changes_x, aes(x = afue_value/100, y = median_annual_change)) +
  #shade light green above y=0
  theme(panel.grid.major = element_line(color = "black"),
        panel.grid.minor = element_line(color = "grey93")) +
  geom_rect(aes(xmin = -Inf, xmax = Inf, ymin = 0, ymax = Inf), fill = savings_colors["light_red"], alpha = 0.3) +
  #shade light red below y=0
  geom_rect(aes(xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = 0), fill = savings_colors["light_green"], alpha = 0.3) +
  ...
  scale_y_continuous(limits = c(-100, 1000), breaks = seq(-1000, 1000, by = 100), labels = scales::dollar_format()) +
  geom_point(size=2, color = sb_colors["midnight"]) +
  geom_line(size=1, color = sb_colors["midnight"]) +

  # use ggrepel to write the counts on the points
  # add a small offset to the counts
  ggrepel::geom_text_repel(aes(label = counts), size = 4, color = sb_colors["sky"], nudge_y = 15) +

  # add a text that says "Count of Bldg IDs" in sky color
  annotate("text", x = 0.62+0.05, y = 50, label = "Count of Bldg IDs", color = sb_colors["sky"], size = 4)

print(plot_bill_change_vs_afue)

# save the plot to cache as an image
ggsave(file.path(path_to_cache, "bill_change_vs_afue_plot.png"), plot_bill_change_vs_afue, width = 10, height = 8)

```

### Plot: fig-hist_1_annual_change_heating_baseline_hp_high
```{r}
...
### Plot: fig-hist_3_heating_bill_changes_by_afue
```{r}
#| label: fig-hist_3_heating_bill_changes_by_afue
...
plot_3hists_heating_bill_changes_by_afue <- p1 / p2 / p3 +
...
print(plot_3hists_heating_bill_changes_by_afue)
# save the plot to cache as an image
ggsave(file.path(path_to_cache, "heating_bill_changes_by_afue_plot.png"), plot_3hists_heating_bill_changes_by_afue, width = 10, height = 8)
...
### Plot: Backup Ratio vs Annual Change Total
```{r}
#| label: plot_scatter_bkup_ratio_vs_annual_change_total
#| fig-cap: "Scatter Plot: Backup Ratio vs Annual Change Total"
...
plot_scatter_bkup_ratio_vs_annual_change_total <- ggplot(heating_change_vs_bkup, aes(x = bkup_ratio, y = annual_change_total)) +
  geom_point(alpha = 0.2) +
  labs(x = "Backup Ratio", y = "Annual Change Total")
print(plot_scatter_bkup_ratio_vs_annual_change_total)
...
```{r}
#| label: calculate_peak_loads

library(tidyverse)

housing_units_peak_loads <- housing_units |>
  select(bldg_id, upgrade, out.electricity.winter.peak.kw, out.electricity.summer.peak.kw, out.electricity.total.energy_consumption.kwh) |>
  rename(
    winter_peak_load = out.electricity.winter.peak.kw,
    summer_peak_load = out.electricity.summer.peak.kw,
    total_energy_consumption = out.electricity.total.energy_consumption.kwh
  ) |>
  mutate(
    annual_peak_load = pmax(winter_peak_load, summer_peak_load),
    winter_peak_pct = winter_peak_load/(summer_peak_load),
    load_factor = total_energy_consumption / (annual_peak_load * 8760)
  ) |>
  left_join(
    # Get the baseline peak for each building (upgrade == 0)
    housing_units |>
      filter(upgrade == 0) |>
      select(bldg_id, out.electricity.winter.peak.kw, out.electricity.summer.peak.kw, out.electricity.total.energy_consumption.kwh) |>
      mutate(
        baseline_peak_load = pmax(out.electricity.winter.peak.kw, out.electricity.summer.peak.kw),
        baseline_total_energy_consumption = out.electricity.total.energy_consumption.kwh
      ) |>
      select(bldg_id, baseline_peak_load, baseline_total_energy_consumption),
    by = "bldg_id"
  ) |>
  mutate(
    annual_peak_load_pct = annual_peak_load / baseline_peak_load,
    consumption_growth = total_energy_consumption / baseline_total_energy_consumption
  )

stats_peak_loads <- housing_units_peak_loads |>
  group_by(upgrade) |>
  summarize(
    mean_winter_peak_pct = mean(winter_peak_pct),
    median_winter_peak_pct = median(winter_peak_pct),
    mean_annual_peak_load_pct = mean(annual_peak_load_pct),
    median_annual_peak_load_pct = median(annual_peak_load_pct),
    mean_load_factor = mean(load_factor),
    median_load_factor = median(load_factor),
    mean_total_energy_consumption = mean(total_energy_consumption),
    median_total_energy_consumption = median(total_energy_consumption),
    mean_consumption_growth = mean(consumption_growth),
    median_consumption_growth = median(consumption_growth),
  )
print(stats_peak_loads)

stats_winter_peaking_pct <- housing_units_peak_loads |>
  mutate(winter_peaking = winter_peak_pct > 1.0) |>
  group_by(upgrade) |>
  summarize(
    pct_winter_peaking = sum(winter_peaking == TRUE) / n()
  )
#Print(stats_winter_peaking_pct)
```

```{r}
#| label: load_factor_histogram
#| fig-cap: "Histogram of Load Factor"
# Plot the distribution of load_factor for upgrade = 0 and upgrade = 3

median_load_factor_0 <- stats_peak_loads |> filter(upgrade == 0) |> pull(median_load_factor)
median_load_factor_3 <- stats_peak_loads |> filter(upgrade == 3) |> pull(median_load_factor)

load_factor_histogram <- ggplot(
  housing_units_peak_loads %>% filter(upgrade %in% c(0, 3)),
  aes(x = load_factor, fill = as.factor(upgrade))
) +
  geom_histogram(binwidth = 0.01, alpha = 0.4, position = "identity") +
  geom_vline(xintercept = median_load_factor_0, color = "red", linetype = "dashed") +
  geom_text(x = median_load_factor_0+0.005, y = 90, label = paste("Median Load Factor (Baseline):", round(median_load_factor_0, 3)), color = "red", vjust = -1, hjust = 0) +
  geom_vline(xintercept = median_load_factor_3, color = "blue", linetype = "dashed") +
  geom_text(x = median_load_factor_3+0.005, y = 100, label = paste("Median Load Factor (High-eff. ccASHP):", round(median_load_factor_3, 3)), color = "blue", vjust = -1, hjust = 0) +
  labs(
    x = "Load Factor",
    y = "Number of Homes",
    fill = "Upgrade",
    title = "Load Factor Distribution in Rhode Island by Upgrade (2024.2)"
  ) +
  scale_fill_brewer(palette = "Set1", labels = c("0" = "Baseline (u0)", "3" = "High-eff. ccASHP (u3)"))

print(load_factor_histogram)
# save
ggsave(file.path(path_to_cache, "load_factor_histogram.png"), load_factor_histogram, width = 10, height = 8)

```


### Scatter Plot: Winter vs Summer Peak Loads
```{r}
#| label: fig-plot_scatter_winter_vs_summer_peak_loads
#| fig-cap: "Winter vs Summer Peak Loads"
library(tidyverse)

housing_units_peak_loads <- housing_units |>
  select(bldg_id, upgrade, out.electricity.winter.peak.kw, out.electricity.summer.peak.kw) |>
  rename(
    winter_peak_load = out.electricity.winter.peak.kw,
    summer_peak_load = out.electricity.summer.peak.kw
  ) |>
  ...
  left_join(
    ...
    housing_units |>
      filter(upgrade == 0) |>
      select(bldg_id, out.electricity.winter.peak.kw, out.electricity.summer.peak.kw) |>
      mutate(
        baseline_peak_load = pmax(out.electricity.winter.peak.kw, out.electricity.summer.peak.kw)
      ) |>
      select(bldg_id, baseline_peak_load),
    by = "bldg_id"
  ) |>
  mutate(
    annual_peak_load_pct = annual_peak_load / baseline_peak_load
...
plot_winter_vs_summer_peak_loads <- ggplot(
  housing_units_peak_loads,
  aes(x = summer_peak_load, y = winter_peak_load, color = as.factor(upgrade))
) +
  geom_point() +
  labs(
    x = "Summer Peak Load (kW)",
    y = "Winter Peak Load (kW)",
    color = "Upgrade"
  ) +
  scale_x_continuous(limits = c(0, 60), labels = scales::comma) +
  scale_y_continuous(limits = c(0, 60), labels = scales::comma) +
  scale_color_brewer(palette = "Set1")
print(plot_winter_vs_summer_peak_loads)

```

### fig-hist_4_winter_peak_load_pct_by_upgrade
```{r}
#| label: fig-hist_4_winter_peak_load_pct_by_upgrade
#| fig-cap: "Histograms: Winter vs Summer Peak Loads"

y_max <- 55

# Upgrade 0
hist_winter_peak_load_pct_hist_upgrade0 <- ggplot(
  housing_units_peak_loads |> filter(upgrade==0), aes(x = winter_peak_pct)
) +
  ...
  scale_x_continuous(limits = c(0, 5), breaks = seq(0, 5, by = 0.5), labels = scales::percent_format(accuracy = 1)) +
  scale_y_continuous(limits = c(0, 160), labels = scales::comma)

# Upgrade 1
hist_winter_peak_load_pct_hist_upgrade1 <- ggplot(
  housing_units_peak_loads |> filter(upgrade==1), aes(x = winter_peak_pct)
) +
  geom_histogram(binwidth = 0.05, fill = "steelblue", color = "white") +
  labs(x = NULL, y = NULL, title = "Upgrade 1") +
  scale_x_continuous(limits = c(0, 5), breaks = seq(0, 5, by = 0.5), labels = scales::percent_format
  (accuracy = 1)) +
  scale_y_continuous(limits = c(0, y_max), breaks = seq(0, y_max, by = 25), labels = scales::comma)


# Upgrade 2
hist_winter_peak_load_pct_hist_upgrade2 <- ggplot(
  housing_units_peak_loads |> filter(upgrade==2), aes(x = winter_peak_pct)
) +
  geom_histogram(binwidth = 0.05, fill = "steelblue", color = "white") +
  labs(x = NULL, y = NULL, title = "Upgrade 2") +
  scale_x_continuous(limits = c(0, 5), breaks = seq(0, 5, by = 0.5), labels = scales::percent_format(accuracy = 1)) +
  scale_y_continuous(limits = c(0, y_max), breaks = seq(0, y_max, by = 25), labels = scales::comma)

# Upgrade 3
hist_winter_peak_load_pct_hist_upgrade3 <- ggplot(
  housing_units_peak_loads |> filter(upgrade==3), aes(x = winter_peak_pct)
) +
  geom_histogram(binwidth = 0.05, fill = "steelblue", color = "white") +
  labs(x = "Winter Peak/Summer Peak", y = NULL, title = "Upgrade 3") +
  scale_x_continuous(limits = c(0, 5), breaks = seq(0, 5, by = 0.5), labels = scales::percent_format(accuracy = 1)) +
  scale_y_continuous(limits = c(0, y_max), breaks = seq(0, y_max, by = 25), labels = scales::comma)

# Combine all the histograms into a single plot
plot_winter_vs_summer_peak_loads_histograms <- hist_winter_peak_load_pct_hist_upgrade0 / hist_winter_peak_load_pct_hist_upgrade1 / hist_winter_peak_load_pct_hist_upgrade2 / hist_winter_peak_load_pct_hist_upgrade3 +
  plot_layout(ncol = 1) +
  plot_annotation(title = "Winter vs Summer Peak Loads", subtitle = "Rhode Island")
print(plot_winter_vs_summer_peak_loads_histograms)
```

### fig-hist_3_annual_peak_growth_by_upgrade
```{r}
#| label: fig-hist_3_annual_peak_growth_by_upgrade
#| fig-cap: "Distribution of Annual Peak Load as a Percentage of Baseline Peak Load by Upgrade"

annual_peak_growth_by_upgrade <- housing_units_peak_loads |>
  filter(upgrade %in% c(1,2,3)) |>
  mutate(
    upgrade_label = case_when(
      upgrade == 1 ~ "Upgrade 1: ENERGY STAR heat pump with elec backup",
      upgrade == 2 ~ "Upgrade 2: High efficiency cold-climate air-to-air heat pump with electric backup",
      upgrade == 3 ~ "Upgrade 3: Ultra high efficiency heat pump with elec backup",
      TRUE ~ as.character(upgrade)
    )
  )

plot_annual_peak_growth_by_upgrade <- ggplot(
  annual_peak_growth_by_upgrade,
  aes(x = annual_peak_load_pct)
) +
  geom_histogram(binwidth = 0.05) +
  facet_wrap(~ upgrade_label, ncol = 1) +
  labs(
    x = "New Peak Load Relative to Current Annual Peak",
    y = "Number of Homes",
    title = "Peak Load Growth by Upgrade"
  ) +
  scale_x_continuous(limits = c(0, 5), breaks = seq(0, 5, by = 1), labels = scales::percent_format(accuracy = 1))
print(plot_annual_peak_growth_by_upgrade)
...
### Count by Weather File City
```{r}
#| label: count of homes by weather_file_city
housing_units |>
...
### Count by Heating Fuel
```{r}
#| label: count of homes by heating fuel
housing_units |>
filter(upgrade == 0) |>
filter(occupants_group != "Vacant") |>
filter(in.weather_file_city == "Pawtucket  Awos") |>
...
### plot_heating_load_change
```{r}
#| label: fig-plot_heating_load_change
#| fig-cap: "Change in Heating Load by Upgrade (Baseline Heating Type = Natural Gas)"

heating_load_change <- housing_units |>
filter(upgrade == 0) |>
filter(baseline_heating_type == "Natural Gas") |>
select(bldg_id, out.load.heating.energy_delivered.kbtu) |>
rename(baseline_heating_load = out.load.heating.energy_delivered.kbtu) |>
right_join(housing_units |> select(bldg_id, upgrade, out.load.heating.energy_delivered.kbtu) |> rename(new_heating_load = out.load.heating.energy_delivered.kbtu), by = c("bldg_id")) |>
mutate(
  heating_load_change = new_heating_load - baseline_heating_load,
  heating_load_change_pct = heating_load_change / baseline_heating_load
  ) |>
select(bldg_id, upgrade, baseline_heating_load, new_heating_load, heating_load_change, heating_load_change_pct)


# plot three stacked histograms of the heating_load_change_pct by upgrade
# Manually set titles for each subplot by creating a label column and then faceting by that
heating_load_change_labeled <- heating_load_change |>
  filter(upgrade %in% c(1,2,3)) |>
  mutate(
    upgrade_label = case_when(
      upgrade == 1 ~ "Upgrade 1: ENERGY STAR heat pump with elec backup",
...
plot_heating_load_change <- ggplot(heating_load_change_labeled, aes(x = heating_load_change_pct)) +
  geom_histogram(binwidth = 0.01) +
  facet_wrap(~ upgrade_label, nrow = 3) +
  labs(title = "Change in Heating Load by Upgrade (Baseline Heating Type: Natural Gas)",
       x = "Change in Heating Load (%)",
       y = "# of Homes"
       ) +
  scale_x_continuous(labels = scales::percent, breaks = seq(-0.3, 0.7, by = 0.1)) +
  scale_y_continuous(labels = scales::comma)


print(plot_heating_load_change)
```

### plot_cooling_load_change
```{r}
#| label: fig-plot_cooling_load_change
#| fig-cap: "Change in Cooling Load by Upgrade"

cooling_load_change <- housing_units |>
filter(upgrade == 0) |>
select(bldg_id, out.load.cooling.energy_delivered.kbtu) |>
rename(baseline_cooling_load = out.load.cooling.energy_delivered.kbtu) |>
right_join(housing_units |> select(bldg_id, upgrade, out.load.cooling.energy_delivered.kbtu) |> rename(new_cooling_load = out.load.cooling.energy_delivered.kbtu), by = c("bldg_id")) |>
mutate(
  cooling_load_change = new_cooling_load - baseline_cooling_load,
  cooling_load_change_pct = cooling_load_change / baseline_cooling_load
  ) |>
select(bldg_id, upgrade, baseline_cooling_load, new_cooling_load, cooling_load_change, cooling_load_change_pct)


# plot three stacked histograms of the cooling_load_change_pct by upgrade
# Manually set titles for each subplot by creating a label column and then faceting by that
cooling_load_change_labeled <- cooling_load_change |>
  filter(upgrade %in% c(1,2,3)) |>
  mutate(
    upgrade_label = case_when(
      upgrade == 1 ~ "Upgrade 1: ENERGY STAR heat pump with elec backup",
...
plot_cooling_load_change <- ggplot(cooling_load_change_labeled, aes(x = cooling_load_change_pct)) +
  geom_histogram(binwidth = 0.01) +
  facet_wrap(~ upgrade_label, nrow = 3) +
  labs(title = "Change in Cooling Load by Upgrade",
       x = "Change in Cooling Load (%)",
       y = "# of Homes"
       ) +
  scale_x_continuous(labels = scales::percent, breaks = seq(0, 8, by = 0.5)) +
  scale_y_continuous(labels = scales::comma)


print(plot_cooling_load_change)
```
