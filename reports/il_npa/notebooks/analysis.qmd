...
pg_summary <- pg_summary |>
  mutate(
    ...
    elec_sf_cost = (sf_parcels * cost_elec_sf) + (grid_upgrade_cost_hh * sf_parcels),
    elec_mf_cost = (mf_units * cost_elec_mf) + (grid_upgrade_cost_hh * mf_units),
...
pg_summary <- pg_summary |>
  mutate(
    ...
    scattershot_npv = scattershot_annual_cost * sum(1 / (1 + DISCOUNT_RATE)^(0:(TIME_PERIOD-1))),
    ...
    comp_cost_discrete_res_only_ss = comp_cost_discrete_ss,
    comp_cost_discrete_ss = if_else(ind_flag, "not eligible", comp_cost_discrete_ss),
    comp_cost_discrete_res_only_ss = if_else(comm_ind_flag, "not eligible", comp_cost_discrete_res_only_ss)
  )
```
