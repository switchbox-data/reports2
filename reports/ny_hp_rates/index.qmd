...
[^resstock-cite]: Default national sample weights represent ~252 real-world homes each. [@nrel_EndUseLoadProfiles_2024].
...
[^cite-tariff-gas]: Filed gas tariffs are committed in the [RDP](https://github.com/switchbox-data/rate-design-platform/tree/main/rate_design/hp_rates/ny/config/tariffs/gas).
...
All bill comparisons are include Low-income discounts for 40 percent of elidible households for natural gas and electric bills. See @sec-methods-lmi. Results are computed building-by-building, aggregated using ResStock sample weights, and broken out by baseline heating fuel type (natural gas, oil/propane, electric resistance).
...
For single-zone utilities, we use that zone's hourly LBMP directly. For multi-zone utilities, we compute a load-weighted average: $\text{LBMP}_h = \sum_z (\text{LBMP}_{z,h} \times L_{z,h}) / \sum_z L_{z,h}$, where the zonal load shares come from NYISO zone-level hourly demand mapped to utility territories.
...
[^cite-lbmp]: NYISO real-time 5-minute LBMPs aggregated to hourly arithmetic means per zone; multi-zone utilities are blended via the load-weighted formula above. Output is normalized to the CAIRO 8,760-hour calendar (December 31 dropped in leap years) with small gaps linearly interpolated. No transmission-loss gross-up and no inflation adjustment are applied. Hourly energy MC signals are built by the [RDP](https://github.com/switchbox-data/rate-design-platform/blob/main/utils/pre/marginal_costs/generate_supply_energy_mc.py).

**Generation capacity.** NYISO operates an Installed Capacity (ICAP) market with monthly spot auctions. Unlike ISO-NE's single forward auction, NYISO prices capacity by **locality** — geographic zones with different levels of transmission constraint. Four auction localities clear at different prices: Rest of State (NYCA-priced; the ROS capacity obligation covers zones A–F, but peak hours are identified from the full statewide load, zones A–K), Lower Hudson Valley (auction locality GHIJ, zones G–J inclusive of NYC; labeled "LHV" internally), New York City (NYC, zone J), and Long Island (LI, zone K). Downstate utilities pay higher capacity prices because their constrained transmission paths require local generation. Only Spot auction clearances are ingested; Strip and Monthly auctions are excluded. Published ICAP $/kW-month clearing prices are used directly, with no UCAP derating applied.
...
[^cite-icap]: NYISO ICAP monthly Spot auction clearing prices by locality. Auction-locality-to-nested-footprint mappings and utility capacity-obligation weights are sourced from the NYISO ICAP Manual (Table 7-5) via the [RDP](https://github.com/switchbox-data/rate-design-platform/blob/main/data/nyiso/zone_mapping/generate_zone_mapping_csv.py). Hourly capacity MC signals are built in the [RDP](https://github.com/switchbox-data/rate-design-platform/blob/main/utils/pre/marginal_costs/generate_supply_capacity_mc.py).
...
1. **Inputs.** Project-level NYISO Public Policy Transmission Planning and Competitive System Planning Process (CSPP) benefit studies, together with the NiMo MCOS bulk-TX entry, are collected as pipeline inputs. Seven input scenarios — AC Primary, the two Addendum Optimizer variants (Existing Localities and G-J Elimination), the two Addendum MMU variants (Baseline and CES+Ret, merged), LI Export (Policy), and NiMo MCOS (Bulk TX) — map to six distinct constraint groups.
...
$$w_h = \phi_s \cdot \frac{\max(L_h - T_s,\ 0)}{\sum_{h' \in \text{season } s} \max(L_{h'} - T_{s'},\ 0)}$$
where $T_s$ is season $s$'s 41st-highest load and $\phi_s = (\text{peak}_s - \tau_{\min}) / \sum_{s'} (\text{peak}_{s'} - \tau_{\min})$ with $\tau_{\min} = \min_s T_s$. Weights sum to 1.0 globally (validated to 1e-4).
...
We aggregate project-level capital from worksheet W2 of the CRA workbook using the NERA method, rather than adopting
