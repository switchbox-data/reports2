...
For single-zone utilities (RG&E, CenHud, O&R, PSEG-LI), we use that zone's hourly LBMP directly. For multi-zone utilities (Con Edison spans zones G–J; NYSEG and NiMo span zones A–F), we compute a load-weighted average: $\text{LBMP}_h = \sum_z (\text{LBMP}_{z,h} \times L_{z,h}) / \sum_z L_{z,h}$, where the zonal load shares come from NYISO zone-level hourly demand and the utility-to-zone mapping in `ny_utility_zone_mapping.csv`.

[^cite-lbmp]: NYISO real-time 5-minute LBMPs aggregated to hourly arithmetic means per zone; multi-zone utilities are blended via the load-weighted formula above. Output is normalized to the Cairo 8,760-hour calendar (February 29 dropped in leap years) with small gaps linearly interpolated. No transmission-loss gross-up and no inflation adjustment are applied. The pipeline supports a load year distinct from the price year; in the current NY runs both are 2025 (per `state.env`), so no cross-year remap occurs. Hourly energy MC signals are built by the RDP's [supply energy MC generator](https://github.com/switchbox-data/rate-design-platform/blob/main/utils/pre/marginal_costs/generate_supply_energy_mc.py).

**Generation capacity.** NYISO operates an Installed Capacity (ICAP) market with monthly spot auctions. Unlike ISO-NE's single forward auction, NYISO prices capacity by **locality** — geographic zones with different levels of transmission constraint. Four auction localities clear at different prices: Rest of State (NYCA-priced; the NYCA partition covers zones A–F), Lower Hudson Valley (auction locality GHIJ, zones G–J inclusive of NYC; labeled "LHV" internally), New York City (NYC, zone J), and Long Island (LI, zone K). Downstate utilities pay higher capacity prices because their constrained transmission paths require local generation. Only Spot auction clearances are ingested; Strip and Monthly auctions are excluded. Published ICAP `$/kW-month` clearing prices are used directly, with no UCAP derating applied.
...
- $c_w$ — capacity weight: the utility's share of its capacity obligation in each locality (e.g. Con Edison: 0.87 NYC + 0.13 LHV, from Table 7-5 of the NYISO ICAP Manual, surfaced by `generate_zone_mapping_csv.py` as `capacity_weight`)
...
[^cite-icap]: NYISO ICAP monthly Spot auction clearing prices by locality. Auction-locality-to-nested-footprint mappings and utility capacity-obligation weights are sourced from the NYISO ICAP Manual (Table 7-5) via [`generate_zone_mapping_csv.py`](https://github.com/switchbox-data/rate-design-platform/blob/main/data/nyiso/zone_mapping/generate_zone_mapping_csv.py). Hourly capacity MC signals are built by the RDP's [supply capacity MC generator](https://github.com/switchbox-data/rate-design-platform/blob/main/utils/pre/marginal_costs/generate_supply_capacity_mc.py).
...
1. **Inputs.** Project-level NYISO Public Policy Transmission Planning and Competitive System Planning Process (CSPP) benefit studies, together with the NiMo MCOS bulk-TX entry, are recorded in `ny_bulk_tx_projects.csv`. Seven input scenarios — AC Primary, the two Addendum Optimizer variants (Existing Localities and G-J Elimination), the two Addendum MMU variants (Baseline and CES+Ret, merged), LI Export (Policy), and NiMo MCOS (Bulk TX) — map to six distinct constraint groups.
2. **Derivation.** For each constraint group, `derive_tx_values.py` computes a `$/kW-yr` value via a cumulative-secant aggregation over its member projects (output: `ny_bulk_tx_constraint_groups.csv`).
3. **Paying-locality aggregation.** Each constraint group is mapped to a disjoint set of paying localities (ROS, LHV, NYC, LI) via the nested-locality scheme in `derive_tx_values.py`; per-paying-locality costs are averaged within group.
4. **Hourly allocation.** For each utility, `bulk_tx_nyiso.py` identifies System Capability Resource (SCR) hours as the top 40 load hours in each of summer and winter (80 total) within the utility's nested-locality load. Each constraint-group annual value is spread across those 80 hours using a two-level weight:
...
5. **Utility blend.** `generate_bulk_tx_mc.py` combines each utility's paying-locality streams using the utility's capacity-obligation shares from the zone mapping, yielding the utility's hourly bulk-TX `$/kW-hr`.
...
[^cite-bulk-tx]: Project-level inputs (`ny_bulk_tx_projects.csv`, a Switchbox-curated ledger) trace back to NYISO Public Policy Transmission Planning and Competitive System Planning Process (CSPP) benefit studies, plus the NiMo MCOS bulk-TX entry. Constraint-group `$/kW-yr` values are computed by [`derive_tx_values.py`](https://github.com/switchbox-data/rate-design-platform/blob/main/data/nyiso/transmission/derive_tx_values.py) and written to `ny_bulk_tx_constraint_groups.csv` (published to S3 at `s3://data.sb/nyiso/bulk_tx/`). Hourly allocation to the 80-hour SCR window with two-level weighting is implemented in [`bulk_tx_nyiso.py`](https://github.com/switchbox-data/rate-design-platform/blob/main/utils/pre/marginal_costs/bulk_tx_nyiso.py); the per-utility blend is assembled by the [bulk transmission MC generator](https://github.com/switchbox-data/rate-design-platform/blob/main/utils/pre/marginal_costs/generate_bulk_tx_mc.py). See [`context/code/marginal_costs/ny_bulk_tx_marginal_costs.md`](https://github.com/switchbox-data/rate-design-platform/blob/main/context/code/marginal_costs/ny_bulk_tx_marginal_costs.md) for end-to-end pipeline documentation.
...
[^script-rr-mid]: BAT values, per-customer economic burden, and cross-subsidy are computed by CAIRO during the run. The RDP's [subclass revenue-requirement allocator](https://github.com/switchbox-data/rate-design-platform/blob/main/utils/mid/compute_subclass_rr.py) then applies the per-customer split between HP and non-HP subclasses. BAT results are consolidated across utilities by the RDP's [master BAT builder](https://github.com/switchbox-data/rate-design-platform/blob/main/utils/post/build_master_bat.py).
...
[^script-supply-held]: From the supply tariff alone, the decomposition into a marginal-cost share and a residual share is not uniquely identified — any pair of shares that sum to the observed bill is consistent with the tariff. See the RDP's methodology note [passthrough_cost_allocation_and_rate_design.md](https://github.com/switchbox-data/rate-design-platform/blob/main/context/methods/bat_mc_residual/passthrough_cost_allocation_and_rate_design.md) for the full argument.
