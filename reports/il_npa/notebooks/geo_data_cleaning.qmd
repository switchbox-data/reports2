...
This notebook cleans and prepares the geo data for the analysis. We need to create a single geojson file that contains census block level data for:
 1. Miles of pipleline to be replaced for each block. We use street miles as a proxy. Conceptually, we want to split the cost the the pipeline between the blocks on either side of the street.
...
The assessor class codes only provide so much information about the type of parcel. Because our analysis is focused on fully residential blocks, we need to make some additional rules for identifying blocks that have no commercial or industrial parcels but are still not fully residential. Examples of this are blocks that are fully covered by a church, school, parks or railroad properties (RR). Typically if a block has only one or two parcels is is not a residential block so we drop blocks with less than 3 parcels. If all of the parcels on a block have the RR class code, we drop the block. We mapped the results of this programmatic filtering and manually identified some additional blocks to exclude by visually comparing the results to satelite imagery. This process would need to be better refined to replicate this analysis in other areas.


# Street Data
We use the Chicago Street Centerlines dataset to calculate street miles. This dataset is downloaded from the Chicago Data Portal using reports/il_npa/utils/download_chicago_streets.py.

Street data is clipped to only include segments that fall within the Peoples Gas construction polygons.

For each clipped block, we calculate the perimeter of the block in meters. We assign a scaling factor to the block equal to the ratio of the block perimeter to the total perimeter of all blocks in the construction area. We then multiply the totallength of the street segments in the construction polygon by the scaling factor to assign the street miles to each of the blocks.
...
```{python}
def read_geojson_with_s3_fallback(local_path, s3_bucket, s3_key):
    """
    ...
    -----------
    local_path : Path
        Local file path to check first
    s3_bucket : str
        S3 bucket name
    s3_key : str
...
s3_key_blocks = 'gis/pgl/pgp_blocks.geojson'
print(f"Loading city blocks...")
city_blocks = read_geojson_with_s3_fallback(
    local_path=blocks_file,
    s3_bucket='data.sb.east',
    s3_key=s3_key_blocks
)
...
if street_files:
    ...
    local_path = street_file
    # Extract date from filename for S3 key
    s3_key = f'gis/pgl/{street_file.name}'
    ...
    try:
        ...
        response = s3_client.list_objects_v2(
            Bucket="data.sb.east",
            Prefix="gis/pgl/chicago_streets_",
        )
        if "Contents" in response:
            # Sort by last modified, get most recent
            objects = sorted(response["Contents"], key=lambda x: x["LastModified"], reverse=True)
            s3_key = objects[0]["Key"]
            local_path = geo_data_dir / 'chicago_streets_NOT_FOUND.geojson'  # Won't exist, triggers S3 read
...
if street_files or (s3_key is not None and local_path is not None):
    streets = read_geojson_with_s3_fallback(
...
parcels_in_buffered = parcels_in_buffered[
    ~parcels_in_buffered["parcel_temp_id"].isin([25302, 19326, 19336, 19340, 6500, 6499])
]
...
# Only sum units from residential and mixed-use parcels (exclude commercial/industrial)
unit_counts = parcels_by_block[
    parcels_by_block['type'].isin(['residential', 'mixed-use'])
...
# Include mixed-use parcels as multi-family
...
# Include mixed-use units in multi-family units
...
# Track mixed-use units separately for identification
mixed_use_units = parcels_by_block[
    parcels_by_block['type'] == 'mixed-use'
].groupby(group_key)['building_units'].sum()
...
```{python}
# Identify geoid10 values where:
# 1. Total number of parcels assigned is <= 2, OR
# 2. All parcels on the block have assessorbldgclass == "RR"

print("\nIdentifying blocks to exclude...")

# Count parcels per geoid10
parcel_counts_by_block = parcels_by_block.groupby("geoid10").size()
print(f"  Total blocks with parcels: {len(parcel_counts_by_block):,}")

# Condition 1: Blocks with < 2 parcels
blocks_with_few_parcels = parcel_counts_by_block[parcel_counts_by_block <= 2].index.tolist()
print(f"  Blocks with <= 2 parcels: {len(blocks_with_few_parcels):,}")

# Condition 2: Blocks where all parcels have assessorbldgclass == "RR"
# First check if assessorbldgclass column exists
if "assessorbldgclass" in parcels_by_block.columns:
    # Group by geoid10 and check if all parcels in each block have assessorbldgclass == "RR"
    # A block qualifies if: all parcels have assessorbldgclass == "RR" AND no NaN values
    block_rr_check = parcels_by_block.groupby("geoid10").agg(
        {"assessorbldgclass": lambda x: (x == "RR").all() and x.notna().all()}
    )
...
excluded_geoid10 = list(set(blocks_with_few_parcels + blocks_all_rr))
print(f"\n  Total unique geoid10 to exclude: {len(excluded_geoid10):,}")

# Show breakdown
only_few_parcels = set(blocks_with_few_parcels) - set(blocks_all_rr)
only_all_rr = set(blocks_all_rr) - set(blocks_with_few_parcels)
both_conditions = set(blocks_with_few_parcels) & set(blocks_all_rr)

print(f"    Blocks with only < 2 parcels condition: {len(only_few_parcels):,}")
if "assessorbldgclass" in parcels_by_block.columns:
    print(f"    Blocks with only all-RR condition: {len(only_all_rr):,}")
    print(f"    Blocks meeting both conditions: {len(both_conditions):,}")

# now find blocks that are included in the clipped blocks dataset but found no parcel match (so they do not appear in parcel counts by block) and add them to the exlusion list

no_parcel_match_blocks = blocks_in_construction_clipped[
    ~blocks_in_construction_clipped["geoid10"].isin(parcel_counts_by_block.index)
]

# Store as a list for use in subsequent analysis -  added specific geoids that were invalid after visual inspection
excluded_geoid10_list = (
    excluded_geoid10
    + no_parcel_match_blocks["geoid10"].tolist()
    + list(
        [
            "170315001004030",
            "170315001004015",
...
if len(excluded_geoid10_list) > 0:
    print(f"\nSample of excluded geoid10 values (first 20):")
    for i, geoid10 in enumerate(sorted(excluded_geoid10_list)[:20]):
        print(f"  {geoid10}")
    if len(excluded_geoid10_list) > 20:
        print(f"  ... and {len(excluded_geoid10_list) - 20} more")

    # Also create a DataFrame for easier inspection
    excluded_geoid10_df = pd.DataFrame({"geoid10": sorted(excluded_geoid10_list)})
    print(f"\nFull list stored in 'excluded_geoid10_list' (list) and 'excluded_geoid10_df' (DataFrame)")
else:
    print("\nNo blocks meet the exclusion criteria")
```
...
blocks_in_construction_clipped = blocks_in_construction_clipped[~blocks_in_construction_clipped['geoid10'].isin(excluded_geoid10_list)]
...
summary = summary.merge(
    ...
    mixed_use_units.reset_index().rename(columns={mixed_use_units.name if mixed_use_units.name else 0: 'mixed_use_units'}),
    on='geoid10',
    how='left'
).merge(
...
city_blocks_utm = city_blocks_utm[~city_blocks_utm['geoid10'].isin(excluded_geoid10_list)]
...
int_cols = [
    ...
    'parcels_with_unit_data', 'total_residential_units', 'mf_units', 'mixed_use_units'
]
