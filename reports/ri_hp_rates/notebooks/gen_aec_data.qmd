...
from buildstock_fetch.releases import BuildstockRelease
...
_resstock_local = Path(
    os.environ.get(
        "RDP_RESSTOCK_DATA",
        Path.home() / "rate-design-platform" / "data" / "nrel" / "resstock_sb_alias",
    )
)
...
heat_pump_adoption_factor = 2#1 + (96.3-22.6)/22.6
...
```{python}
sbmeta = pl.read_parquet("s3://data.sb/nrel/resstock/res_2024_amy2018_2_sb/metadata/state=RI/upgrade=00/metadata-sb.parquet")
...
```{python}
all_years = list(range(2024, 2040))
bau_cumulative = get_adoption_forecast_num_homes(all_years, col_name="Cumulative_Base")
high_cumulative = get_adoption_forecast_num_homes(all_years, col_name="Cumulative_High")
base_start = bau_cumulative[0]
if ALTERNATIVE_SCENARIO == "hp_factor":
    hp_rates_cumulative = [
        min(base_start + heat_pump_adoption_factor * (x - base_start), total_num_homes)
        for x in bau_cumulative
    ]
elif ALTERNATIVE_SCENARIO == "rie_high":
    hp_rates_cumulative = high_cumulative

df_adoption = pl.DataFrame({
    "year": all_years,
    "RIE Base": bau_cumulative,
    "RIE High": high_cumulative,
    "HP Rates": hp_rates_cumulative,
})
```
...
plot_df = df_adoption.unpivot(
    index="year",
    on=_version_order,
    variable_name="Version",
    value_name="num_homes",
).to_pandas()

(
    ggplot(plot_df, aes(x="year", y="num_homes", color="Version"))
    + geom_line(size=1.2)
    + scale_color_manual(
    ...
    + labs(
        title="RI Cumulative Heat Pump Adoption",
        x="Year",
        y="Cumulative HP Homes",
    )
    + theme_minimal()
    + theme(
        figure_size=(10, 5),
...
```{python}
bau_hp_count_lag = np.array(get_adoption_forecast_num_homes([2025, 2026, 2027, 2028, 2029, 2030, 2031]))- np.array(get_adoption_forecast_num_homes([2024, 2025, 2026, 2027, 2028, 2029, 2030]))
...
replacement_df = (
    ...
    .filter(pl.col("failure_year").is_in([2025, 2026, 2027, 2028, 2029, 2030, 2031]), pl.col("postprocess_group.heating_type") != "heat_pump")
