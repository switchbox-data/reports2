...
In New York State, when a household that heats with natural gas switches to an air-source heat pump,[^heatpump_efficiency] its annual energy use plummets (@fig-consumption-only-state):
...
However, while switching to heat pumps cuts energy _use_, it often increases energy _costs_ (@fig-consumption-vs-bills-state).

:::{.column-body-outset-right}
{{< embed notebooks/analysis.qmd#fig-consumption-vs-bills-state >}}
:::

This is because electricity is more expensive than natural gas in New York State: while it makes up only `{python} pct(v.elec_share_of_energy_use)` of the median gas-heated household's annual energy use, it accounts for `{python} pct(v.elec_share_of_energy_bill)` of its annual energy bill.

After the home replaces its gas furnace with a heat pump, its annual [gas bill]{style="color: #a0af12; font-weight: bold"} drops by `{python} dollar(v.median_gas_drop)`.[^using_gas_for_cooking_and_hot_water] But its annual [electric bill]{style="color: #e6b400; font-weight: bold"} jumps by `{python} dollar(v.median_elec_jump)`.

The result: despite the heat pump's lower energy use, switching a home's heating fuel from natural gas to electricity causes the median annual energy bills to jump by `{python} dollar(v.median_net_increase)`.
...
This is the experience of the _median_ gas-heated household today. But New York State's building stock is diverse, so the full range of bill impacts varies widely (@fig-quadrant-bar-natgas-state).
...
Adding LMI discounts changes our original bill impact chart only slightly: households saving more $1,000 per year would grow by 3 percentage points, and households losing more than $1,000 per year would grow by 4 percentage points.
...
Low natural gas prices are a factor, but it's not the only one. As we'll see in the next section, the main culprit is (artificially) high electricity prices: outdated volumetric rates are causing heat pump customers to pay more than their fair share for the poles and wires that deliver electricity to their homes.

## HP customers are cross-subsidizing non-HP customers

### Why electric bills go up when heat pumps are installed
When a household switches to a heat pump, its annual electricity consumption goes up ~`{python} f"{v.elec_kwh_multiplier:.1f}"`×, and its annual electric bill goes up by approximately the same amount (@fig-consumption-elec-focus-state).

:::{.column-body-outset-right}
{{< embed notebooks/analysis.qmd#fig-consumption-elec-focus-state >}}
:::

To understand how this happens, we need to drill down into the bill's components (@fig-bill-components-decomposed-state).

:::{.column-body}
{{< embed notebooks/analysis.qmd#fig-bill-components-decomposed-state >}}
:::

The home pays `{python} f"{v.supply_multiplier:.1f}"`× as much for [supply]{style="color: #E69F00; font-weight: bold"}, which makes sense: the home is consuming `{python} f"{v.elec_kwh_multiplier:.1f}"`× as much electricity after installing the heat pump.

But the home also pays `{python} f"{v.delivery_multiplier:.1f}"`× as much as before for the same poles and wires that bring that electricity to their home.

This is because delivery costs are largely recovered through [volumetric charges]{style="color: #56B4E9; font-weight: bold"}, rather than [fixed charges]{style="color: #023047; font-weight: bold"}. The result: when a customer consumes more electricity, not only do they pay more for that electricity, but they also pay more for the grid itself.

Is this fair?


### Heat pump customers are overpaying for delivery
To be fair, a customer's **bill** should match the **costs** the customer imposes on the system.

When a customer doubles their electricity use after installing a heat pump, they double their **energy costs**—power plants have to burn more fuel to generate that electricity, and so on. So it is fair that their [supply bill]{style="color: #E69F00; font-weight: bold"} also doubles.

But they do not double their **network costs**, so they [delivery bill]{style="color: #56B4E9; font-weight: bold"} should not double either (@fig-cross-subsidy-example).
...
We find that today, households with heat pumps are systematically paying more than their cost-of-service for delivery statewide.

Their average cost-of-service is $X per year, compared to $Y per year for non-heat pump customers, but they pay an average of $Z.
...
LMI electrification programs present an opportunity to enroll low-income households in LMI discounts, however. So the following chart shows what happens to post-heat pump bills if all eligible LMI customers are enrolled in LMI discounts when heat pumps are installed, assuming around 40% of these customers are already receiving these discounts (in line with the current enrollment rate).
...
- Before heat pumps, `{python} pct(v.pct_burdened_current)` of natural gas-heated households making less than $50,000 a year in New York pay more than 6% of their annual income on energy bills.
...
Here is what the burdens would look like if no LMI households received discounts, before and after switching to heat pumps.
...
LMI electrification if an opportunity for enrolling low-income households in LMI discounts, however. So the following chart shows the what post-electrification burdens would look like—under default and fair rates—if all LMI households were enrolled in LMI discounts when heat pump pumps are installed.
