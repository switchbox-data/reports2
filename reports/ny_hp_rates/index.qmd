...
[^cite-tariff-elec]: Rates sourced from the [Genability (Arcadia)](https://www.arcadia.com/products/genability) utility rate database. Filed tariffs for all seven utilities are committed in the [RDP](https://github.com/switchbox-data/rate-design-platform/tree/main/rate_design/hp_rates/ny/config/tariffs/electric). A [detailed charge-by-charge taxonomy](https://github.com/switchbox-data/rate-design-platform/tree/main/rate_design/hp_rates/ny/config/rev_requirement/top-ups) of every charge on each utility's residential bill — including which charges are included in the delivery revenue requirement and which are excluded — is documented as well.
...
[^cite-tariff-gas]: Rates sourced from the [RateAcuity](https://rateacuity.com) residential gas tariff database. Filed gas tariffs are committed in the [RDP](https://github.com/switchbox-data/rate-design-platform/tree/main/rate_design/hp_rates/ny/config/tariffs/gas).
...
[^link_cairo]: Electric bills are computed by [CAIRO](https://github.com/NatLabRockies/CAIRO). The [RDP](https://github.com/switchbox-data/rate-design-platform/blob/main/utils/post/build_master_bills.py) combines the delivery and supply portions; gas bills are computed independently by the [RDP](https://github.com/switchbox-data/rate-design-platform/blob/main/utils/post/gas_bills.py).
...
[^cite-icap]: NYISO ICAP monthly Spot auction clearing prices by locality [@nyiso_ICAPSpotAuctionResults_2024]. Auction-locality-to-nested-footprint mappings and utility capacity-obligation weights are sourced from the NYISO ICAP Manual (Table 7-5) [@nyiso_InstalledCapacityManual_2026] via the [RDP](https://github.com/switchbox-data/rate-design-platform/blob/main/data/nyiso/zone_mapping/generate_zone_mapping_csv.py). Hourly capacity MC signals are built in the [RDP](https://github.com/switchbox-data/rate-design-platform/blob/main/utils/pre/marginal_costs/generate_supply_capacity_mc.py).
...
2. **Derivation.** For each constraint group, the [derivation script](https://github.com/switchbox-data/rate-design-platform/blob/main/data/nyiso/transmission/derive_tx_values.py) computes a $/kW-yr value via a cumulative-secant aggregation over its member projects.
3. **Paying-locality aggregation.** Each constraint group is mapped to one or more paying localities (ROS, LHV, NYC, LI) via the nested-locality scheme in the [derivation script](https://github.com/switchbox-data/rate-design-platform/blob/main/data/nyiso/transmission/derive_tx_values.py); per-paying-locality costs are averaged within group.
4. **Hourly allocation.** For each utility, the [allocation script](https://github.com/switchbox-data/rate-design-platform/blob/main/utils/pre/marginal_costs/bulk_tx_nyiso.py) identifies System Capability Resource (SCR) hours as the top 40 load hours in each of summer and winter (80 total) within the utility's nested-locality load. Each constraint-group annual value is spread across those 80 hours using a two-level weight:
...
5. **Utility blend.** The [blend script](https://github.com/switchbox-data/rate-design-platform/blob/main/utils/pre/marginal_costs/generate_bulk_tx_mc.py) combines each utility's paying-locality streams using the utility's capacity-obligation shares from the zone mapping, yielding the utility's hourly bulk-TX $/kW-hr.
...
[^cite-bulk-tx]: Project-level inputs trace back to NYISO Public Policy Transmission Planning and Competitive System Planning Process (CSPP) benefit studies, plus the NiMo MCOS bulk-TX entry [@nyiso_ACTransmissionPublic_2018; @nyiso_ACTransmissionAddendum_2019; @nyiso_LongIslandOffshore_2023]. Constraint-group $/kW-yr values are computed by the [RDP](https://github.com/switchbox-data/rate-design-platform/blob/main/data/nyiso/transmission/derive_tx_values.py). Hourly allocation to the 80-hour SCR window with two-level weighting is implemented in the [RDP](https://github.com/switchbox-data/rate-design-platform/blob/main/utils/pre/marginal_costs/bulk_tx_nyiso.py); the per-utility blend is assembled by the [RDP](https://github.com/switchbox-data/rate-design-platform/blob/main/utils/pre/marginal_costs/generate_bulk_tx_mc.py). See the [RDP](https://github.com/switchbox-data/rate-design-platform/blob/main/context/code/marginal_costs/ny_bulk_tx_marginal_costs.md) for end-to-end pipeline documentation.
...
[^cite-eia930]: EIA Hourly Electric Grid Monitor (Form 930) — hourly demand by balancing authority. [@eia_HourlyElectricGridMonitor_2025]. Hourly sub-TX + distribution MC signals are built by the [RDP](https://github.com/switchbox-data/rate-design-platform/blob/main/utils/pre/marginal_costs/generate_utility_tx_dx_mc.py).
...
[^script-rr-mid]: BAT values, per-customer economic burden, and cross-subsidy are computed by CAIRO during the run. The [RDP](https://github.com/switchbox-data/rate-design-platform/blob/main/utils/mid/compute_subclass_rr.py) then applies the per-customer split between HP and non-HP subclasses. BAT results are consolidated across utilities by the [RDP](https://github.com/switchbox-data/rate-design-platform/blob/main/utils/post/build_master_bat.py).
...
[^script-supply-held]: From the supply tariff alone, the decomposition into a marginal-cost share and a residual share is not uniquely identified — any pair of shares that sum to the observed bill is consistent with the tariff. See the [RDP](https://github.com/switchbox-data/rate-design-platform/blob/main/context/methods/bat_mc_residual/passthrough_cost_allocation_and_rate_design.md) for the full argument.
...
[^script-fair-rate]: The fair seasonal flat delivery rate is derived by the [RDP](https://github.com/switchbox-data/rate-design-platform/blob/main/utils/mid/compute_subclass_rr.py).
...
[^script-eh-rate]: The electric heating rate uses the same [RDP calibration script](https://github.com/switchbox-data/rate-design-platform/blob/main/utils/mid/compute_subclass_rr.py) as the HP seasonal rate, with the subclass definition widened to include electric resistance customers.
...
[^lmi-cite]: EAP/EEAP credit amounts by utility and tier are committed in the [RDP](https://github.com/switchbox-data/rate-design-platform/blob/main/utils/post/data/ny_eap_credits.yaml), compiled from utility filings and public EAP program pages as of February 2026.
...
[^lmi-script]: LMI tier assignment and discounts are applied by the [RDP](https://github.com/switchbox-data/rate-design-platform/blob/main/utils/post/apply_ny_lmi_to_master_bills.py), using [RDP](https://github.com/switchbox-data/rate-design-platform/blob/main/utils/post/lmi_common.py) shared tier logic and [RDP](https://github.com/switchbox-data/rate-design-platform/blob/main/utils/post/data/ny_eap_credits.yaml) EAP credit tables.
...
GitHub](https://github.com/switchbox-data/rate-design-platform/tree/main/utils/pre/dist_mc).
...
GitHub](https://github.com/switchbox-data/rate-design-platform/tree/main/utils/pre/dist_mc/cenhud).
...
The full analysis script, cell-level workbook references, and year-by-year outputs are [available on
GitHub](https://github.com/switchbox-data/rate-design-platform/tree/main/utils/pre/dist_mc/nyseg).
...
The full analysis script and outputs are [available on GitHub](https://github.com/switchbox-data/rate-design-platform/tree/main/utils/pre/dist_mc/rge).
...
enter after 2029. The full analysis script and outputs are [available on GitHub](https://github.com/switchbox-data/rate-design-platform/tree/main/utils/pre/dist_mc/psegli).
...
GitHub](https://github.com/switchbox-data/rate-design-platform/blob/main/utils/pre/dist_mc/nimo/nimo_project_classifications.csv).
...
[^nimo_peak]: 2024 actual peak from NiMo's Peak Load Forecast (March 2025), used as a CLI input to the analysis script. The peak is not read from the MCOS workbook itself [@nimo_Docket19E02832025_2025a].

The full analysis is in a [script on GitHub](https://github.com/switchbox-data/rate-design-platform/blob/main/utils/pre/dist_mc/nimo/analyze_nimo_mcos.py). We averaged the real (FY2026-dollar)
