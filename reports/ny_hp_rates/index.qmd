...
- Wholesale electricy prices are volatile, reflecting that the underlying cost of generating electricity changes based
...
- We find that at present, HP customers are systematically paying more than their cost-of-service for delivery: their
  average cost-of-service is $X per year, compared to $Y per year for non-HP customers, but they pay an average of $Z.
...
- It would work in the simplest way possible, by reducing the winter delivery rate to a level that maximizes bill
  alignment for heat pump customers.
...
[^pseg_mcos]:
    See PSEG-LI's 2025 MCOS filing, [Exhibit 1](TODO_documentcloud_url_pseg_mcos_exhibit1) [@TODO_pseg_mcos_2025].
    [^pseg_mva]:
...
[^lipa_peak]:
    2024 actual non-coincident peak for Zone K (Long Island), [Table I-4a](TODO_documentcloud_url_gold_book_table_i4a)
    of the 2025 NYISO Gold Book [@TODO_nyiso_gold_book_2025].
...
We use the **total (Sub-TX + Distribution) of
~$1.67/kW-yr** (incremental diluted, levelized) as our distribution marginal cost input for LIPA. Unlike our initial analysis which used only the Distribution component (~**
(incremental diluted, levelized) as our distribution marginal cost input for
LIPA. Unlike our initial analysis which used only the Distribution component
(~**
(incremental diluted, levelized) as our distribution marginal cost input for
LIPA. Unlike our initial analysis which used only the Distribution component
(~\*\*
(incremental diluted, levelized) as our distribution marginal cost input for LIPA. Unlike our initial analysis which
used only the Distribution component (\~$0.46/kW-yr), the reclassification of all T-Substation projects as local
sub-transmission means both components are included — consistent with how we treat sub-TX for all other utilities.
...
[^nimo_mcos]:
    See [Exhibit 1](TODO_documentcloud_url_nimo_mcos_exhibit1) of NiMo's 2025 MCOS filing [@TODO_nimo_mcos_2025].
...
[^gold_book]:
    See [Table VII](TODO_documentcloud_url_gold_book_table_vii) of the 2025 NYISO Gold Book
    [@TODO_nyiso_gold_book_2025].
