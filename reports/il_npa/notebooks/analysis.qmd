...
#| fig-cap: "Stacked PRP vs NPA cost components for a single example census block (170314407001003): pipeline replacement on the left vs single- and multi-family electrification, grid upgrade, and gas decommissioning on the right."
#| fig-height: 9
#| fig-width: 8
...
#| fig-cap: "Same single-block PRP vs NPA decomposition with scattershot electrification NPV added to the PRP side of the comparison."
#| fig-height: 9
#| fig-width: 8
...
# Stack order: factor levels reversed by position_stack, so the LAST level
# ends up at the bottom of the stack and at the LEFT after coord_flip.
# Levels c("remain", "mixed", "res") → res on left, mixed middle, remain on right.
...
# Segment midpoints along the y axis (horizontal after coord_flip)
res_mid <- n_miles_res / 2
mixed_mid <- n_miles_res + n_miles_mixed / 2
remain_mid <- n_miles_res + n_miles_mixed + n_miles_remaining / 2

# Bar is centered at categorical x = 1, width = 0.5 → spans 0.75..1.25
bar_top <- 1.25
bar_bot <- 0.75
...
ggplot() +
  ...
  # Residential callout above bar with leader line
  annotate("segment",
    x = bar_top, xend = 1.55, y = res_mid, yend = res_mid,
    color = sb_colors[["midnight"]], linewidth = 0.4
  ) +
  annotate("text",
    x = 1.6, y = res_mid,
    label = paste0("Planned fully residential\n(", round(n_miles_res), " miles)"),
    color = sb_colors[["midnight"]],
    hjust = 0, vjust = 0, size = 3.5, fontface = "bold"
  ) +
  # Mixed callout below bar with leader line
  annotate("segment",
    x = bar_bot, xend = 0.45, y = mixed_mid, yend = mixed_mid,
    color = sb_colors[["saffron"]], linewidth = 0.4
  ) +
  annotate("text",
    x = 0.4, y = mixed_mid,
    label = paste0("Planned mixed-zoning\n(", round(n_miles_mixed), " miles)"),
    color = sb_colors[["saffron"]],
    hjust = 0, vjust = 1, size = 3.5, fontface = "bold"
  ) +
  # Remaining label centered above the gray segment
  annotate("text",
    x = 1.6, y = remain_mid,
    label = paste0(
      "Remaining PRP miles\n(",
      format(round(n_miles_remaining), big.mark = ","),
      " miles)"
    ),
    color = "gray30",
...
# Levels chosen so coord_flip puts planned on LEFT, remain on RIGHT
# (position_stack reverses factor order: highest level → bottom of stack → LEFT after flip)
plot_data <- data.frame(
  category = factor(
    c("remain", "planned"),
...
planned_mid <- n_miles_planned / 2
remain_mid <- n_miles_planned + n_miles_remaining / 2

# Bar centered at categorical x = 1, width = 0.5 → spans 0.75..1.25
bar_top <- 1.25
...
ggplot() +
  ...
  # Planned callout above bar with leader line
  annotate("segment",
    x = bar_top, xend = 1.55, y = planned_mid, yend = planned_mid,
    color = sb_colors[["sky"]], linewidth = 0.4
  ) +
  annotate("text",
    x = 1.6, y = planned_mid,
    label = paste0("Planned PRP miles\n(", round(n_miles_planned), " miles)"),
    color = sb_colors[["sky"]],
    hjust = 0, vjust = 0, size = 3.5, fontface = "bold"
  ) +
  # Remaining label centered above the gray segment
...
plot_data <- data.frame(
  category = factor(c("remain"), levels = c("remain")),
  miles = c(total_prp_miles)
)
...
ggplot() +
  ...
  annotate("text",
    x = 1.6, y = total_prp_miles / 2,
    label = paste0(
      "Remaining PRP miles\n(",
      format(round(total_prp_miles), big.mark = ","),
      " miles)"
    ),
    color = "gray30",
    hjust = 0.5, vjust = 0, size = 3.5, fontface = "bold"
  ) +
  scale_fill_manual(values = c("remain" = "gray80")) +
  scale_y_continuous(expand = expansion(mult = c(0, 0.02))) +
  scale_x_discrete(expand = expansion(add = c(0.6, 0.9))) +
  coord_flip(clip = "off") +
  labs(y = "", x = "") +
